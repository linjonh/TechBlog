...
打开终端窗口，执行命令：/usr/libexec/java\_home -V
...
执行命令：ls -l /Library/Java/JavaVirtualMachines/jdk1.8.0\_144.jdk/Contents/Home/bin/
...
若第一次配置环境变量，使用“touch .bash\_profile” 创建一个.bash\_profile的隐藏配置文件。
  
若配置文件.bash\_profile已存在，则直接使用"open -e .bash\_profile"命令，打开配置文件。
