...
（2）database\_name：所要建立数据库的名称
  
（3）logical\_file\_name：逻辑主文件名
  
（4）master\_file\_size：主文件名
  
（5）initial\_file\_name：初始文件大小（单位为MB）
  
（6）max\_file\_size：最大文件大小
  
（7）per\_growth\_size：文件每次增加大小
