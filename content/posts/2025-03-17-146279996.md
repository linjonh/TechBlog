...
![生成 AI
科技感封面.png](https://i-blog.csdnimg.cn/img\_convert/3f0a26e8a5aed6327734e5032c2b5257.png)
#### 第一部分：AI开发的背景与历史
##### 1.1 人工智能的起源与发展
...
\text{softmax}\left(\frac{QK^T}{\sqrt{d\_k}}\right) V
Attention(Q,K,V)=softmax(dk​ ​QKT​)V
其中，`Q`、`K`、`V`分别代表查询（Query）、键（Key）和值（Value），`d\_k`是键向量的维度。
2. \*\*多头注意力（Multi-Head Attention）\*\*
...
dataset = load\_dataset("csv", data\_files="path/to/your/dataset.csv")
3. \*\*微调模型\*\*
...
trainer.train()
4. \*\*保存和加载微调后的模型\*\*
trainer.save\_model("path/to/save/model")
\* \* \*
##### 3.4 LLM的挑战与未来
...
# 提取嵌入
embeddings = outputs.last\_hidden\_state.mean(dim=1)
...
model\_name = "bert-base-uncased"
tokenizer = AutoTokenizer.from\_pretrained(model\_name)
model = AutoModelForSequenceClassification.from\_pretrained(model\_name, num\_labels=3)
# 加载数据集
...
tokenized\_datasets = dataset.map(preprocess\_function, batched=True)
# 定义训练参数
training\_args = TrainingArguments(
output\_dir="./results",
...
test\_case = generator(prompt, max\_length=50)
print(test\_case[0]["generated\_text"])
\* \* \*
###### 5.2.5 应用部署
...
# 示例对话
user\_input = "Hello, how are you?"
response = chatbot(user\_input, max\_length=50)
...
# 保存量化后的模型
torch.save(quantized\_model.state\_dict(), "path/to/save/quantized\_model.pth")
\* \* \*
##### 5.4 LLM的挑战与解决方案
...
# 示例数据
data = pd.read\_csv("path/to/your/dataset.csv")
# 数据清洗
data = data.dropna() # 删除缺失值
data = data.sample(frac=1).reset\_index(drop=True) # 打乱数据
# 数据编码
def encode\_text(text):
return tokenizer(text, padding="max\_length", truncation=True, max\_length=128, return\_tensors="pt")
data["encoded"] = data["text"].apply(encode\_text)
\* \* \*
##### 6.4 模型选择与训练
\*\*模型选择\*\* ：
...
# 加载预训练模型
model = AutoModelForSequenceClassification.from\_pretrained("bert-base-uncased", num\_labels=3)
# 准备数据集
dataset = Dataset.from\_pandas(data)
# 数据预处理
def preprocess\_function(examples):
return tokenizer(examples["text"], truncation=True, padding="max\_length", max\_length=128)
tokenized\_datasets = dataset.map(preprocess\_function, batched=True)
# 定义训练参数
training\_args = TrainingArguments(
output\_dir="./results",
...
# 评估模型
predictions = trainer.predict(tokenized\_datasets["test"])
y\_true = tokenized\_datasets["test"]["labels"]
y\_pred = predictions.predictions.argmax(-1)
# 打印评估报告
print(classification\_report(y\_true, y\_pred, target\_names=["Negative", "Neutral", "Positive"]))
# 保存评估报告
with open("path/to/save/evaluation\_report.txt", "w") as f:
f.write(classification\_report(y\_true, y\_pred, target\_names=["Negative", "Neutral", "Positive"]))
\* \* \*
##### 6.6 部署与维护
\*\*部署平台\*\* ：
...
# 加载模型
model\_data = "path/to/save/model.tar.gz"
pytorch\_model = PyTorchModel(
model\_data=model\_data,
role="SageMakerRole",
framework\_version="1.8.0",
entry\_point="inference.py"
)
# 部署模型
predictor = pytorch\_model.deploy(
