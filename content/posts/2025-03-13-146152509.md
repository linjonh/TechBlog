...
1. **输入连接**
   ...
   d
   n
   z\_{m\_1}^{d\_1}, z\_{m\_2}^{d\_2}, \dots, z\_{m\_n}^{d\_n}
...
3. **优化目标**
   ...
   ∥
   2
   J\_{tr} = \left\| \hat{z}\_m^k - z\_m^k \right\|^2
...
3. **对抗训练**
   ...
   \begin{aligned} \min\_{G} \max\_{D} J\_{adv}^{t}(D, G) &= \mathbb{E}\_{x \sim p\_{z\_{tr}}(x)}[\log D(x)] \\ &+ \mathbb{E}\_{z \sim p\_{z\_t}(z)}\left[\log \left(1 - D(z\_g)\right)\right] \end{aligned}
...
d
v
J\_{total} = J\_{task} + \lambda J\_{adv}
