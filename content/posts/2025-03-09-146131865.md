---
layout: post
title: "MySQL创建表"
date: 2025-03-09 15:13:54 +0800
description: "(14, '林冲', 35, '男', '豹子头', '八十万禁军教头'),(3, '猪八戒', 300, '男', '天蓬元帅', '净坛使者'),(1, '孙悟空', 500, '男', '齐天大圣', '美猴王'),(2, '唐僧', 30, '男', '三藏法师', '御弟圣僧'),(10, '张飞', 45, '男', '翼德', '车骑将军'),(11, '曹操', 50, '男', '曹阿瞒', '魏武帝'),(15, '武松', 28, '男', '行者', '打虎英雄'),"
keywords: "MySQL创建表"
categories: ['未分类']
tags: ['数据库', 'Mysql']
artid: "146131865"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=146131865
    alt: "MySQL创建表"
render_with_liquid: false
featuredImage: https://bing.ee123.net/img/rand?artid=146131865
featuredImagePreview: https://bing.ee123.net/img/rand?artid=146131865
cover: https://bing.ee123.net/img/rand?artid=146131865
image: https://bing.ee123.net/img/rand?artid=146131865
img: https://bing.ee123.net/img/rand?artid=146131865
---

<div class="blog-content-box">
 <div class="article-header-box">
  <div class="article-header">
   <div class="article-title-box">
    <h1 class="title-article" id="articleContentId">
     MySQL创建表
    </h1>
   </div>
  </div>
 </div>
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   <link href="../../assets/css/kdoc_html_views-1a98987dfd.css" rel="stylesheet"/>
   <link href="../../assets/css/ck_htmledit_views-704d5b9767.css" rel="stylesheet"/>
   <div class="htmledit_views" id="content_views">
    <blockquote>
     <h3>
      这里我们简单创建一张关于四大名著的表
     </h3>
    </blockquote>
    <h3>
     一、新建建数据库db_ck
    </h3>
    <h4>
     查看一下当前的数据库
    </h4>
    <p>
     <img alt="" height="339" src="https://i-blog.csdnimg.cn/direct/f1797c11fc3c43498e368571d0a4503a.png" width="1010"/>
    </p>
    <h4>
     创建数据库db_ck
    </h4>
    <pre><code class="language-sql">mysql&gt; create database db_ck;
Query OK, 1 row affected (0.03 sec)
</code></pre>
    <h4>
     查看以 db 开头的的数据库
    </h4>
    <p>
     <img alt="" height="262" src="https://i-blog.csdnimg.cn/direct/2bdf83917c2846a48aafc4240dacdf5c.png" width="825"/>
    </p>
    <h3>
     二、创建表
    </h3>
    <h4>
     新建表t_hero
    </h4>
    <pre><code class="language-sql">mysql&gt; use db_ck
Database changed
mysql&gt; create table t_hero(
    -&gt; id int,
    -&gt; name varchar(255),
    -&gt; age int,
    -&gt; gender char(2),
    -&gt; nickname varchar(100),
    -&gt; title varchar(100)
    -&gt; );
Query OK, 0 rows affected (0.07 sec)</code></pre>
    <h4>
     根据四大名著（《红楼梦》、《西游记》、《三国演义》、《水浒传》）中的主要人物，插入数据
    </h4>
    <blockquote>
     <p>
      -- 西游记人物
      <br/>
      INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
      <br/>
      (1, '孙悟空', 500, '男', '齐天大圣', '美猴王'),
      <br/>
      (2, '唐僧', 30, '男', '三藏法师', '御弟圣僧'),
      <br/>
      (3, '猪八戒', 300, '男', '天蓬元帅', '净坛使者'),
      <br/>
      (4, '沙悟净', 400, '男', '卷帘大将', '金身罗汉');
     </p>
     <p>
      -- 红楼梦人物
      <br/>
      INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
      <br/>
      (5, '贾宝玉', 16, '男', '绛洞花主', '荣国府公子'),
      <br/>
      (6, '林黛玉', 15, '女', '潇湘妃子', '金陵十二钗'),
      <br/>
      (7, '薛宝钗', 17, '女', '蘅芜君', '贾府少奶奶');
     </p>
     <p>
      -- 三国演义人物
      <br/>
      INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
      <br/>
      (8, '刘备', 48, '男', '刘皇叔', '蜀汉昭烈帝'),
      <br/>
      (9, '关羽', 46, '男', '美髯公', '汉寿亭侯'),
      <br/>
      (10, '张飞', 45, '男', '翼德', '车骑将军'),
      <br/>
      (11, '曹操', 50, '男', '曹阿瞒', '魏武帝'),
      <br/>
      (12, '诸葛亮', 40, '男', '卧龙先生', '蜀汉丞相');
     </p>
     <p>
      -- 水浒传人物
      <br/>
      INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
      <br/>
      (13, '宋江', 38, '男', '及时雨', '梁山泊总兵都头领'),
      <br/>
      (14, '林冲', 35, '男', '豹子头', '八十万禁军教头'),
      <br/>
      (15, '武松', 28, '男', '行者', '打虎英雄'),
      <br/>
      (16, '鲁智深', 32, '男', '花和尚', '梁山步军头领');
     </p>
    </blockquote>
    <pre><code class="language-sql">#西游记
mysql&gt; INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
    -&gt; (1, '孙悟空', 500, '男', '齐天大圣', '美猴王'),
    -&gt; (2, '唐僧', 30, '男', '三藏法师', '御弟圣僧'),
    -&gt; (3, '猪八戒', 300, '男', '天蓬元帅', '净坛使者'),
    -&gt; (4, '沙悟净', 400, '男', '卷帘大将', '金身罗汉');
Query OK, 4 rows affected (0.02 sec)
Records: 4  Duplicates: 0  Warnings: 0
#红楼梦
mysql&gt; INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
    -&gt; (5, '贾宝玉', 16, '男', '绛洞花主', '荣国府公子'),
    -&gt; (6, '林黛玉', 15, '女', '潇湘妃子', '金陵十二钗'),
    -&gt; (7, '薛宝钗', 17, '女', '蘅芜君', '贾府少奶奶');
Query OK, 3 rows affected (0.01 sec)
Records: 3  Duplicates: 0  Warnings: 0
#三国演义
mysql&gt; INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
    -&gt; (8, '刘备', 48, '男', '刘皇叔', '蜀汉昭烈帝'),
    -&gt; (9, '关羽', 46, '男', '美髯公', '汉寿亭侯'),
    -&gt; (10, '张飞', 45, '男', '翼德', '车骑将军'),
    -&gt; (11, '曹操', 50, '男', '曹阿瞒', '魏武帝'),
    -&gt; (12, '诸葛亮', 40, '男', '卧龙先生', '蜀汉丞相');
Query OK, 5 rows affected (0.01 sec)
Records: 5  Duplicates: 0  Warnings: 0
#水浒传
mysql&gt; INSERT INTO t_hero (id, name, age, gender, nickname, title) VALUES
    -&gt; (13, '宋江', 38, '男', '及时雨', '梁山泊总兵都头领'),
    -&gt; (14, '林冲', 35, '男', '豹子头', '八十万禁军教头'),
    -&gt; (15, '武松', 28, '男', '行者', '打虎英雄'),
    -&gt; (16, '鲁智深', 32, '男', '花和尚', '梁山步军头领');
Query OK, 4 rows affected (0.01 sec)
Records: 4  Duplicates: 0  Warnings: 0

</code></pre>
    <h4>
     查看内容
    </h4>
    <pre><code class="language-sql">-- 查询所有人物
SELECT * FROM t_HERO;

-- 查询所有女性角色
SELECT * FROM t_hero WHERE gender = '女';</code></pre>
    <p>
     <img alt="" height="632" src="https://i-blog.csdnimg.cn/direct/5e107ca0697b43aa9487c9c2e813270c.png" width="1073"/>
    </p>
    <p>
     <img alt="" height="234" src="https://i-blog.csdnimg.cn/direct/5ffd4881b681483e95d5e56d6f2a1b0e.png" width="955"/>
    </p>
   </div>
  </div>
 </article>
 <p alt="68747470733a2f2f626c6f67:2e6373646e2e6e65742f72726c31383231353832313838392f:61727469636c652f64657461696c732f313436313331383635" class_="artid" style="display:none">
 </p>
</div>


