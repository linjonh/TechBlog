...
db.setPassword(“psw”);作用：数据库文件没有密码的时候，配合db.setConnectOptions(“QSQLITE\_CREATE\_KEY”);为数据库文件添加密码，当数据库文件有密码的时候，单独用db.setPassword(“psw”);使用密码打开数据库。

总结：
  
我们为数据库加密一般都在创建的时候加密，故创建的时候db.setPassword(“psw”);设置密码，db.setConnectOptions(“QSQLITE\_CREATE\_KEY”);创建为其密码；
  
调用数据库时不要出现db.setConnectOptions(“QSQLITE\_CREATE\_KEY”);，另外注意db.close()操作。

后续发现每次新生成数据库文件时仍然会读取错误，解决办法是将db.setConnectOptions(“QSQLITE\_CREATE\_KEY”);直接改为设置加密方式