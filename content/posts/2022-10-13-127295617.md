...
./initdb -D /usr/local/postgresql/data -U postgre --encoding=UTF8 --lc-collate=en\_US.UTF-8 --lc-ctype=en\_US.UTF-8
...
./pg\_ctl -D /usr/local/postgresql/data/ start
...
保存退出后，执行 vim pg\_hba.conf   翻到最后

![](https://i-blog.csdnimg.cn/blog_migrate/1b85c95cbcb9584d1b4dc654f100437d.png)

重启数据库，再次进行连接

pg\_ctl -D /usr/local/postgresql/data/  restart
