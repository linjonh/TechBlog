...
3、修改后的文件内容如下，此时通过sudo命令，test\_user即可获得root权限
...
**libcudnn\_cnn\_train.so.8 is not a symbolic link**
  
参考
[libcudnn\_cnn\_train.so.8 is not a symbolic link此类问题解决方法](https://blog.csdn.net/qq_36577574/article/details/119174973)
，新建软链接解决
...
注意，安装路径默认是当前用户的home目录下的anaconda3，例如：/home/test\_user/anaconda3。在询问是否执行conda initialization时，选择yes，这样就不需要自己配置环境变量了（默认写入~/.bashrc文件）。
