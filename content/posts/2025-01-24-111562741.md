...
Pygame 使用内部定义的 Surface 对象表示所有载入的图像，其中.get\_rect()方法返回一个覆盖图像的矩形 Rect 对象
...
椭圆弧形 pygame.draw.arc(Surface, color, Rect, start\_angle, stop\_angle, width=0)
...
直线 pygame.draw.line(Surface, color, start\_pos, end\_pos, width=1)
...
无锯齿线 pygame.draw.aaline(Surface, color, start\_pos, end\_pos, blend=1)
...
Font 类的绘制方法 Font.render\_to(surf, dest, text, fgcolor=None, bgcolor=None, rotation=0, size=0) ——> Rect
