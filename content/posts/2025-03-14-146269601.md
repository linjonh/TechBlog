...
x\_data现在包含了数据集中的特征，是除了最后一列之外的所有数据。
  
y\_data现在包含了数据集中的特征，选择了数据数组 data 的所有行和最后一列。
  
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/5fc7abe760a24bb4b7bf784d457ea75d.png)

定义了一个名为 plot\_logi 的函数，该函数将数据集根据标签（0或1）分割成两组，并分别使用散点图绘制这两组数据。
...
从数据集 data 中提取特征（x\_data）和标签（y\_data）。
  
在特征矩阵 x\_data 的最左侧添加了一列全为1的数据，这个新列是通过 np.ones 函数生成的，其长度与 x\_data 的行数相同，
  
使用 np.concatenate 函数沿着列方向（axis=1）将其与 x\_data 拼接起来，形成新的特征矩阵 X\_data。
  
![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/3280f46d4304423d88b68686fdc5848e.png)

sigmoid函数：将任意实数值映射到(0,1)区间内；
  
损失函数 (cost\_):计算逻辑回归模型的损失（成本）；
...
使用x\_data（特征）和y\_data（标签）来训练逻辑回归模型
