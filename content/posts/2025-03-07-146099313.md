...
keywords: "基于Kubernetes部署MySQL主从集群"
categories: ['未分类']
...
#### 关键注意事项：
  ...
  1. **主节点高可用** ：此方案主节点单点，若需高可用，需结合 **Orchestrator** 或 **ProxySQL** 实现故障转移。
  2. **数据持久化** ：确保 `storageClassName` 与实际存储系统匹配（如 `rook-cephfs`、`nfs`）。
