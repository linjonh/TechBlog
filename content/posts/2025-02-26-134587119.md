...
SDL\_PumpEvents：用于更新事件队列并处理事件

SDL\_PeepEvents：用于从事件队列中获取特定类型的事件

如果 remaining\_time 大于 0，则等待一会

接着给 remaining\_time 赋值 0.01
...
SDL\_KEYDOWN：检测到按键的按压，如果是 escape，则退出

SDL\_QUIT：如果类型为 QUIT（停止），则退出
