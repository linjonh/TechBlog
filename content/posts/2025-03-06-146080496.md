...
\- 包含四个纯虚函数：`Boil()`（煮水）、`Brew()`（冲泡）、`PourInCup()`（倒入杯中）和
`PutSomething()`（加入辅料），这些函数需要在派生类中具体实现。
...
\- 实现 `Boil()`、`Brew()`、`PourInCup()` 和 `PutSomething()`
四个纯虚函数，具体描述制作咖啡的步骤，如煮开水、冲泡咖啡、倒入杯中、加入糖和牛奶。
