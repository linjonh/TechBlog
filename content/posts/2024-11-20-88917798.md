...
如果是AAC，AAC的frame\_size＝1024，如果每帧一个包的话，每个rtp之间时间差为1024
...
1、文件中计算音视频pts，都是根据每秒算出有多少个音视频帧， 然后根据time\_base，算出每帧占有多少个时间单位，即为pts差值
  
2、rtp传输中时间单位与采样率一致，传输多少个样本即为增加多少时间
  
3、重点理解time\_base是一种度量单位，pts是基于此种单位的数值计算
...
AVPacket->flags & AV\_PKT\_FLAG\_KEY
...
time = stream->duration \* stream->time\_base;
