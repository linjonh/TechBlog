...
select \* from t\_user where username like ? and address like ?
  
如果第一个不成立,第二个条件成立
  
select \* from t\_user where and address like ?
...
如果多表联查的二级缓存，user 表 left join user\_order 表 on user.id = user\_order.user\_id
  
我们考虑这样一种情况，该联查执行两次，第二次联查前更新 user\_order 表，如果只使用 cache 配置，将会查不到更新的 user\_orderxi，因为两个 mapper.xml 的作用域不同，要想合到一个作用域，就需要用到 cache-ref
