...
print(device)</code></pre>
    ...
    <p>
     ...
     <br/>
     <img alt="" height="129" src="https://i-blog.csdnimg.cn/direct/845a0c4be4d542f0b41200551aa9092f.png" width="535"/>
    </p>
    <p>
     这里下载到data目录，如果已有数据则不会下载。这段代码可以实现数据向tensor的转换：
     <img alt="" height="203" src="https://i-blog.csdnimg.cn/direct/b9ad36cd47894cd08a678644a92fa2df.png" width="1039"/>
    </p>
    <p>
...
# 当然也可以用 torch.utils.data.Dataset 实现人为划分</code></pre>
    <pre><code class="language-python">type(train_ds[0]) # 元组，第一个元素是图片，第二个元素是标签</code></pre>
    <p>
     如果使用了数据类型变换：
    </p>
    <pre><code class="language-python">img_tensor, label = train_ds[0]
img_tensor.shape  #img这时是一个tensor，shape=(1, 28, 28)</code></pre>
    <p>
     在PyTorch中，
     <code>
      DataLoader
    ...
    <pre><code class="language-python"># 从数据集到dataloader
train_loader = torch.utils.data.DataLoader(train_ds, batch_size=32, shuffle=True) #batch_size分批，shuffle洗牌
val_loader = torch.utils.data.DataLoader(test_ds, batch_size=32, shuffle=False)</code></pre>
    <p>
...
model = NeuralNetwork()</code></pre>
    ...
    <pre><img alt="" height="197" src="https://i-blog.csdnimg.cn/direct/b1d38c4cfead4633a1ed1f05d072e3fa.png" width="820">
</img></pre>
    <p>
     在训练之前需要测试一下模型能不能用，所以我们随机一个或者从样本拿一个，同尺寸就行：
    </p>
    <pre><code class="language-python">#为了查看模型运算的tensor尺寸
x = torch.randn(32, 1, 28, 28)
print(x.shape)
logits = model(x) # 把x输入到模型中，得到logits
print(logits.shape)</code></pre>
    <p>
     然后开始训练，pytorch的训练需要自行实现，包括定义损失函数、优化器、训练步，训练
    </p>
    <pre><code class="language-python"># 1. 定义损失函数 采用交叉熵损失
loss_fct = nn.CrossEntropyLoss() #内部先做softmax，然后计算交叉熵
# 2. 定义优化器 采用SGD
# Optimizers specified in the torch.optim package,随机梯度下降
optimizer = torch.optim.SGD(model.parameters(), lr=0.001, momentum=0.9)</code></pre>
    <pre><code class="language-python">from sklearn.metrics import accuracy_score # sk里面有一个算子，可以计算准确率
...
def evaluating(model, dataloader, loss_fct): # 评估函数，评估也要做一次向前计算，不需要求梯度
    ...
    return np.mean(loss_list), acc</code></pre>
...
record = training(model, train_loader, val_loader, epoch, loss_fct, optimizer, eval_step=1000)</code></pre>
    <pre><code class="language-python">#画线要注意的是损失是不一定在零到1之间的
...
plot_learning_curves(record)  #横坐标是 steps</code></pre>
    <p>
     <img alt="" height="271" src="https://i-blog.csdnimg.cn/direct/6e8876e656a54c9eb515137e6ffb4230.png" width="509"/>
    </p>
    <pre><code class="language-python"># dataload for evaluating

model.eval() # 进入评估模式
loss, acc = evaluating(model, val_loader, loss_fct)
print(f"loss:     {loss:.4f}\naccuracy: {acc:.4f}")</code></pre>
    <p>
    </p>
   </div>
  </div>
 </article>
 <p alt="68747470733a2f2f626c6f672e:6373646e2e6e65742f77656978696e5f35353834333932312f:61727469636c652f64657461696c732f313436303234333736" class_="artid" style="display:none">
 </p>
</div>
