...
可参考ARM64体系结构编程与实践P373。在访问浮点寄存器之前需要将CPACR\_EL1寄存器的FPEN位域设置为0b11，否则会触发异常。FMOV指令编码中只用了7位来表示单精度浮点数，所以可以表示的浮点数常量是有限的，如下图（bcd和efgh分别表示高三位和低四位的编码）:
...
关于1021号中断的两种不同情况的示例如下图所示，情况一，SCR\_EL3.FIQ设置为1时，中断会直接路由到EL3：

![](https://i-blog.csdnimg.cn/direct/af2344af167f428c854f3da4cbad84c2.png)

情况二，SCR\_EL3.FIQ设置为0时，中断不会直接路由到EL3：
...
* GICR\_IPRIORITYR<n>用来设置PPI和SGI中断优先级
* GICD\_IPRIORITYR<n>用来设置SPI中断优先级
* LPI配置表保存了LPI的中断优先级

Binary Point Register寄存器将中断优先级分成两个字段，组优先级（group priority）即抢占优先级，用来决定是否能被抢占，在需要抢占时会比较，组优先级（group priority）；子优先级（subpriority）主要用于在抢占优先级和组优先级相同的情况下，进一步细化中断的处理顺序。中断阈值：对应的寄存器是ICC\_PMR\_EL1，PMR决定目标CPU的优先级阈值，GIC只有当pending中断的优先级高于这个中断优先级阈值，才会发送中断到CPU。若PMR为0，表示屏蔽了所有中断发送给CPU。运行优先级：对应的寄存器是ICC\_PMR\_EL1，这个寄存器中的值是CPU正在响应的中断的group priority，主要用于中断抢占的判断。GICv3支持中断抢占，当一个中断优先级同时满足下面条件会发生抢占：优先级高于CPU接口的优先级值PMR，group priority高于正在处理的running priority。
...
* 中断配置表：用来存储每个LPI中断的优先级和enable位，每个表项占8bit，第0位表示当前LPI中断是否开启，2~7表示当前LPI中断优先级，第1位是保留位，这是一张全局的表。该配置表的基地址存放在GICR\_PROPBASER.Physical\_Address域中，表项的个数可查看GICR\_PROPBASER.IDbits域获取（具体换算方式查看手册）
* 中断pending表：每个redistributor都有一个中断pending表，每个表项占1bit
* Device Table（全局唯一）：Device Table由OS软件来创建，OS会分配内存，创建Table并把表的基地址设置到GITS\_BASER.Physical\_Address，Device Table的相关参数在GITS\_BASER<n>寄存器中，主要包括表的类型GITS\_BASER.Type、表项的大小GITS\_BASER.Entry\_Size(例如8字节)、Table中每个page的大小GITS\_BASER.Page\_Size(例如64KB)、是否需要二级表GITS\_BASER.Indirect、设置表基地址GITS\_BASER.Physical\_Address、需要多少个表项2^(Device\_id)，Device\_id位宽在GITS\_TYPER.Devbits。Device Table支持1级或者2级表，Ddevice Table表项的内容由软件通过command queue来发送命令给硬件，由硬件来填充表项，例如可通过MAPD命令，把DevicelD映射到ITT表中，
...
* Interrupt Translation Table：ITT表项用来映射EventID到物理中断号INTID及ICID（Interrupt collection number），与ITT表相关的参数有：ITT表项大小GITS\_TYPER.ITT\_entry\_size (例如16字节)、ITT表项个数即在申请中断时请求的vector个数、ITT表的基地址（由OS来分配，存储在Device Table的表项中）。ITT表项的内容由软件通过command queue来发送命令给硬件，并由硬件来完成写入， ITT表项的组成如下图所示（可参考GICv3手册5.2.4节）：

  ![](https://i-blog.csdnimg.cn/direct/f90476dc526a43fe9ac3d221ea251641.png)
* Collection Table：该表的表项用来映射ICID到redistributor，这个表不需要在内存中分配内存，Collection Table表项的内容由软件通过command queue来发送命令给硬件，并由硬件来完成填充。软件需要告知redistributor的物理地址或者GICR TYPER.Processor\_Number，可通过MAPC命令来映射ICID到redistributor。在GIC初始化的时候就会遍历所有的redistributor，并且调用MAPC命令初始化，该表表项的组成如下图所示：
...
想要触发一个MSI（Message Signaled Interrupts）中断，必须由外设来向寄存器GITS\_TRANSLATER中写入EventID，且这个外设必须有统一的deviceID，并且必须注册在ITS表里面（可参考GICv3手册12.19.12节）。三个与GICv3的command queue相关的寄存器：GITS\_CBASER：指定command queue的大小和基地址，基地址必须64KB对齐，大小必须是4KB整数倍；GITS\_CREADR：ITS下一条要处理的command；GITS\_CWRITER:下一条要写入的command，如下图所示：
...
2.irq\_domain：在Linux内核中，irq\_domain是一个用于管理和映射中断请求的数据结构，特别是在中断控制器和设备驱动之间进行中断标识符（IRQ）的映射和转换。它是处理系统中断、尤其是复杂硬件平台（如ARM、RISC-V等）上的中断控制器和虚拟化中断的一种机制。在多核系统、虚拟化环境、或者复杂的中断控制器系统中，中断标识符（IRQ）可能是设备特定的或者平台特定的。这时，irq\_domain的作用就是将设备中断标识符（IRQ）映射到GIC或其他中断控制器所能识别的IRQ ID。不同的硬件平台可能会使用不同的中断控制器，不同的控制器有自己的中断标识符范围和规则，irq\_domain提供了一种机制，将设备的本地中断ID转换成系统中使用的全局中断ID。irq\_domain主要使用一个映射函数(map)来将设备的本地中断标识符映射到系统的全局IRQ ID，这是irq\_domain的核心功能。