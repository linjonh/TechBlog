...
我们前文讲过车辆声明报文，就是用在这个时候，由DoIP节点主动发送3次，用UDP协议向整个网络进行广播，目的端口是UDP\_DISCOVERY，源端口可以是UDP\_DISCOVERY，也可以动态分配。

#### ③ 诊断设备请求车辆信息

如果诊断设备没有收到DoIP节点的车辆声明报文，则需要主动请求车辆信息，使用UDP协议，目的端口是UDP\_DISCOVERY，源端口是动态定义的UDP\_TEST\_EQUIPMENT\_REQUEST。
