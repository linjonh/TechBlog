...
**命令执行**
：在应用层，可以使用一些命令来实现模式切换。使用 “su” 命令获取 root 权限，然后执行 “echo host > /sys/class/android\_usb/android0/function” 命令将设备切换到主机模式；执行 “echo device > /sys/class/android\_usb/android0/function” 命令将设备切换到设备模式。在终端中输入这些命令时，要确保设备已获取 root 权限，并且命令的路径和参数正确。
