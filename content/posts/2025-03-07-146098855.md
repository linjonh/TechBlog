...
驱动注册->分配cdev设备号（静态分配register\_chrdev\_region/动态分配alloc\_chrdev\_region）->初始化cdev（cdev\_init）->注册cdev（cdev\_add）->初始化硬件->设备操作函数的实现（open/read/write/close）->驱动注销（cdev\_del/unregister\_chrdev\_region）

### 3.Linux2.6开发的相关接口函数

#### ·alloc\_chrdev\_region()申请设备号函数
...
base\_minor：起始次设备号。跟系统指定准备从哪个次设备号开始使用。
...
#### ·unregister\_chrdev\_region()释放设备号函数
...
cdevp：填入经cdev\_init初始化完毕的结构体

dev\_num：经过alloc\_chrdev\_region申请的设备号
