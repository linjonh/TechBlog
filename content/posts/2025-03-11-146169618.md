...
错误地将两个层作为位置参数传递给tf.keras.Sequential构造函数，导致第二个参数被误解为name参数，而name必须是一个字符串，但用户传递了一个层实例。这导致在调用层时，TensorFlow无法正确创建name\_scope，从而引发错误。解决方法是把这两个层放在一个列表中，作为Sequential构造函数的第一个参数。
