...
cmd\_str = f'kubectl -n haha get deploy --token "{token}"'
cmd = shlex.split(cmd\_str)
try:
...
11. \_\_name\_\_是python文件里面的一个内置变量，当运行一个python文件的时候，这个内置变量默认的值为\_\_main\_\_，这个python文件被当成模块导入的时候，这个\_\_name\_\_变量的值就不是\_\_main\_\_了，这样就，如果要把这个python脚本作为模块使用的时候，这个里面的函数就要写到if \_\_name\_\_ = \_\_main\_\_里面，否则，别的脚本一导入这个模块，什么都没写，然后运行这个别的脚本文件，就会自动执行模块里面的函数。
...
25. \*\*alter\*\* \*\*table\*\* student \*\*add\*\* \*\*column\*\* \_gender\_ \*\*varchar\*\*(10)，给表添加新字段
