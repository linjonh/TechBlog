...
1. src\_img\_name = 'blue\_sky\_white\_clound\_002594.jpg';
2. img = imread(src\_img\_name);
4. % get binary image
5. gray\_img = rgb2gray(img);
6. T = graythresh(gray\_img);
7. bw\_img = im2bw(gray\_img, T);
9. % find the largest connected region
10. img\_reg = regionprops(bw\_img,  'area', 'boundingbox');
11. areas = [img\_reg.Area];
12. rects = cat(1,  img\_reg.BoundingBox);
...
1. [~, max\_id] = max(areas);
2. max\_rect = rects(max\_id, :);
4. % show the largest connected region
5. figure(2),
6. imshow(bw\_img);
7. rectangle('position', max\_rect, 'EdgeColor', 'r');
