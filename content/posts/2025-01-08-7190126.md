...
1）通过【程序】-》【Oracle - OraDb10g\_home1】-》【配置和移植工具】-》【Net Configuration Assistant】，运行“网络配置助手”工具：
...
先运行cmd命令，进入命令行模式，转到下面的目录：D:\oracle\product\10.2.0\db\_1\BIN【该目录下有exp.exe文件】
...
create temporary tablespace zfmi\_temp
  
tempfile 'D:\oracle\oradata\zfmi\zfmi\_temp.dbf'
...
下面的过程中，filename是已经被删除的数据文件，如果有多个，则需要多次执行；tablespace\_name是相应的表空间的名称。
...
SQL> drop tablespace tablespace\_name including contents;
...
default tablespace zfmi temporary tablespace zfmi\_temp;
...
3 将数据库中的表inner\_notify、notify\_staff\_relat导出
  
exp aichannel/aichannel@TESTDB2 file= d:datanewsmgnt.dmp tables=(inner\_notify,notify\_staff\_relat)
...
DROP ANY VIEW,EXP\_FULL\_DATABASE,IMP\_FULL\_DATABASE,
...
F:WorkOracle\_Databackup>imp userid=test/test full=y file=inner\_notify.dmp
...
file==/backup/ctgpc\_20030623.dmp
  
log==/backup/import\_20030623.log