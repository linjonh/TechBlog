...
然后在游戏里新建一个存档，根据自己的需求设置好，一定记得起一个比较特殊的名字，到时候好找！！！把要安装的服务器mod都选好配置好，不用进游戏。在创建存档界面这个时候就能看到自己的新建的存档了，点击“管理世界”——“打开世界文件夹”。新建一个文本文档cluster\_tokentxt，把复制的这个token粘贴进去。然后这个文件夹里的四个内容全部传到服务器上去。
...
服务器根目录下新建一个.sh文件run\_deldicated\_servers.sh.
...
**需要我们把mod传上去！网上之前的教程让修改dedicated\_server\_mods\_setup.lua这个文件方法是会导致有一部分mod安装失败的！！！**
你会看到你连进去的服务器怎么少mod了。
...
> **./donotstarvetogether\_dedicated\_server/mods/**
...
在我们刚才新建的run\_dedicated\_servers.sh，有一行代码是这样的：
