...
另外，检测到您服务器支持pdo\_sqlite扩展，您也可以修改数据库配置连接驱动为pdo\_sqlite试试！
...
第一种方法：把数据库配置连接驱动改为pdo\_sqlite
  
按照提示把数据库配置连接驱动修改未pdo\_sqlite，接下来照着图文操作下。
  
打开数据库配置文件/apps/config/database.php，找到'type'这一行，把sqlite改为pdo\_sqlite
