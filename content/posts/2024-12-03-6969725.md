...
PIPE\_ACCESS\_DUPLEX, //管道可以双向通信
  
PIPE\_TYPE\_BYTE,//数据以字节流的方式写入管道
  
PIPE\_UNLIMITED\_INSTANCES, //表示管道能够创建的最大实例数目 这里只和操作系统相关
...
WriteFile(hNamedPipe,(void\*)s.c\_str(),s.length()+1,&len,NULL);
...
if(!::WaitNamedPipe("\\\\.\\pipe\\path",NMPWAIT\_WAIT\_FOREVER) )  //连接管道
...
HANDLE h1=::CreateFile("\\\\.\\pipe\\path",GENERIC\_WRITE|GENERIC\_READ,FILE\_SHARE\_READ|FILE\_SHARE\_WRITE,NULL,OPEN\_EXISTING,FILE\_ATTRIBUTE\_NORMAL,NULL) ;  //打开管道
