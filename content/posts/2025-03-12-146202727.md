...
##### 2\. 特点
  ...
  * **内边距和外边距** ：可以设置垂直方向的内边距（`padding-top`、`padding-bottom`）和外边距（`margin-top`、`margin-bottom`），但水平方向的内边距和外边距可能不会按预期工作。
...
#### 四、总结
  ...
  * **转换显示方式** ：通过CSS的`display`属性可以灵活地改变元素的显示方式，实现更复杂的布局需求。
