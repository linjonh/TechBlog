...
保存登录态这里是这样的，将用户的登录信息传给后台，后台保存用户信息，同时用 open\_id 在后台换取一个SessionId  用换取的这个SessionId 存在缓存，做为登录态验证。
...
s\_url = "b.aspx?name=" + Label1.Text;
  
Response.Redirect(s\_url);
...
HttpCookie cookie\_name = new HttpCookie("name");
  
cookie\_name.Value = Label1.Text;
  
Reponse.AppendCookie(cookie\_name);
