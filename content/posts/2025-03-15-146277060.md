...
-- 解析主机名和服务名 resolve :: HostName -> ServiceName -> IO AddressInfo resolve hostname port = do addrInfo <- getAddrInfo (Just hints) (Just hostname) (Just port) return $ head addrInfo where hints = defaultHints { addrFlags = [AI\_PASSIVE] }
