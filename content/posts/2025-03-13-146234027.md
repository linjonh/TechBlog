...
* ​检查缓存有效性：YUM 会先检查本地缓存（/var/cache/yum/ 目录）是否过期。若缓存过期（由仓库配置的 metadata\_expire 参数决定）或不存在，​自动触发 yum makecache 流程​（即下载最新元数据并生成缓存）。
* ​直接使用缓存：若缓存有效，直接读取本地元数据（如 SQLite 数据库），无需重新下载。

**​2. 解析软件包名称与依赖**

* ​名称匹配：在元数据中搜索 nginx 软件包，确定其全名（如 nginx-1.20.1-1.el7.x86\_64）。
* ​依赖树构建：
...
* ​并行下载：
  + 从仓库 URL（如 http://mirror.centos.org/centos/7/os/x86\_64/Packages/）下载 nginx 及其依赖的 RPM 文件。
