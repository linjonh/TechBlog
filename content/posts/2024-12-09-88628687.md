...
指定活动纹理单元的目标纹理。必须是GL\_TEXTURE\_2D，GL\_TEXTURE\_CUBE\_MAP\_POSITIVE\_X，GL\_TEXTURE\_CUBE\_MAP\_NEGATIVE\_X，GL\_TEXTURE\_CUBE\_MAP\_POSITIVE\_Y，GL\_TEXTURE\_CUBE\_MAP\_NEGATIVE\_Y，GL\_TEXTURE\_CUBE\_MAP\_POSITIVE\_Z，或GL\_TEXTURE\_CUBE\_MAP\_NEGATIVE\_Z。
...
指定纹理的内部格式。必须是下列符号常量之一：GL\_ALPHA（按照alpha值存储纹理单元），GL\_LUMINANCE（按照亮度值存储纹理单元），GL\_LUMINANCE\_ALPHA（按照亮度值和alpha值存储纹理单元），GL\_RGB（按照r,g,b成分存储纹理单元），GL\_RGBA（按照r，g，b，alpha方式存储纹理单元）。
...
指定纹理数据的格式。必须匹配internalformat。下面的符号值被接受：GL\_ALPHA，GL\_RGB，GL\_RGBA，GL\_LUMINANCE，和GL\_LUMINANCE\_ALPHA。

**type**
  
指定纹理数据的数据类型。下面的符号值被接受：GL\_UNSIGNED\_BYTE，GL\_UNSIGNED\_SHORT\_5\_6\_5，GL\_UNSIGNED\_SHORT\_4\_4\_4\_4，和GL\_UNSIGNED\_SHORT\_5\_5\_5\_1。
