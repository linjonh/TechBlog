...
windows 下一个程序的dll搜索路径首先是程序目录，然后是一些系统目录。linux 下的可以通过设置 LD\_LIBRARY\_PATH 等方法来添加搜索路径，但是这些路径是定死的，如果要实现程序目录，就要使用链接时的rpath选项。rpath 适用于 elf 文件，不适用 pe。
...
如果链接时没使用 '-rpath' ，那么如果定义了 LD\_RUN\_PATH 就会使用该值作为 rpath
...
1. QMAKE\_RPATHDIR += :\'\$\$dir\'

注意QMAKE\_RPATHDIR最开始要加冒号，因为生成Makefile会自动在前面加上-Wl,-rpath,$$PWD
...
0x6ffffffb (FLAGS\_1)                    Flags: ORIGIN
...
@executable\_path : relative to the main executable
  

@loader\_path : relative to the referring binary
...
!isEmpty(TIGER\_COMPAT\_MODE) {
  

QMAKE\_LFLAGS\_SONAME = -Wl,-install\_name,@executable\_path/../PlugIns/
  

} else {
  

QMAKE\_LFLAGS\_SONAME = -Wl,-install\_name,@rpath/PlugIns/
  

QMAKE\_LFLAGS += -Wl,-rpath,@loader\_path,-rpath,@executable\_path
...
4. RUNPATH, DT\_RUNPATH
  
  

##### 参考文献：

http://itee.uq.edu.au/~daniel/using\_origin/
...
http://www.delorie.com/gnu/docs/binutils/ld\_3.html
