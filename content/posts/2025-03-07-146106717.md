...
安装完 TensorRT 后，在 vscode 里直接 #include <NvInfer.h> 会报错找不到，解决办法是在 vscode 设置 (.vscode/c\_cpp\_properties.json) 里添加如下 TensorRT 和 cuda 两个路径，vscode 添加 include 路径的办法参考
