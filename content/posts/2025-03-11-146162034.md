...
> 3. **使用静态成员变量\_pinst，存储单例对象的唯一实例。静态成员属于类本身，而不是类的某个对象。因此，所有对象共享同一个静态成员。在整个程序运行期间，\_pinst只会存在一份，用于指向唯一的Singleton对象。**
>
...
使用 std::unique\_lock ，此时不论是否抛异常，出来作用域，生命周期到了之后，会自动调用析构函数释放锁。
