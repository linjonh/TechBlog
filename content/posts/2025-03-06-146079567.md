...
●
**tests/**
: 包含测试脚本，如 test\_intranode.py（节点内测试）、test\_internode.py（节点间测试）和 test\_low\_latency.py（低延迟测试），以及 utils.py（工具函数）。
...
●
**csrc/**
: 包含 C++ 源代码，内有 kernels / 子目录，如 ibgda\_device.cuh，用于内核实现。
  
deep\_ep.cpp 用于管理 CUDA 内存和执行分布式通信任务：针对非对称域通信分别设计了 num\_nvl\_bytes 和 num\_rdma\_bytes。low\_latency\_mode 可配置低延迟模式。
  
ibgda\_device.cuh 实现了与 NVSHMEM（NVIDIA 共享内存库）相关的
