...
式(1)
max\_{\Phi}\sum\_{(x,y)\in Z}\sum^{|y|}\_{t=1}log(P\_{\Phi}(y\_t|x,y<t)) \qquad \text{式(1)}
...
max\_{\Theta}\sum\_{(x,y)\in Z}\sum^{|y|}\_{t=1}log(p\_{\Phi\_0+\Delta \Phi(\Theta})(y\_t|x,y<t))
