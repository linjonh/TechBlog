...
* add\_device() 通过 UserForm 实现对新设备信息的添加，并调 add\_device.html
* remove\_device(id) 通过id删除设备，但不能删除管理员添加的设备
...
| number | VARCHAR(128) |  |  |  |  |  |
| username | VARCHAR(64) |  |  |  |  |  |
| password\_hash | VARCHAR(128) |  |  |  |  |  |
| role\_id | INTEGER |  |  |  |  | roles(id) |
...
|  |  |  |  |  |
| --- | --- | --- | --- | --- |
| id | number | username | password\_hash | role\_id |
...
| 名称 | 类型 | NOT NULL约束 | PRIMARY KEY约束 | UNIQUE约束 | 默认 | 外键 |
| id | INTEGER | √ | √ |  |  |  |
| device\_id | VARCHAR(64) |  |  |  |  |  |
| lab | VARCHAR(64) |  |  |  |  |  |
| name | VARCHAR(64) |  |  |  |  |  |
| password\_hash | DATETIME |  |  |  |  |  |
| user\_id | VARCHAR(64) |  |  |  |  | users(id) |
...
|  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- |
| id | device\_id | lab | name | time | user\_id |
