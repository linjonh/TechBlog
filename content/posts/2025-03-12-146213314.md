...
> /etc/ansible/ansible.cfg文件是Ansible的全局配置文件，用于定义Ansible的默认行为和设置，可以自定义Ansible的运行方式，比如否需要输入密码、是否开启sudo认证、action\_plugins插件的位置、hosts主机组的位置、是否开启log功能、默认端口、key文件位置等。
