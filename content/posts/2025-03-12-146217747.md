 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <p>
     <img alt="" height="546" src="https://i-blog.csdnimg.cn/direct/283cc18fc432493eac6dc4b095a25a9d.png" width="601"/>
    </p>
    <p>
     一般来讲，EPT使用的是IA-32e的分页模式，即使48位物理地址，总共分为四级页表，每级页表使用9位物理地址定位，最后12位表示在一个页（4KB）内的偏移
    </p>
    <p>
     <img alt="" height="429" src="https://i-blog.csdnimg.cn/direct/a8f821217332477cacefd2b07446e35e.png" width="1280"/>
    </p>
    <p>
     启动qemu并在4444 端口开启 hmp
    </p>
    <pre><code class="language-bash">/home/xiyanxiyan10/project/qemu/build/qemu-system-x86_64 -m 10240 -enable-kvm -cpu host  -s -kernel /home/xiyanxiyan10/project/linux-source-6.2.0/arch/x86/boot/bzImage -hda ./     
    rootfs.img -nographic -append "root=/dev/sda rw console=ttyS0 nokaslr" -qmp tcp:127.0.0.1:4444,server,nowait </code></pre>
    <p>
     使用qemu提供的脚本进入交互式hmp命令行界面, 查询虚拟机内存信息
    </p>
    <pre><code class="language-cpp">xiyanxiyan10@xiyanxiyan10:~/project/qemu$ scripts/qmp/qmp-shell -H localhost:4444
...
...
    ...
    <pre><code class="language-cpp">                            struct MemoryRegion
                            +------------------------+                                         
                            |name                    |                                         
                            |  (const char *)        |                                         
                            +------------------------+                                         
    ...
    <pre><code class="language-cpp">(QEMU) info mtree
address-space: VGA
  0000000000000000-ffffffffffffffff (prio 0, i/o): bus master container
...
address-space: memory
  0000000000000000-ffffffffffffffff (prio 0, i/o): system
    ...
    <pre><code class="language-cpp">FlatView (An array of FlatRange)
+----------------------+
|nr                    |
|nr_allocated          |
...
+----------------------+         |                     |                     |
    ...
    <pre><code class="language-cpp">AddressSpace               
+-------------------------+
|name                     |
...
+-------------------------+          +----------------------+</code></pre>
    ...
    <pre><code class="language-cpp">  ram_list (RAMList)
  +------------------------------+
  |dirty_memory[]                |
  |    (unsigned long *)         |
  +------------------------------+
  ...
  +------------------------------+
   ...
   +---&gt; |next                       | -------------&gt; |next                       |
    ...
    <pre><code class="language-bash">        RAMBlock                                                     RAMBlock
         +---------------------------+                                +---------------------------+
         |next                       | -----------------------------&gt; |next                       |
         |    QLIST_ENTRY(RAMBlock)  |                                |    QLIST_ENTRY(RAMBlock)  |
         +---------------------------+                                +---------------------------+
    ...
    <pre><code class="language-bash">  MemoryListerner
  +---------------------------+
  |begin                      |
  |commit                     |
  +---------------------------+
  ...
  +---------------------------+</code></pre>
    ...
    <p>
     <img alt="" height="837" src="https://i-blog.csdnimg.cn/direct/0438d214c26e49fa93ccdca831cb6391.png" width="1198"/>
    </p>
    <h2>
     数据结构
    </h2>
...
struct MemoryRegionOps {
    ...
    <pre><code class="language-cpp">pc_memory_init()
  memory_region_allocate_system_memory()
    ...
    <pre><code class="language-cpp">RAMBlock *qemu_ram_alloc(ram_addr_t size, uint32_t ram_flags,
                           ...
                           uint32_t qemu_map_flags, off_t map_offset)
{
   // ...
    ...
    <pre><code class="language-cpp">        AddressSpaceDispatch
        +-------------------------+
        |as                       |
        |   (AddressSpace *)      |
        +-------------------------+
    ...
    <pre><code class="language-cpp">typedef PhysPageEntry Node[P_L2_SIZE];
...
struct MemoryRegionSection {
    ...
    <pre><code class="language-cpp">static void memory_region_finalize(Object *obj)
...
...</code></pre>
    ...
    <pre><code class="language-cpp">memory_region_transaction_commit(), update topology or ioeventfds
     flatviews_reset()
         flatviews_init()
     ...
     MEMORY_LISTENER_CALL_GLOBAL(commit, Forward)</code></pre>
    <ul>
     <li>
      <p>
    ...
    <pre><code class="language-cpp"> kml-&gt;listener.region_add = kvm_region_add;
 kml-&gt;listener.region_del = kvm_region_del;</code></pre>
    <p>
     以region_add为例, 调用链为以region_add为例, 调用链为
    </p>
    <pre><code class="language-bash">kvm_region_add
        kvm_set_phys_mem
            kvm_set_user_memory_region
                kvm_vm_ioctl(s, KVM_SET_USER_MEMORY_REGION, &amp;mem);kvm_region_add
        kvm_set_phys_mem
            kvm_set_user_memory_region
                kvm_vm_ioctl(s, KVM_SET_USER_MEMORY_REGION, &amp;mem);</code></pre>
    <p>
    ...
    <pre><code class="language-cpp">static int kvm_set_user_memory_region(KVMMemoryListener *kml, KVMSlot *slot, bool new)
{
   ...
   //...
    ...
    <pre><code class="language-cpp">static void init_kvm_tdp_mmu(struct kvm_vcpu *vcpu,
                 union kvm_cpu_role cpu_role)
{
    ...
    <pre><code class="language-cpp">int kvm_tdp_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
    // ...
    ...
    <pre><code class="language-cpp">static int direct_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
...
#8  0x0000555555abe1c6 in qemu_init_board () at ../system/vl.c:2614
    ...
    <pre><code class="language-cpp">Breakpoint 3, kvm_tdp_mmu_map (vcpu=vcpu@entry=0xffff88810a168000, fault=fault@entry=0xffffc90000b83bd0) at arch/x86/kvm/mmu/tdp_mmu.c:1159
1159        {
(gdb) c
Continuing.
...
#3  kvm_mmu_page_fault (vcpu=0xffff88810a168000, cr2_or_gpa=1008168, error_code=4294967300, insn=0x0 &lt;fixed_percpu_data&gt;, insn_len=0) at arch/x86/kvm/mmu/mmu.c:5592
 ...
 <p alt="68747470733a2f2f626c:6f672e6373646e2e6e65742f786979616e786979616e31302f:61727469636c652f64657461696c732f313436323137373437" class_="artid" style="display:none">
 </p>
</div>
