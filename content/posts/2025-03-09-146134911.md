...
上图可以看到，查询的走了 idx\_age这个索引，而且explain中显示 的key\_len为5

我把那个允许null，改成不允许null，再试一下

![](https://i-blog.csdnimg.cn/direct/ed15f27ef6224d1992006a6b63cd38c7.png)

看到没，改成不允许null后，key\_len变成4了

#### 2、多列联合索引长度的验证

首先，先给这个表添加一个联合索引，idx\_name\_age 联合的是name和age这2列
...
所以，咱们的预期是，最后idx\_name\_age 这个联合索引的长度是两者相加，402 + 4 = 406

好，下面 使用 explain 查询验证一下

![](https://i-blog.csdnimg.cn/direct/b3b38e0b7f7349a490b9ccd22b5def8b.png)

可以看到结果跟咱们预期的一样：走了 idx\_name\_age 这个联合索引，并且idx\_name\_age联合索引的长度是406，和咱们预期的结果一样。
