...
[C#调用Python脚本并使用Python的第三方模块\_dfskchfk72506的博客-CSDN博客](https://blog.csdn.net/dfskchfk72506/article/details/101499871 "C#调用Python脚本并使用Python的第三方模块_dfskchfk72506的博客-CSDN博客")

但是连接中使用的egg文件在相应位置是找不到的（如果你使用了pip或者conda安装的第三方库），因此你需要卸载该三方库并通过源码安装，

可以参考：
[python安装第三方库时生成.egg文件\_曲終~的博客-CSDN博客](https://blog.csdn.net/qq_40569107/article/details/89815173 "python安装第三方库时生成.egg文件_曲終~的博客-CSDN博客")
...
1.5 重启电脑之后重新进行1.3步骤，会在py文件的同目录下的build\lib.win-amd64-3.9文件夹中得到对应的pyd文件test.cp39-win\_amd64.pyd。

1.6 将test.cp39-win\_amd64.pyd重命名与py文件相同test.pyd
...
还有另一种解决方案，
[pyinstaller打包遇到的一些坑\_HQ1356466973的博客-CSDN博客](https://blog.csdn.net/HQ1356466973/article/details/91038504 "pyinstaller打包遇到的一些坑_HQ1356466973的博客-CSDN博客")
...
引号在json中不能传递，转义也不行，现在的解决方法是在传递之前replace替换成指定字符串（我用的是 shuangyinhao\_Archer），传递完成后在替换回来，有更好的方法希望不吝赐教。
...
参考：
[手把手教你将Python程序打包为DLL\_zmr1994的博客-CSDN博客\_python生成dll](https://blog.csdn.net/zmr1994/article/details/90703017 "手把手教你将Python程序打包为DLL_zmr1994的博客-CSDN博客_python生成dll")