...
同理第1位即    DS1302\_IO=Command&0x02;

第2位即    DS1302\_IO=Command&0x04; .......
...
这里提醒一下，DS1302初始化有写保护，只能读不能写，在DS1302初始化之后加一句DS1302\_WriteByte(0x8e,0x00）；0x8E为写保护寄存器，需要先关闭写保护，
...
所以我们在这里想让Second自加显示在LCD上，就得使用下面这个函数，而不是ShowNum，否则他会从9突变到16，这是因为ShowNum是以十进制来显示，而时钟寄存器的自加是上面所说的BCD码，而下面这个函数是以十六进制显示的            LCD\_ShowHexNum(2,1,Second,3);  十六进制和BCD码有部分兼容
...
LCD\_ShowNum(2,1,Second/16\*10+Second%16,3);
