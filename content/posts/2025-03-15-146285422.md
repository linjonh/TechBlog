...
* 可以用于任意关系间通信。
* 管道中的数据一次性读取。
* 读端：以O\_RDONLY 打开 fifo 通道。
* 写端：以 O\_WRONLY/RDWR 打开同一个管道。
...
2. 创建映射区，需要 read 权限。指定访问权限为 MAP\_SHARED，mmap需要读写权限。应该 <= 文件权限。
...
1. fd = open(“filename”, O\_RDWR)
2. mmap(NULL, size of valid file, PROT\_READ|PROT\_WRITE, MAP\_SHARED, fd, 0)

#### 进程间通信

##### 父子间通信

1. 父进程先创建映射区。open(O\_RDWR); mmap(MAP\_SHARED);
2. fork() 创建子进程。
3. 一个进程使用映射区读，另一个进程写。
...
* MAP\_ANON 只在 Linux 中有效，系统中没有该选项，可以借助 /dev/zero 实现。