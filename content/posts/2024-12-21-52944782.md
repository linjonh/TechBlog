---
arturl_encode: "68747470733a2f2f:626c6f672e6373646e2e6e65742f6875696c616e5f73616d65:2f61727469636c652f64657461696c732f3532393434373832"
layout: post
title: "Python必会的单元测试框架-unittest"
date: 2024-12-21 08:40:22 +08:00
...
[原文链接](http://www.cnblogs.com/hackerain/p/3682019.html)
）：

![unittest类图](https://img-blog.csdn.net/20161026164739484)

> * 一个TestCase的实例就是一个测试用例。什么是测试用例呢？就是一个完整的测试流程，包括测试前准备环境的搭建(setUp)，执行测试代码(run)，以及测试后环境的还原(tearDown)。元测试(unit test)的本质也就在这里，一个测试用例是一个完整的测试单元，通过运行这个测试单元，可以对某一个问题进行验证。
> * 而多个测试用例集合在一起，就是TestSuite，而且TestSuite也可以嵌套TestSuite。
> * TestLoader是用来加载TestCase到TestSuite中的，其中有几个loadTestsFrom\_\_()方法，就是从各个地方寻找TestCase，创建它们的实例，然后add到TestSuite中，再返回一个TestSuite实例。
...
`test`
开头的方法，那么每有一个这样的方法，在load的时候便会生成一个TestCase实例，如：一个class中有四个test\_xxx方法，最后在load到suite中时也有四个测试用例。
...
1. 在第一行给出了每一个用例执行的结果的标识，成功是
   ...
   ，跳过是
   `S`
   。从上面也可以看出，测试的执行跟方法的顺序没有关系，test\_divide写在了第4个，但是却是第2个执行的。
2. 每个测试方法均以
   `test`
   开头，否则是不被unittest识别的。
3. 在unittest.main()中加
...
>
> 灰蓝修改版：
> **[HTMLTestRunner.py(已调整格式，中文显示）](http://download.csdn.net/detail/huilan_same/9598558)**
...
![html report](https://img-blog.csdn.net/20161027113650705)
...
3. 一个class继承unittest.TestCase即是一个TestCase，其中以
   `test`
...
4. verbosity参数可以控制执行结果的输出，
   ...
   `2`
   是详细报告。
5. 可以通过addTest和addTests向suite中添加case或suite，可以用TestLoader的loadTestsFrom\_\_()方法。
6. 用
   `setUp()`
   、
   `tearDown()`
...
[![Python Selenium交流群](https://img-blog.csdn.net/20161020130233897 "Python Selenium交流群")](http://shang.qq.com/wpa/qunwpa?idkey=90801105d449b59723f93b7c51173840de66567e43dcd78fc58f170698636538)
