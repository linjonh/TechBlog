...
基于Web的断言——这意味着Playwright有一个内置的expect语句，例如可以用来断言页面上的元素，如expect(page.get\_by\_role(“heading”, name=”Installation”)).to\_be\_visible()。
...
然后我们使用内置的Web断言expect(page).to\_have\_title(re.compile("Playwright"))来检查我们访问的页面的标题是否包含“Playwright”或其正则表达式。

在第二个测试中：

我们访问同一个页面。

我们使用get\_by\_role定位器来定位名为“Get started”的链接并点击它。
...
让我们看另一个示例，以了解各种定位器。 tests/test\_example.py
...
让我们看另一个稍微复杂一点的示例，在这个示例中你可以登录到一个页面。 tests/test\_example.py
...
在这个示例中，我们使用get\_by\_label定位器获取用户名和密码字段，并使用文本填充它们。

然后我们使用get\_by\_role定位器获取按钮并点击它。
...
在你的测试中添加以下内容： tests/test\_example.py
...
让我们调整我们的DuckDuckGo搜索测试以录制视频。 tests/test\_example.py
