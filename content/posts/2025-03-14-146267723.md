...
1. raw\_response\_event：原始响应事件
2. agent\_updated\_stream\_event：代理更新事件
3. run\_item\_stream\_event：运行项目事件
...
由于智谱AI的API返回的用量统计格式与OpenAI的格式不同导致的。错误显示 CompletionUsage 对象没有 input\_tokens 属性。
