...
1. **基本配置指令**
   ...
   * **`server`
   ...
   server\_name download.example.com; # 服务器域名，替换为实际的域名或使用 localhost 进行本地测试
...
1. **下载相关指令**
   ...
   * **`add_header`
     ...
     + 可以使用
       ...
       add\_header Content-Disposition ‘attachment; filename=“$uri”’; # 设置下载文件名与请求的 URI 一致
...
* 在该目录下放置一些测试文件，如
  ...
  echo “This is test file 1” > /data/downloads/test\_file1.txt
    
  zip -r /data/downloads/test\_file2.zip /data/downloads/test\_file1.txt # 创建一个压缩文件
...
* 添加以下配置内容：
  ...
  server\_name localhost; # 本地测试使用 localhost，实际应用中替换为真实域名
...
* **使用命令行工具测试下载（可选）**
  ...
  curl -O http://localhost/downloads/test\_file1.txt
* 执行后，
  `test_file1.txt`
