...
layout: post
title: "多线程-参数传递之间的关系"
date: 2025-03-10 14:38:45 +0800
description: "在C++中创建线程时，传递参数的方式会影响参数的生命周期、线程的安全性和性能。"
keywords: "多线程--参数传递之间的关系"
categories: ['C']
tags: ['算法', '开发语言', 'C']
artid: "146154301"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=146154301
    alt: "多线程-参数传递之间的关系"
render_with_liquid: false
featuredImage: https://bing.ee123.net/img/rand?artid=146154301
featuredImagePreview: https://bing.ee123.net/img/rand?artid=146154301
cover: https://bing.ee123.net/img/rand?artid=146154301
image: https://bing.ee123.net/img/rand?artid=146154301
img: https://bing.ee123.net/img/rand?artid=146154301
---
 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="markdown_views prism-atom-one-dark" id="content_views">
    ...
    <p>
     在C++中创建线程时，传递参数的方式会影响参数的生命周期、线程的安全性和性能。以下是几种常见的传递方式及其适用情况：
    </p>
    <h4>
     <a id="1__2">
     </a>
    ...
    <p>
     值传递会创建参数的副本，并在线程函数内部使用该副本。这种方式可以避免线程之间的竞态条件，因为每个线程都有自己独立的参数副本。
    </p>
    <p>
     <strong>
      适用情况
    ...
    <ul>
     <li>
      <strong>
       参数是简单的基本类型（如整数、浮点数等）或小型结构体。
      </strong>
     </li>
     <li>
      <strong>
       你希望每个线程都有参数的独立副本，不会影响其他线程。
      </strong>
     </li>
     <li>
      <strong>
...
int main() {
    ...
    <p>
     引用传递允许线程直接访问原始参数，可以避免拷贝大对象的开销。但这也意味着如果主线程修改了参数，子线程可能会遇到未定义行为。
    </p>
    <p>
     <strong>
      适用情况
    ...
    <ul>
     ...
     <li>
      <strong>
       确保参数在整个线程执行期间保持有效（主线程不会销毁或修改该参数）。
      </strong>
     </li>
     <li>
      <strong>
       需要在多个线程间共享数据，并通过同步机制（如互斥锁）来保护数据
      </strong>
      。
     </li>
    </ul>
...
int main() {
    ...
    <p>
     指针传递和引用传递类似，也允许线程直接访问原始参数。但需要注意指针的生命周期管理，避免悬空指针。
    </p>
    <p>
     <strong>
      适用情况
    ...
    <ul>
     ...
     <li>
      <strong>
       需要传递数组等数据结构，使用指针进行操作
      </strong>
      。
     </li>
     <li>
      确保指针在整个线程执行期间保持有效，避免悬空指针。
     </li>
    </ul>
    <pre><code>#include &lt;iostream&gt;
#include &lt;thread&gt;
...
int main() {
    ...
    <ul>
     ...
     <li>
      ...
      ：适用于动态分配的对象和数组，需注意指针的生命周期管理。
     </li>
    </ul>
    <p>
     选择合适的传递方式取决于具体的使用场景和性能需求。
    </p>
    <pre><code>std::thread是不可复制的，但可以移动。push_back使用了移动语义来避免拷贝临时对象。
</code></pre>
...
//    cout &lt;&lt; result.get() &lt;&lt; endl;  // 不可以连续两次get()
 ...
 <p alt="68747470733a2f2f62:6c6f672e6373646e2e6e65742f6d305f37323136393535372f:61727469636c652f64657461696c732f313436313534333031" class_="artid" style="display:none">
 </p>
</div>
