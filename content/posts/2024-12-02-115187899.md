...
1.检查JAVA\_HOME的配置是否有错，这是新建的，变量值为jdk的安装路径，仔细检查
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/e3e67466e96788c2d80ff0e8ae340413.png)
  
2.检查ClassPath的配置是否有错，这也是新建的，但变是值为【.;%JAVA\_HOME%\lib\dt.jar;%JAVA\_HOME%\lib\tools.jar】，注意前面的符号
  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/b161888656275d631a22ba532de0f30d.png)
  
3.配置时都会是复制这段【%JAVA\_HOME%\bin;%JAVA\_HOME%\jre\bin;】win10的都分开显示，还是比较直观，但由于编辑器的原因，所有变量连成一串，并且自动在配置的变量值前后加了引号和一个分号，引起路径失效
