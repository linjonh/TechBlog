...
affinity:
podAntiAffinity:
preferredDuringSchedulingIgnoredDuringExecution: - weight: <span class="token number">100</span>
podAffinityTerm:
labelSelector:
...
app: gatekeeper
spec:
ports: - name: <span class="token string">"http-8000"</span>
protocol: TCP
port: <span class="token number">8000</span>
targetPort: <span class="token number">8000</span> - name: <span class="token string">"http-8080"</span>
protocol: TCP
...
name: gatekeeper
port:
number: <span class="token number">8000</span> - host: gatekeeper.diyu.10.86.68.66.nip.io
http:
paths: - path: /
pathType: ImplementationSpecific
backend:
...
<ol>
<li>
处于安全考虑：我们的源码无法在本地存放，所以基于 gitlab 构建打包的方案，对我们用处不是很大，使用是一种资源浪费。
</li>
<li>
功能简洁性：虽然 Syncd 已经停更 2 年多但是，但其核心的 CICD 功能比较完善且前后端拓展性强，我们可以很轻松拓展相应的功能。
</li>
</ol>
<p>
Syncd 核心思路：
...
container*name=<span class="token variable">${project}</span>"*"<span class="token variable">${module}</span>
#镜像名称
image_name=<span class="token variable">${project}</span>"/"<span class="token variable">${module}</span> #服务端口映射：宿主机端口:容器端口，多个逗号间隔
...
<span class="token comment">#docker_build 举例：</span>
...
<span class="token comment">#docker_run 举例:</span>
...
<span class="token comment">#docker_push 举例：</span>
...
<span class="token comment">#最后登陆 Harbor 测试镜像是否上传</span>
...
<li>
支持 helm chart 一键完成应用及服务部署，私有化场景中我们只需要关心容器融合本地化即可。
</li>
</ol>
...
https://github.com/kubesphere/kubekey/releases/
 ...
 <p alt="68747470733a2f2f:626c6f672e6373646e2e6e65742f653432313038333435382f:61727469636c652f64657461696c732f313235373032333437" class_="artid" style="display:none">
 </p>
</div>
