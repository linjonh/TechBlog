...
ALTER TABLE player RENAME COLUMN NAME to nick\_name;
修改字段名

ALTER TABLE player add COLUMN last\_login datetime;
添加字段名

ALTER TABLE player DROP COLUMN last\_login;
删除字段
...
CREATE TABLE new\_player SELECT \* FROM player WHERE LEVEL<5

#### 使用子查询插入数据

insert into

INSERT INTO new\_player SELECT \* FROM player WHERE LEVEL BETWEEN 6 AND 10;
...
ON table\_name(index\_col\_name,……)

#### 创建索引

CREATE INDEX email\_index ON FAST(email);

alter table fast add index name\_index(name);
...
DROP INDEX email\_index ON fast;
