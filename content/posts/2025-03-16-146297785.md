...
若张量带有梯度（requires\_grad=True），需先分离计算图：
...
| 方法 | 内存共享 | 性能 | 适用场景 |
| --- | --- | --- | --- |
| torch.from\_numpy() | 是 | 高 | 大数据处理，需减少内存复制时使用 |
...
torch.from\_numpy() 会保留 NumPy 数组的数据类型。
