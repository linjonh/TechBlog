...
[B - Ticket Gate Log](https://atcoder.jp/contests/abc397/tasks/abc397_b "B -
Ticket Gate Log")
...
[C - Variety Split Easy](https://atcoder.jp/contests/abc397/tasks/abc397_c "C
- Variety Split Easy ")
...
> 中不同整数的数量，和![\(A_{i+1},A_{i+2},...,A_N\)](https://latex.csdn.net/eq?%28A_%7Bi&plus;1%7D%2CA_%7Bi&plus;2%7D%2C...%2CA_N%29)中不同整数的数量。
...
## [D - Cubes](https://atcoder.jp/contests/abc397/tasks/abc397_d "D - Cubes")
...
> ![N=x^3-y^3 \\leq
> \(y+1\)^3-y^3=3y^2+3y+1](https://latex.csdn.net/eq?N%3Dx%5E3-y%5E3%20%5Cleq%20%28y&plus;1%29%5E3-y%5E3%3D3y%5E2&plus;3y&plus;1)如果y存在,可得y至少都为![y<\\sqrt\\frac{N}{3}](https://latex.csdn.net/eq?y%3C%5Csqrt%5Cfrac%7BN%7D%7B3%7D)。而直接遍历明显不行。
>
> 令d=x-y， 由![N=x^3-y^3=\(x-y\)\(x^2+xy+y^2\)\\geq
> \(x-y\)\(x-y\)^2=d^3](https://latex.csdn.net/eq?N%3Dx%5E3-y%5E3%3D%28x-y%29%28x%5E2&plus;xy&plus;y%5E2%29%5Cgeq%20%28x-y%29%28x-y%29%5E2%3Dd%5E3)可得![d\\leq
> \\sqrt\[3\]N](https://latex.csdn.net/eq?d%5Cleq%20%5Csqrt%5B3%5DN)。那么如果(x,y)存在，则满足![\(d+y\)^3-y^3=N=>d^3+3d^2y+3dy^2=N=>d^2+3dy+3y^2=\\frac{N}{d}](https://latex.csdn.net/eq?%28d&plus;y%29%5E3-y%5E3%3DN%3D%3Ed%5E3&plus;3d%5E2y&plus;3dy%5E2%3DN%3D%3Ed%5E2&plus;3dy&plus;3y%5E2%3D%5Cfrac%7BN%7D%7Bd%7D)。（注意求幂使用pow返回的是浮点型存在精度问题）。且在d确定的情况下上式单调递增，可用二分判断在d确定下y是否存在。
>
>
> 结论：先遍历d区间![\[1,\\sqrt\\frac{N}{3}\)](https://latex.csdn.net/eq?%5B1%2C%5Csqrt%5Cfrac%7BN%7D%7B3%7D%29)，在内部二分搜索y是否有y满足![d^2+3dy+3y^2=\\frac{N}{d}](https://latex.csdn.net/eq?d%5E2&plus;3dy&plus;3y%5E2%3D%5Cfrac%7BN%7D%7Bd%7D)。即可。时间复杂度为![O\(\\sqrt\[3\]NlogN\)](https://latex.csdn.net/eq?O%28%5Csqrt%5B3%5DNlogN%29)。注意在此题中要注意整型越界问题。（纯纯数学题）
...
## [E - Path Decomposition of a
Tree](https://atcoder.jp/contests/abc397/tasks/abc397_e "E - Path
...
>   *
> 对于每个i=1,2,...,N和j=1,2,...,K-1它们间有边连接着点![P_{i,j},P_{i,j+1}](https://latex.csdn.net/eq?P_%7Bi%2Cj%7D%2CP_%7Bi%2Cj&plus;1%7D)。
>
