...
1. 创建文件描述符集合 ： epoll\_create();
  
2. 将关注的文件描述符加入到集合:epoll\_ctl()
  
3. 等待IO事件到达 : epoll\_wait();
...
EPOLL\_CTL\_ADD：向集合中添加
  
EPOLL\_CTL\_MOD：修改集合
  
EPOLL\_CTL\_DEL：从集合中删除
...
uint32\_t     events;      /* Epoll events */  //事件类型
...
3.  int epoll\_wait(int epfd, struct epoll\_event *events,
...
events：保存epoll\_wait监测到的到达事件的结果
