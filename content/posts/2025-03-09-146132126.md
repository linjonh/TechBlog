...
2
\begin{aligned} spiral= & radius\times\left(\sin\left(2\times\pi\times rand\right)+\cos\left(2\times\pi\times rand\right)\right) \\ \\ & radius=\sqrt{\sum\_{j=1}^{Dim}\left(z\_{best,j}\left(t\right)-z\_{i,j}\left(t\right)\right)^{2}} \end{aligned}
...
\begin{aligned} \vec{Z}\_i\left(t+1\right) & =\vec{Z}\_{best}\left(t\right)+F\times G\times Levy\left(Dim\right)\times\left(\vec{Z}\_{best}\left(t\right)-\vec{Z}\_i\left(t\right)\right) \\ G & =2\times\left(1-\frac{t}{T\_{\mathrm{max}}}\right) \end{aligned}
