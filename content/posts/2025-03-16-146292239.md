...
QApplication::style()->drawControl(QStyle::CE\_ProgressBar, &progressBar, painter);
}
};
// 日期格式化委托
...
代理模型定义如下：
proxymodel.h
#ifndef ProxyModel\_H
#define ProxyModel\_H
#include 
#include 
...
private:
QList m\_visibleColumns; //可见列
QMap m\_columnHeaders; // 源列号到标题的映射
int m\_progress\_limit; //数据筛选："进度"栏
bool m\_status; //数据筛选："状态"栏
};
#endif // ProxyModel\_H
proxymodel.cpp
#include "proxymodel.h"
...
{
return m\_visibleColumns.contains(sourceColumn);
}
// 标题数据重写
QVariant ProxyModel::headerData(int section, Qt::Orientation orientation, int role) const
{
if (orientation == Qt::Horizontal && role == Qt::DisplayRole) {
if (section >= 0 && section < m\_visibleColumns.size()) {
int srcCol = m\_visibleColumns.at(section); // 获取对应的源列号
// 优先返回自定义标题
if (m\_columnHeaders.contains(srcCol)) {
return m\_columnHeaders[srcCol];
}
// 回退到源模型标题
...
{
return (progress<m\_progress\_limit);
}