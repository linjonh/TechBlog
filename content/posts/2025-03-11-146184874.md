...
Model用于从数据库读取数据，并且会根据视图需要进行相应的处理，odoo对常用数据通过@web/model/relational\_model/relational\_model中进行处理，如有特殊需求，可以自行构建Model。
...
由于Renderer是根据template\_id生成，因此这里不需要实现Renderer
...
在useEffect中用传过来的template\_xmlid，model中的解析数据生成template页面，并且替换o\_content。
