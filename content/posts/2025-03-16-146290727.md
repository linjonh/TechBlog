...
// 1. 保证队列安全
// 2. 队列中不一定有数据
while (\_task\_queue.empty() && \_isrunning)
{
\_waitnum++;
\_cond.Wait(\_mutex);
\_waitnum--;
...
{
\_threads.emplace\_back(std::bind(&ThreadPool::HandlerTask, this));
...
int main()
{
ENABLE\_CONSOLE\_LOG\_STRATEGY();
std::unique\_ptr> tp = std::make\_unique>();
tp->Start();
int cnt = 10;
char c;
while (cnt)
