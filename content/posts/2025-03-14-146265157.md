...
bh1750 = adafruit\_bh1750.BH1750(i2c)
...
client.publish("farm/sensor/data", json.dumps(sensor\_data))
...
client.switch\_database('agriculture')
...
client.write\_points(json\_body)
...
data = conn.execute('SELECT \* FROM sensor\_data').fetchall()

return render\_template('dashboard.html', data=data)
...
def check\_threshold(sensor\_value):

if sensor\_value['temp'] > 35:
...
body=f"当前温度：{sensor\_value['temp']}℃"
...
if soil\_moisture < 30:

GPIO.output(WATER\_PUMP\_PIN, GPIO.HIGH)

time.sleep(10) # 浇水10秒

GPIO.output(WATER\_PUMP\_PIN, GPIO.LOW)
