...
> 这段话的意思翻译过来大概就是：
> **rand()生成随机数的范围是 0~RAND\_MAX 之间的数字**
> ，而 RAND\_MAX 大小是 0x7fff ,这是十六进制表示形式，转换成十进制就是 32767，所以 rand()生成随机数的范围就是 0~32767。

那么  int random\_num = rand() % 100 + 1; 又是什么意思呢？
...
因为 time()函数的返回值类型是 time\_t 类型，但是 srand()函数参数类型是 unsigned int 型，所以要进行强制类型转换
