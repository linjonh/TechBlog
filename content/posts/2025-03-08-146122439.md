...
> 题目地址：[最小路径和](https://leetcode.cn/problems/minimum-path-sum/)
...
t m p [ i ] [ j ] = g r i d [ i ] [ j ] tmp[i][j] = grid[i][j]
tmp[i][j]=grid[i][j]

    * 当 `i == 0` 时，只能从左边到达，到达当前位置的路径最小和满足如下公式：  
t m p [ i ] [ j ] = g r i d [ i ] [ j ] \+ t m p [ i ] [ j − 1 ] tmp[i][j] =
grid[i][j] + tmp[i][j-1] tmp[i][j]=grid[i][j]+tmp[i][j−1]

    * 当 `j == 0` 时， 只能从上面到达，到达当前位置的路径最小和满足如下公式：  
t m p [ i ] [ j ] = g r i d [ i ] [ j ] \+ t m p [ i − 1 ] [ j ] tmp[i][j] =
grid[i][j] + tmp[i-1][j] tmp[i][j]=grid[i][j]+tmp[i−1][j]

    * 当 `i != 0` 并且 `j != 0` 时，到达当前位置的路径最小和满足如下公式：  
t m p [ i ] [ j ] = g r i d [ i ] [ j ] \+ m i n ( t m p [ i − 1 ] [ j ] , t m
p [ i ] [ j − 1 ] ) tmp[i][j] = grid[i][j] + min(tmp[i-1][j], tmp[i][j-1])
