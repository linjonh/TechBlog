...
* dump-file "/var/named/data/cache\_dump.db";: 指定缓存转储文件的路径。缓存转储文件用于保存 DNS 缓存的内容。
* statistics-file "/var/named/data/named\_stats.txt";: 指定统计信息文件的路径。该文件记录了 BIND 服务器的运行统计信息，如查询数量、响应时间等。
* memstatistics-file "/var/named/data/named\_mem\_stats.txt";: 指定内存统计信息文件的路径。该文件记录了 BIND 服务器使用的内存统计信息。
