...
> #!/bin/bash
>   
> file\_a="/data/jqtest/a.txt"
>   
> file\_json="/data/jqtest/ryxx.json"
>   
> mapfile -t logins < <(tr ',' '\n' < "$file\_a")
>
> for dlm in "${logins[@]}"; do
>   
> dlm\_display="${dlm:-null}"  # 空值替换为 null
>   
> name=$( jq -r --arg uid "$dlm"  ' ([.[] | select(.loginId == $uid) | .name] | first) // "null" ' "$file\_json" )
>   
> echo "$dlm\_display $name"
>   
> done
...
>
> jq -r --arg uid "$dlm"  ' ([.[] | select(.loginId == $uid) | .name] | first) // "null" ' "$file\_json"
>
