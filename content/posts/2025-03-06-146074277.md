...
##### 报错处理一：找不到指定的模块mkl\_intel\_thread.2.dll
...
解决：建议直接下载numpy-1.24.5+mkl-cp39-cp39-win\_amd64.whl下载地址：
...
模型位置： 命令执行路径/pretrained\_models/2stems
...
通过环境变量MODEL\_PATH指定模型文件所在位置，2stems、4stems、5stems这些模型文件夹都是MODEL\_PATH的子目录
