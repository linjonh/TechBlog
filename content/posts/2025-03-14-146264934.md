...
read 函数也接受可选参数 first\_cell 和 last\_cell。
...
中的第一行第一列，你应该使用元组 `(1, 1)`，而不是 `(0, 0)`。`read(sheet, first\_cell=(1, 1),
last\_cell=(3, 3))` 将读取从第一行第一列到第三行第三列的单元格区域。
first\_cell 的默认值是 A1，而 last\_cell 的默认值是所用区域的右下角。因此如果你只提供了 sheet
对象作为参数，那么它就会读取整张工作表。
与 read 函数的工作方式类似，write 函数接受 xlwt、 OpenPyXL 或 XlsxWriter 的 sheet 对象（read 和
write 函数都是自定义函数），以及以嵌套列表和可选的 first\_cell 表示的值。可选参数 first\_cell
