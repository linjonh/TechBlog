...
if (page == null || page.getSize() < 0 || !page.searchCount() || resultHandler != Executor.NO\_RESULT\_HANDLER) {
return true;
}
BoundSql countSql;
...
if (subSelect.toString().contains(StringPool.QUESTION\_MARK)) {
canRemoveJoin = false;
break;
}
...
if (expression.toString().contains(StringPool.QUESTION\_MARK)) {
...
// 优化 SQL
plainSelect.setSelectItems(COUNT\_SELECT\_ITEM);
...
Plus默认的查询总行数的方法，如果指定CountId，会避免进行总行数SQL优化如（"com.zguiz.musicplayer.mapper.ClassifyMapper.testMybatisPlus\_mpCount")。