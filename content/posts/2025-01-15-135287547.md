...
# 计算机网络【DHCP动态主机配置协议】
![图片](https://i-blog.csdnimg.cn/blog\_migrate/cd4e3fb085ef89c94972de47fd090e87.png)
#### DHCP 出现
电脑或手机需要
\*\*IP 地址\*\*
...
\*\*太麻烦\*\*
了。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/9821c545329ea0e3b4da9a54a2fd43b5.png)
于是就有了
\*\*DHCP\*\*
协议，会
\*\*自动配置\*\*
设备的网络参数，包括 IP 地址、子网掩码、网关地址、DNS 服务器等，替代手动配置。还能统一 IP 地址分配，方便网络管理。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/645858b1ea9fecb8582d501c76135d8a.png)
#### DHCP 简介
\*\*DHCP\*\*
协议是从
\*\*BOOTP\*\*
...
\*\*所有配置信息\*\*
。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/619f46e0c422f57297415126284df903.png)
...
\*\*应用程序\*\*
，可以运行在电脑、服务器、路由器等设备上。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/c0ee2f474264834943f888164ebb74e5.png)
\*\*举个栗子\*\*
：
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/e6071eca67820887e524056a489d1fe8.png)
#### DHCP 分配机制
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/477d4f54ea0fee16e53637b8345f7d79.png)
...
\*\*报错\*\*
处理。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/94503e7752044e2f18706fae12d29290.png)
#### DHCP 系统组成
DHCP 系统由
\*\*DHCP Server\*\*
（ DHCP 服务器）、
...
\*\*DHCP Relay\*\*
（ DHCP 中继）等组成。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/b757763233ec56d8111cfaa5110f3054.png)
\* \*\*DHCP Server\*\*
DHCP Server 提供
\*\*网络参数\*\*
...
\*\*无线路由器\*\*
。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/520272afcbd6eb6e92cd9691e46062f9.png)
\* \*\*DHCP Client\*\*
...
\*\*手机\*\*
。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/6492a92937cef20e4772f8f5ef78d636.png)
\* \*\*DHCP Relay\*\*
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/ef411141762568008f1aabf7ed103a30.png)
为方便描述，
\*\*DHCP Server\*\*
简称
\*\*小 S\*\*
...
\*\*小 S\*\*
。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/36ba0baad0f391b2758c5efcb65be79c.png)
##### 2、提供阶段
本地网段的
\*\*小 S\*\*
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/2725651576b865ca5f16f70b7672de20.png)
如果 小 C 在一定时间后
\*\*一直没收到\*\*
...
\*\*DHCP Nak\*\*
报文。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/837689c35135ed989ea60dc2eacb4c7b.png)
...
\*\*重新来申请\*\*
一个 IP 地址。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/f1cf466ca6166bcf9098faf07ba3d9b5.png)
#### DHCP Relay
动态获取 IP 地址的过程中，使用
\*\*广播\*\*
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/02c46e83b73666f5508c36053891954c.png)
DHCP Relay 的
\*\*工作原理\*\*
如下：
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/c45c3881bbdf9689f3a65806e7b4bcfb.png)
Client 从
\*\*INIT\*\*
...
##### DHCP Server 示例
###### 网络拓扑
![图片](https://i-blog.csdnimg.cn/blog\_migrate/9a80f260f06dc601ac0f73d53474c088.png)
###### 实验要求
\* RT（路由器）配置
\*\*DHCP Server\*\*
，PC
...
\*\*dhcp select global\*\*
。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/c22e264c6f48dfd8dc923c72867425a6.png)
###### 功能验证
...
\*\*PC\*\*
上查看动态获取地址情况，进行双向验证。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/d2527925f03fdea4caa828a2a21d401a.png)
抓包还可以看到 DHCP
\*\*报文交互\*\*
的详细过程，同时也是
\*\*检验理论知识\*\*
是否正确。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/2a5557c57ba2dff510a6be4f07610155.png)
...
##### DHCP Relay 示例
###### 网络拓扑
![图片](https://i-blog.csdnimg.cn/blog\_migrate/95ed92d720fb7eb6a2984cc287440369.png)
###### 实验要求
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/63ea13c0926f5b50cea84267fdb80911.png)
1. \*\*RT2\*\*
（DHCP Server）开启 DHCP 功能，创建地址池并配置相关参数，在接口下引用地址池，实现 DHCP Server 功能。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/492544e45eb0f420e41b11e96c3f13db.png)
###### 功能验证
...
![图片](https://i-blog.csdnimg.cn/blog\_migrate/eaaa1e7a8d02cd18d6df22f199354f87.png)
在 RT1 的 G0/0/0 抓包，查看
\*\*DHCP Server\*\*
和 DHCP Relay 的报文交互过程。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/2f6fe0c340c26059f2c5c07d2fc7a7a0.png)
#### DHCP 报文格式
如果想要更深入的了解 DHCP 协议，那就要看它的报文格式。DHCP 设备通过识别报文内容，实现协议功能。
![图片](https://i-blog.csdnimg.cn/blog\_migrate/06ea404b7b12c562ed00cf2eb0390073.png)
\* \*\*op\*\*
...
。
DHCP Discover 报文
![图片](https://i-blog.csdnimg.cn/blog\_migrate/593780460dc851d9a2c0130f4eca923c.png)
DHCP Offer 报文
![图片](https://i-blog.csdnimg.cn/blog\_migrate/82676983ef8e13fee7957df75e10dfa7.png)
DHCP Request 报文
...
DHCP Ack 报文
![图片](https://i-blog.csdnimg.cn/blog\_migrate/48de55d10d75f59bc0cfca675f649111.png)