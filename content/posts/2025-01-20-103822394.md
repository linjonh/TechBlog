...
| help all | 基本命令帮助说明 |
| exit | 退出控制台 |
| Run | run cts --plan test\_plan\_name | 运行指定计划 |
| run cts --package/-p | 运行指定包 |
| run cts --class/-c [--method/-m] | 运行指定方法 |
| run cts --continue-session session\_ID | 继续运行指定会话session\_id |
| run cts [options] --serial/-s device\_ID | 运行指定设备 |
| run cts [options] --shards number\_of\_shards | 运行多个设备 |
