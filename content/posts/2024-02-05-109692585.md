...
因此, 在 nginx 配置的地方, 增加了一个 add\_header backendIP $upstream\_addr; 来获取实际处理 后端请求的服务的信息
...
两个服务使用 同一个镜像, 只是传递的环境变量的 APP\_NAME 有一些区别, 因此 访问 /hello/context 的时候 appName 的输出会有一些区别
...
可以看到 /hello 开头的这部分请求, 我们把它 转发到了 app0 的服务上面, 使用 add\_header backendIP $upstream\_addr; 输出了一下 具体的处理业务的后端服务器的信息
