...
input_tensor.scalar()() = prompt;
return input_tensor;
}
// 运行模型进行推理
Status RunModel(const std::unique_ptr& session, const Tensor& input_tensor, std::vector& outputs) {
std::vector> inputs = {{"input_prompt", input_tensor}};
...
std::string video_data = output_tensor.scalar()();
// 初始化FFmpeg库
av_register_all();
...
input_tensor_mapped(0, y, x, c) = image.at(y, x)[c];
}
}
}
return input_tensor;
...
// 运行模型进行推理
Status RunModel(const std::unique_ptr& session, const Tensor& input_tensor, std::vector& outputs) {
std::vector> inputs = {{"input", input_tensor}};
Status status = session->Run(inputs, {"output_video"}, {}, &outputs);
if (!status.ok()) {
std::cerr << "Failed to run model: " << status.ToString() << std::endl;
}
...
#### 处理模型输出并保存为视频
// 处理模型输出并保存为视频
void ProcessOutput(const std::vector& outputs, const std::string& output_path) {
if (outputs.empty()) {
std::cerr << "No output from model." << std::endl;
return;
}
...
auto frame_tensor_mapped = frame_tensor.tensor();
// 将帧数据复制到 OpenCV Mat
Mat frame_mat(height, width, CV_32FC3);
for (int y = 0; y < height; ++y) {
for (int x = 0; x < width; ++x) {
for (int c = 0; c < channels; ++c) {
frame_mat.at(y, x)[c] = frame_tensor_mapped(0, y, x, c);
}
}
}
frame_mat.convertTo(frame_mat, CV_8UC3, 255.0);
// 将 RGB 帧转换为 YUV 帧
const int stride[] = {static_cast(frame_mat.step)};
