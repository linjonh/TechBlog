...
1. 进入 Pydev 透视图，在 Python Package Explorer 中，右键单击 src，选择 New->Pydev Package，输入 Package 名称并单击 Finish，Python 包就创建好了，此时，自动生成\_\_init\_\_.py 文件，该文件不包含任何内容。
...
|  |
| --- |
| def **\_\_init\_\_** ( *self* , dbUser, dbPswd, dbHost): |

按 Enter 应用该功能后，自动为该函数添加了参数注解。

|  |
| --- |
| def **\_\_init\_\_** ( *self* , dbUser, dbPswd, dbHost):  *'''*  **: param** *dbUser:*  **: param** *dbPswd:*  **: param** *dbHost:*  *'''* |
...
|  |
| --- |
| def **method** ( *self* , a):  *self* .call\_method = a.callMethod() |
