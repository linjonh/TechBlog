...
\hat{e} = \arg\max\_e P(e|f) = \arg\max\_e \frac{P(f|e)P(e)}{P(f)}
...
\mathcal{L} = \sum\_{(f,e)} \log P(f|e) = \sum\_{(f,e)} \sum\_{j=1}^{m} \log \sum\_{i=0}^{l} t(f\_j|e\_i)a(i|j,l,m)
...
1. **E步**
   ...
   P(a\_j=i|f,e) = \frac{t(f\_j|e\_i)a(i|j,l,m)}{\sum\_{k=0}^l t(f\_j|e\_k)a(k|j,l,m)}
...
2. **M步**
   ...
   t(f|e) = \frac{\text{count}(f,e)}{\sum\_{f'}\text{count}(f',e)} \\ a(i|j,l,m) = \frac{\text{count}(i,j,l,m)}{\sum\_{k=0}^l \text{count}(k,j,l,m)}
...
\hat{\lambda} = \arg\min\_{\lambda} \sum\_{k=1}^K \text{Err}(e^{(k)}, \hat{e}^{(k)})
...
h\_t^{enc} = \text{LSTM}(x\_t, h\_{t-1}^{enc})
...
h\_i^{dec} = \text{LSTM}(y\_{i-1}, h\_{i-1}^{dec}) \\ P(y\_i|y\_{<i}, X) = \text{softmax}(W\_o h\_i^{dec} + b\_o)
...
c\_i = \sum\_{j=1}^T \alpha\_{ij} h\_j^{enc} \\ \alpha\_{ij} = \frac{\exp(\text{score}(h\_i^{dec}, h\_j^{enc}))}{\sum\_{k=1}^T \exp(\text{score}(h\_i^{dec}, h\_k^{enc}))}
...
\text{score}(a,b) = v^T \tanh(W\_a a + W\_b b)
...
V
Q = X W^Q, \quad K = X W^K, \quad V = X W^V \\ \text{Attention}(Q,K,V) = \text{softmax}\left(\frac{QK^T}{\sqrt{d\_k}}\right)V
...
d
k
W^Q, W^K \in \mathbb{R}^{d \times d\_k}
...
PE(pos+\Delta) = T\_\Delta \cdot PE(pos)
...
\begin{bmatrix}\sin(\theta\_i (pos+\Delta)) \\\cos(\theta\_i (pos+\Delta))\end{bmatrix}= \begin{bmatrix} \cos(\theta\_i \Delta) & \sin(\theta\_i \Delta) \\ -\sin(\theta\_i \Delta) & \cos(\theta\_i \Delta) \end{bmatrix} \cdot \begin{bmatrix} \sin(\theta\_i \, pos) \\ \cos(\theta\_i \, pos) \end{bmatrix}
...
\text{Count}\_{clip}(n\text{-gram}) = \min(\text{Count}(n\text{-gram}), \max\_{ref}\text{Count}\_{ref}(n\text{-gram}))
...
P(y|x) = \sum\_{d\in\mathcal{D}} \pi\_d(x) P\_d(y|x)
