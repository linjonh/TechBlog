...
**ACTION :[72] 'select action from gv$session where sid=:1 and serial#=:2 and inst\_id=:3' 日志里看不出来，好像备份没有关，猜测大概率是备份导致的**
...
1. 从传统审计转移到统一审计，包括弃用AUDIT\_SYS\_OPERATIONS参数（推荐）。
2. 通过设置AUDIT\_SYS\_OPERATIONS = FALSE来禁用系统审计（并重启数据库以使其生效）。

audit\_sys\_operations 默认为false，当设置为true时，所有sys用户（包括以sysdba, sysoper身份登录的用户）的操作都会被记录
