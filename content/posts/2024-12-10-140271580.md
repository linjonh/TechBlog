...
**目录下，所以选择的file\_save\_address为**
DATA\_PUMP\_DIR**
，这个地址是Oracle服务器所安装的主机中的地址。

1. 在Oracle服务器所安装的主机上运行CMD命令

导出mydb用户下的数据库，并将地址保存在DATA\_PUMP\_DIR 目录下，导出文件名为mydb20191113\_expdp.dmp ，导出的日志文件为mydb20191113\_expdp.log，Oracle数据库版本为11.2.0.1.0
...
DATA\_PUMP\_DIR 是在Oracle管理工具中执行SQL语句后的地址信息中有一条地址名称。
