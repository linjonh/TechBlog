...
2. 可以在windows更改系统环境变量加上JAVA\_OPTS=-Xms64m -Xmx512m。
3. 如果用的tomcat,在windows下,可以在C:\tomcat5.5.9\bin\catalina.bat（具体路径根据自己tomcat的位置而定） 中加上:set JAVA\_OPTS=-Xms64m -Xmx256m （大小依自己内存而定）位置在: rem Guess CATALINA\_HOME if not defined 这行的下面加合适.
4. 如果是linux系统Linux 在{tomcat\_home}/bin/catalina.sh的前面，加 set JAVA\_OPTS=’-Xms64 -Xmx512’

### 设置

1、设置环境变量
  
set JAVA\_OPTS= -Xms32m -Xmx512m
