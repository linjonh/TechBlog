 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre><code>public class NettyServerHandler extends ChannelInboundHandlerAdapter {
    @Override
    ...
    <pre><code>public class NettyClientHandler extends ChannelInboundHandlerAdapter {
...
ctx.writeAndFlush(resp);</code></pre>
    <p>
    </p>
    <p>
     <strong>
...
ctx.writeAndFlush(resp);</code></pre>
    ...
    <pre><code>//添加固定长度解码器
//在绝大多数情况下，这些解码器会添加到对应ChannelPipeline的起始位置
ch.pipeline().addLast(new FixedLengthFrameDecoder(200));</code></pre>
    <p>
    </p>
    <p>
     <strong>
    ...
    <p class="img-center">
     <img alt="图片" height="282" src="https://i-blog.csdnimg.cn/img_convert/cd1316f0339b1e072578efea049ffff9.png" width="1080"/>
    </p>
    <p>
     于是就可以根据长度域偏移量和长度域的长度来构造一个解码器，其中第一个参数是数据包的最大长度，第二个参数是长度域的偏移量，第三个参数是长度域的长度。
    </p>
    <pre><code>//添加基于长度域的解码器
//在绝大多数情况下，这些解码器会添加到对应ChannelPipeline的起始位置
ch.pipeline().addLast(new LengthFieldBasedDecoder(Integer.MAX_VALUE, 7, 4));</code></pre>
    <p>
 ...
 <p alt="687474:70733a2f2f626c6f672e6373646e2e6e65742f6d6a756e7a2f:61727469636c652f64657461696c732f313436323638393130" class_="artid" style="display:none">
 </p>
</div>
