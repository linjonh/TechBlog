...
这部分代码首先使用新的工具变量 “new\_z” 重新估计内生性随机边界模型，并将结果存储在 “model2” 中。然后，使用 “estimates table” 命令输出两个模型的估计结果比较。
...
这部分代码在模型中加入控制变量 “control\_var”，重新估计内生性随机边界模型，并将结果存储在 “model4” 中。然后，输出两个模型的估计结果比较。
