...
mid = (left + right) // 2
# 递归解决左半部分和右半部分
left\_max = divideAndConquer(left, mid)
right\_max = divideAndConquer(mid+1, right)
# 计算跨越中点的最大子数组和
cross\_max = maxCrossingSum(left, mid, right)
return max(left\_max, right\_max, cross\_max)
def maxCrossingSum(left, mid, right):
left\_sum = float('-inf')
current\_sum = 0
for i in range(mid, left-1, -1):
current\_sum += nums[i]
left\_sum = max(left\_sum, current\_sum)
right\_sum = float('-inf')
current\_sum = 0
for i in range(mid+1, right+1):
current\_sum += nums[i]
right\_sum = max(right\_sum, current\_sum)
