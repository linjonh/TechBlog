...
参考华为官方镜像网站：
[华为开源镜像站\_软件开发服务\_华为云](https://mirrors.huaweicloud.com/mirrorDetail/5ebe3408c8ac54047fe607f0?mirrorName=openeuler&catalog=os "华为开源镜像站_软件开发服务_华为云")
...
show\_custom\_stats = True

interpreter\_python = auto\_legacy\_silent
...
[root@controller ansible]# awk '!/^#/ && !/^$/' all-in-one > yoga\_all-in-one
...
"discovered\_interpreter\_python": "/usr/bin/python3.10"
...
[root@controller ~]# sed -i 's/keystone\_admin\_password: .\*/keystone\_admin\_password: wxic@2024/g' /etc/kolla/passwords.yml

#

验证修改结果

[root@controller ~]# grep keystone\_admin /etc/kolla/passwords.yml

keystone\_admin\_password: wxic@2024
...
docker\_kernel\_memory: 0

docker\_registry: "quay.nju.edu.cn"

kolla\_base\_distro: "ubuntu"

kolla\_install\_type: "source"

openstack\_release: "yoga"

kolla\_internal\_vip\_address: "192.168.100.10"

kolla\_sysctl\_conf\_path: /etc/sysctl.conf

docker\_client\_timeout: 120

network\_interface: "ens224"

network\_address\_family: "ipv4"

neutron\_external\_interface: "ens160"

neutron\_plugin\_agent: "openvswitch"

neutron\_ipam\_driver: "internal"

openstack\_region\_name: "RegionWxic"

openstack\_logging\_debug: "False"

enable\_openstack\_core: "yes"

glance\_backend\_file: "yes"

nova\_compute\_virt\_type: "kvm"

nova\_console: "novnc"

enable\_haproxy: "no"

enable\_keepalived: "no"
...
[root@controller config]# cat << MXD > neutron/dhcp\_agent.ini

[DEFAULT]

dnsmasq\_dns\_servers = 8.8.8.8,223.6.6.6,119.29.29.29

MXD

[root@controller config]# cat << MXD > neutron/ml2\_conf.ini

[ml2]

tenant\_network\_types = flat,vxlan,vlan

[ml2\_type\_vlan]

network\_vlan\_ranges = provider:10:1000
...
[root@controller config]# cat << MXD > neutron/openvswitch\_agent.ini
...
bridge\_mappings = provider:br-ex
...
[root@controller ~]# modprobe br\_netfilter
...
Description=Load br\_netfilter and sysctl settings for OpenStack
...
ExecStart=/sbin/modprobe br\_netfilter
...
[root@controller ~]# vi $HOME/.ansible/collections/ansible\_collections/openstack/kolla/roles/docker/tasks/repo-RedHat.yml
...
https://mirrors.aliyun.com/docker-ce/linux/centos/7/x86\_64/stable
...
bugzilla.redhat.com/show\_bug.cgi?id=1734081
...
when: docker\_yum\_gpgcheck | bool
...
MariaDB [(none)]>
**CREATE DATABASE skyline DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8\_general\_ci**
**；**

Query OK, 1 row affected (0.004 sec)

MariaDB [(none)]>
**GRANT ALL PRIVILEGES ON skyline. \* TO'skyline'@'localhost' IDENTIFIED BY 'mariadb\_yoga';**

Query OK, 0 rows affected (0.006 sec)

MariaDB [(none)]>
**GRANT ALL PRIVILEGES ON skyline. \* TO 'skyline'@' %' IDENTIFIED BY 'mariadb\_yoga';**
...
access\_token\_expire: 3600

access\_token\_renew: 1800
...
database\_url: mysql://skyline:mariadb\_yoga@192.168.233.100:3306/skyline

debug: false

log\_dir: /var/log/skyline

log\_file: skyline\_wxic.log

prometheus\_basic\_auth\_password: 'wxic@yoga'

prometheus\_basic\_auth\_user: ''

prometheus\_enable\_basic\_auth: false

prometheus\_endpoint: http://192.168.233.100:9091

secret\_key: aCtmgbcUqYUy\_HNVg5BDXCaeJgJQzHJXwqbXr0Nmb2o
...
default\_region: RegionWxic

enforce\_new\_defaults: true

extension\_mapping:

floating-ip-port-forwarding: neutron\_port\_forwarding

fwaas\_v2: neutron\_firewall

qos: neutron\_qos

vpnaas: neutron\_vpn

interface\_type: public

# keystone

认证地址

keystone\_url: http://192.168.233.100:5000/v3/

nginx\_prefix: /api/openstack

reclaim\_instance\_interval: 604800
...
sso\_region: RegionWxic

system\_admin\_roles:

- admin

- system\_admin

system\_project: service

system\_project\_domain: Default

system\_reader\_roles:

- system\_reader

system\_user\_domain: Default

system\_user\_name: skyline

# skyline

用户密码

system\_user\_password: 'wxic@yoga'
...
- architecture: x86\_architecture

categories:

- name: general\_purpose

properties: []

- name: compute\_optimized

properties: []

- name: memory\_optimized

properties: []

- name: high\_clock\_speed

properties: []

- architecture: heterogeneous\_computing

categories:

- name: compute\_optimized\_type\_with\_gpu

properties: []

- name: visualization\_compute\_optimized\_type\_with\_gpu
...
[root@controller ~]# docker run -d --name skyline\_bootstrap \
...
# 查看 skyline\_bootstrap 容器的日志

[root@controller ~]# docker logs -f skyline\_bootstrap
...
make: \*\*\* [Makefile:119: db\_sync] Error 1
...
CREATE USER 'skyline'@'%' IDENTIFIED BY 'mariadb\_yoga';
...
表结构创建成功后，删除数据库初始化引导容器 skyline\_bootstrap。

[root@controller ~]# docker rm -f skyline\_bootstrap skyline\_bootstrap
...
-e LISTEN\_ADDRESS=192.168.100.10:9942 \
