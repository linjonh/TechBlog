...
#### 3.15、is\_monotonic\_decreasing
返回一个布尔值，表示Series是否按降序排列。
import pandas as pd
series = pd.Series([5, 4, 3, 2, 1])
print(series.is\_monotonic\_decreasing)
True
#### 3.16、is\_monotonic\_increasing
返回一个布尔值，表示Series是否按升序排列。
import pandas as pd
series = pd.Series([5, 4, 3, 2, 1])
print(series.is\_monotonic\_increasing)
False
## 四、 Series中元素的索引与访问
### 4.1、位置索引
...
data = [10, 20, 30, 40, 50]
series = pd.Series(data)
values\_to\_check = [20, 40]
result = series.isin(values\_to\_check)
print(result)
0 False
1 True
2 False
