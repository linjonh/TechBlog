...
首先我们使用@RunWith(SpringRunner.class)声明在Spring的环境中进行单元测试，这样Spring的相关注解才会被识别并起效。然后我们使用@SpringBootTest，它会扫描应用程序的spring配置，并构建完整的Spring Context。我们为其参数webEnvironment赋值为SpringBootTest.WebEnvironment.RANDOM\_PORT，这样就会启动web容器，并监听一个随机的端口，同时，为我们自动装配一个TestRestTemplate类型的bean来辅助我们发送请求。
...
@FixMethodOrder(value = MethodSorters.NAME\_ASCENDING)
