...
> 提醒：请务必使用管理员身份运行终端，防止后续操作因权限不够导致的错误。
1. 使用anaconda创建虚拟环境
conda create -n pytorch\_cpu python=3.9
2. 执行conda初始化
conda init
3. 激活虚拟环境
conda activate pytorch\_cpu
4. 安装cpu版的pytorch
pip3 install torch torchvision torchaudio
5. 测试pytorch是否安装成功
import torch
print(torch.\_\_version\_\_) # 输出版本号
print(torch.cuda.is\_available()) # AMD GPU下可能返回False（需ROCm支持）
(pytorch\_cpu) PS C:\Users\HP> python
