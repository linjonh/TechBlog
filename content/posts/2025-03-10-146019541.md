...
如果想使用内存池，则需要开启RT\_USING\_MEMPOOL宏
...
内存池的释放与内核对象的释放一致，根据初始化的不同，分别调用rt\_mp\_detach和rt\_mp\_delete即可

#### 内存池的申请

void \*rt\_mp\_alloc(rt\_mp\_t mp, rt\_int32\_t time)的核心代码如下所示，首先我们忽略没有内存块可用的情况
...
3. 如果存在suspend\_thread等待内存块申请，则
   唤醒suspend\_thread队列中第一个线程

这里一定要注意，如果你使用的是以下仓库，内存池其实是存在优先级翻转的，因为进入suspend\_thread的线程句柄并没有按照优先级排序，而唤醒队列中的第一个
