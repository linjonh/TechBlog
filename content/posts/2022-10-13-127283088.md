...
* s 缩放视频尺寸
* pix\_fmt 设置视频颜色空间。 网络视频通常用：yuv420p
...
> in\_w in\_h 或者 iw ih 表示输入视频的宽高
>   
> out\_w out\_h 或者 ow oh 表示输出视频的宽高

复杂滤镜用命令 -filter\_complex 表示，它还有一个别名 -lavfi。

> * `-filter_complex`
>   : 相比-vf（-vf 是 -filter:v 的简写）, filter\_complex适合开发复杂的滤镜功能，如同时对视频进行裁剪并旋转。参数之间使用逗号（，）隔开即可
> * `main_w`
>   :视频宽度
> * `overlay_w`
>   : 要添加的图片水印宽度
