...
对str(bytes\_instance)，str(bytearray\_instance) 以及将bytes/bytearray与str比较时，产生警告信息。如果使用了
*-bb*
选项，则产生错误信息。
...
下面的例子，在未指定-B选项时，在执行\_\_main\_\_.py脚本时，会在当前目录下创建\_\_pycache\_\_子目录，在该目录下保存生成的.pyc文件。
...
-O：删掉assert与\_\_debug\_\_依赖的语句；在保存的.pyc文件扩展名之前，添加.opt-1字样；等价于PYTHONOPTMIZE=x
