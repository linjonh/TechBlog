...
1、WordPress 的 GiveWP Donation Plugin 和 Fundraising Platform 插件在 3.14.1 之前的所有版本中都容易受到 PHP 对象注入的攻击，该插件通过反序列化来自“give\_title”参数的不受信任的输入。这使得未经身份验证的攻击者可以注入 PHP 对象。POP 链的额外存在允许攻击者远程执行代码，并删除任意文件。
...
2）使用give\_donation\_form\_nonce获取获取给定捐赠表格的随机数；
  
3）通过接口give\_process\_donation的参数give\_title利用的php调用链进行反序列化执行。

### 2.2 漏洞复现过程

**1）使用admin-ajax.php获取捐赠表单**

https://IP\_or\_域名/wp-admin/admin-ajax.php
  
POST内容为action=give\_form\_search
