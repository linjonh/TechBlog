...
* docker\_data.vhdx文件，Docker在Windows系统上使用WSL2（Windows Subsystem for Linux 2）时创建的虚拟磁盘文件，用于存储Docker容器和镜像等数据。该文件通常位于

  > C:\Users<用户名>\AppData\Local\Docker\wsl\distributions\docker-desktop-data
  >
  > C:\Users\孟智超\AppData\Local\Docker\wsl\disk\docker\_data.vhdx
* 关闭Docker Desktop
