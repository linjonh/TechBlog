 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker volume create -d local test
test
</code></pre>
    <p>
     此时，查看/var/lib/docker/volumes 路径下，会发现所创建的数据卷位置：
    </p>
    <pre><code class="language-bash">[root@open-Euler2 ~]# ll /var/lib/docker/volumes/
drwx-----x 3 root root   4096 Mar 10 14:55 test
    ...
    <pre><code class="language-bash">docker run -d -P --name my-nginx -v test:/usr/share/nginx/html nginx
</code></pre>
    <p>
    ...
    <pre><code class="language-bash">docker run -d -P --name my-nginx --mount source=test,target=/usr/share/nginx/html nginx
</code></pre>
    <p>
     <strong>
      访问数据卷
    ...
    <pre><code class="language-bash">[root@open-Euler2 webapp]# docker run -itd -P --name web1 -v /webapp/:/opt/webapp nginx:latest
...
index.html
    ...
    <pre><code class="language-bash">docker run -d -P --name web2 -v /webapp:/opt/webapp:ro nginx:latest
</code></pre>
    <p>
     加了
     <strong>
    ...
    <p>
     <img alt="" height="71" src="https://i-blog.csdnimg.cn/direct/f806929e016a47caa9394262bf6c2f4f.png" width="941"/>
    </p>
    <p>
     然后，可以在其他容器中使用--volumes-from来挂载 dbdata 容器中的数据卷，例如创建 db1 和 db2 两个容器，并从dbdata容器挂载数据卷：
    </p>
    <pre><code class="language-bash">[root@open-Euler2 webapp]# docker run -it --volumes-from dbdata --name db1 ubuntu
[root@open-Euler2 webapp]# docker run -it --volumes-from dbdata --name db2 ubuntu
    ...
    <pre><code class="language-bash">[root@open-Euler2 webapp]# docker start dbdata
dbdata
...
test
    ...
    <pre><code class="language-bash">[root@open-Euler2 webapp]# docker start db1
db1
...
test
    ...
    <pre><code class="language-bash"> docker run -d --name db3 --volumes-from dbl ubuntu</code></pre>
    <blockquote>
     <p>
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run --volumes-from dbdata -v $(pwd):/backup --name worker ubuntu tar cvf /backup/backup.tar /dbdata
...
-rw-r--r--  1 root root   78603326 Feb 26 22:56 docker-28.0.1.tgz
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -itd -v /dbdata --name dbdata2 ubuntu bash
...
dbdata/test
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -P nginx
...
89a9b4b78277   nginx     "/docker-entrypoint.…"   3 seconds ago   Up 3 seconds   0.0.0.0:32769-&gt;80/tcp, :::32769-&gt;80/tcp   crazy_northcutt
    ...
    <p>
     <img alt="" height="312" src="https://i-blog.csdnimg.cn/direct/be4b892be92142ea9e4f3135340caed3.png" width="946"/>
    </p>
    <p>
     同样， 可以通过docker logs命令来查看应用的信息：
    </p>
    <p>
     <img alt="" height="380" src="https://i-blog.csdnimg.cn/direct/edec2fa8a0474b079880efd93666c4b0.png" width="1091"/>
    </p>
    <p>
     <strong>
      -p
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -p 80:80 nginx
</code></pre>
    <p>
     此时默认会绑定本地所有接口上的所有地址。多次使用-p标记可以绑定多个端口。例如：
    </p>
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -p 5000:5000 -p 3000:80 training/webapp python app.py
</code></pre>
    <h4>
     3.映射到指定地址的指定端口
    </h4>
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -p 192.168.121.151:80:80 nginx
955050b980528f98312c296388d43cc2e2a30e3f84403f758157b06c81a47d25
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -p 127.0.0.1::80 nginx
478a6d0bf890ac189721a50617b1e0d6d6343127a405bda3f1898bb2b473c030
</code></pre>
    <p>
     还可以使用udp标记来指定udp端口：
    </p>
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -d -p 127.0.0.1:80:80/udp nginx
...
955050b98052   nginx     "/docker-entrypoint.…"   41 minutes ago       Up 41 minutes       192.168.121.151:80-&gt;80/tcp     inspiring_cori
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker port 5572bc9e356d
80/udp -&gt; 127.0.0.1:80
</code></pre>
    <blockquote>
     <p>
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker run -itd --name web nginx bash</code></pre>
    <p>
     使用docker ps来验证设定的命名：
    </p>
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker ps -l
CONTAINER ID   IMAGE     COMMAND                  CREATED         STATUS         PORTS     NAMES
2d8f7dc9ddfa   nginx     "/docker-entrypoint.…"   7 seconds ago   Up 7 seconds   80/tcp    web
    ...
    <pre><code class="language-bash">[root@open-Euler2 ~]# docker inspect -f "{<!-- -->{ .Name }}" web
/web
</code></pre>
    <blockquote>
     <p>
    ...
    <p>
     <img alt="" height="260" src="https://i-blog.csdnimg.cn/direct/ff8ee3ddb27a425c9287cfac072de005.png" width="873">
      <code>
    ...
    <p>
     <img alt="" height="55" src="https://i-blog.csdnimg.cn/direct/7322089b1d8f44acb1746666edcb23ad.png" width="666"/>
    </p>
    <p>
     删除之前创建的web容器：
    </p>
    <pre><code class="language-bash">docker rm -f web</code></pre>
    <p>
     然后基于Wordpress镜像创建一个新的web容器，并将它连接到db容器：
    </p>
    <pre><code class="language-bash">docker run --name web -p 80:80 --link db:db -d --restart=always wordpress
</code></pre>
    <p>
     此时，db容器和web容器建立互联关系。
    </p>
    ...
    <p>
     <img alt="" height="108" src="https://i-blog.csdnimg.cn/direct/27369923efd94a0b81ee2955629aba41.png" width="968"/>
    </p>
    <p>
    ...
    <p>
     <img alt="" height="179" src="https://i-blog.csdnimg.cn/direct/ae1bbbf798f1497ca8f6b8da4514c1c5.png" width="620"/>
    </p>
    <p>
     <strong>
      更新环境变量:
    ...
    <p>
     <img alt="" height="500" src="https://i-blog.csdnimg.cn/direct/09317426c94e4bed97ecb88ec64bda04.png" width="913"/>
    </p>
    <p>
     <strong>
      wordpress应用搭建：
    ...
    <p>
     <img alt="" height="111" src="https://i-blog.csdnimg.cn/direct/00816a6296114d47b8537a2189cbd550.png" width="915"/>
    </p>
    <p>
     进入安装界面:
    </p>
    <p>
     <img alt="" height="742" src="https://i-blog.csdnimg.cn/direct/314c4f4642cb4e8f8d2f42ce65f8415b.png" width="946"/>
    </p>
    <p>
     填写数据库信息：
    </p>
    <p>
     <img alt="" height="681" src="https://i-blog.csdnimg.cn/direct/b67da1294dcb484f9ca85bad8233b434.png" width="947"/>
    </p>
    <p>
     填写站点信息
    </p>
    <p>
     <img alt="" height="993" src="https://i-blog.csdnimg.cn/direct/1575a50dfe2d45538b77d717a704fc4f.png" width="937"/>
    </p>
    <p>
     安装成功
    </p>
    <p>
     <img alt="" height="478" src="https://i-blog.csdnimg.cn/direct/a1c510998e264281b2d028670666cccf.png" width="952"/>
    </p>
    <p>
     进入wordpress主页
    </p>
    <p>
     <img alt="" height="988" src="https://i-blog.csdnimg.cn/direct/0b13ec2a39b9412fa24969db411e9913.png" width="946"/>
    </p>
    <h3>
     2.3 本章小结
    </h3>
 ...
 <p alt="68747470733a2f2f626c6f:672e6373646e2e6e65742f323330325f37373739313930352f:61727469636c652f64657461696c732f313436313534313535" class_="artid" style="display:none">
 </p>
</div>
