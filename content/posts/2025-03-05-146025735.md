 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre><code class="language-python"># 导入正则表达式模块，用于处理正则匹配
import re
...
if __name__ == "__main__":
    ...
    for review in product_reviews:
        ...
        # 打印评论内容及其分类结果
        print(f"评论: {review}，分类结果: {classification_result}")</code></pre>
    <h5>
     运行结果
    </h5>
    <pre><code class="language-python">评论: 这个商品非常好，我很满意，分类结果: 好评
...
进程已结束，退出代码为 0</code></pre>
    ...
    <p>
     将文档表示为词的序列
     <img alt="d=(x_1, x_2,\cdots, x_n)" class="mathcode" src="https://latex.csdn.net/eq?d%3D%28x_1%2C%20x_2%2C%5Ccdots%2C%20x_n%29">
      ，类别集合为
    ...
    <p style="text-align:center">
     <img alt="P(c|d)=\frac{P(d|c)P(c)}{P(d)}" class="mathcode" src="https://latex.csdn.net/eq?P%28c%7Cd%29%3D%5Cfrac%7BP%28d%7Cc%29P%28c%29%7D%7BP%28d%29%7D"/>
    </p>
    <p>
     由于
     <img alt="P(d)" class="mathcode" src="https://latex.csdn.net/eq?P%28d%29">
      对于所有类别都是相同的常数，在比较不同类别概率大小时可忽略，因此朴素贝叶斯分类的目标是求解：
     </img>
    </p>
    <p style="text-align:center">
     <img alt="\hat{c}=\arg\max_{c\in C}P(d|c)P(c)=\arg\max_{c\in C}P(x_1, x_2,\cdots, x_n|c)P(c)" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7Bc%7D%3D%5Carg%5Cmax_%7Bc%5Cin%20C%7DP%28d%7Cc%29P%28c%29%3D%5Carg%5Cmax_%7Bc%5Cin%20C%7DP%28x_1%2C%20x_2%2C%5Ccdots%2C%20x_n%7Cc%29P%28c%29"/>
    </p>
    <p>
     基于条件独立假设，
     <img alt="P(x_1, x_2,\cdots, x_n|c)=\prod_{i = 1}^{n}P(x_i|c)" class="mathcode" src="https://latex.csdn.net/eq?P%28x_1%2C%20x_2%2C%5Ccdots%2C%20x_n%7Cc%29%3D%5Cprod_%7Bi%20%3D%201%7D%5E%7Bn%7DP%28x_i%7Cc%29"/>
     ，所以公式可进一步写为：
    </p>
    <p style="text-align:center">
     <img alt="\hat{c}=\arg\max_{c\in C}P(c)\prod_{i = 1}^{n}P(x_i|c)" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7Bc%7D%3D%5Carg%5Cmax_%7Bc%5Cin%20C%7DP%28c%29%5Cprod_%7Bi%20%3D%201%7D%5E%7Bn%7DP%28x_i%7Cc%29"/>
    </p>
    <p>
     其中，
     <img alt="\hat{c}" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7Bc%7D"/>
     是预测的文档类别；
     <img alt="P(c)" class="mathcode" src="https://latex.csdn.net/eq?P%28c%29"/>
     是类别
     <img alt="c" class="mathcode" src="https://latex.csdn.net/eq?c"/>
     的先验概率；
     <img alt="P(x_i|c)" class="mathcode" src="https://latex.csdn.net/eq?P%28x_i%7Cc%29"/>
     是在类别
     <img alt="c" class="mathcode" src="https://latex.csdn.net/eq?c"/>
     下词
    ...
    <pre><code class="language-python">pip install spacy</code></pre>
    <p>
     <strong>
      注：spacy
     </strong>
    ...
    <pre><code class="language-python">python -m spacy download zh_core_web_sm</code></pre>
    <p>
     <strong>
      注：zh_core_web_sm
     </strong>
    ...
    <pre><code class="language-python"># 导入用于处理JSON数据的模块
import json
...
class DatasetProcessor:
    ...
    <pre><code class="language-python"># 导入用于数值计算的numpy库
import numpy as np
...
if __name__ == "__main__":
    ...
    <pre><code class="language-python">训练数据集大小 = 8627 , 测试数据集大小 = 2157
...
进程已结束，退出代码为 0</code></pre>
    ...
    <p>
     ...
     个特征的样本
     <img alt="$x = (x_1, x_2, ..., x_n)$" class="mathcode" src="https://latex.csdn.net/eq?%24x%20%3D%20%28x_1%2C%20x_2%2C%20...%2C%20x_n%29%24"/>
     ，其线性组合为
     <img alt="$z = w_0 + w_1x_1 + w_2x_2 + ... + w_nx_n$" class="mathcode" src="https://latex.csdn.net/eq?%24z%20%3D%20w_0%20&amp;plus;%20w_1x_1%20&amp;plus;%20w_2x_2%20&amp;plus;%20...%20&amp;plus;%20w_nx_n%24"/>
     ，可以写成向量形式
     <img alt="$z = w^Tx$" class="mathcode" src="https://latex.csdn.net/eq?%24z%20%3D%20w%5ETx%24"/>
     ，其中
     <img alt="$w = (w_0, w_1, ..., w_n)$" class="mathcode" src="https://latex.csdn.net/eq?%24w%20%3D%20%28w_0%2C%20w_1%2C%20...%2C%20w_n%29%24"/>
     是模型的参数向量，
     <img alt="$x = (1, x_1, x_2, ..., x_n)$" class="mathcode" src="https://latex.csdn.net/eq?%24x%20%3D%20%281%2C%20x_1%2C%20x_2%2C%20...%2C%20x_n%29%24"/>
     (在
     <img alt="$x$" class="mathcode" src="https://latex.csdn.net/eq?%24x%24"/>
     向量中添加了一个常数项
     <img alt="$1$" class="mathcode" src="https://latex.csdn.net/eq?%241%24"/>
     与
     <img alt="$w_0$" class="mathcode" src="https://latex.csdn.net/eq?%24w_0%24"/>
     对应)。
    </p>
    <p>
     <strong>
     ...
     函数
     <img alt="$\sigma(z) = \frac{1}{1 + e^{-z}}$" class="mathcode" src="https://latex.csdn.net/eq?%24%5Csigma%28z%29%20%3D%20%5Cfrac%7B1%7D%7B1%20&amp;plus;%20e%5E%7B-z%7D%7D%24"/>
     ，将
     <img alt="$z$" class="mathcode" src="https://latex.csdn.net/eq?%24z%24"/>
     映射到区间
     <img alt="$(0, 1)$" class="mathcode" src="https://latex.csdn.net/eq?%24%280%2C%201%29%24"/>
     上，得到样本属于正类(通常标记为
     <strong>
      1
     </strong>
     )的概率
     <img alt="$p = \sigma(z) = \frac{1}{1 + e^{-w^Tx}}$" class="mathcode" src="https://latex.csdn.net/eq?%24p%20%3D%20%5Csigma%28z%29%20%3D%20%5Cfrac%7B1%7D%7B1%20&amp;plus;%20e%5E%7B-w%5ETx%7D%7D%24"/>
     。那么样本属于负类(通常标记为
     <strong>
      0
     </strong>
     )的概率为
     <img alt="$1 - p$" class="mathcode" src="https://latex.csdn.net/eq?%241%20-%20p%24"/>
     。
    </p>
    <p>
     <strong>
    ...
    <ul>
     <li>
      ...
      <img alt="$w$" class="mathcode" src="https://latex.csdn.net/eq?%24w%24"/>
      。对于一个二分类问题，假设样本
      <img alt="$x_i$" class="mathcode" src="https://latex.csdn.net/eq?%24x_i%24"/>
      属于正类的概率为
      <img alt="$p_i = \frac{1}{1 + e^{-w^Tx_i}}$" class="mathcode" src="https://latex.csdn.net/eq?%24p_i%20%3D%20%5Cfrac%7B1%7D%7B1%20&amp;plus;%20e%5E%7B-w%5ETx_i%7D%7D%24"/>
      ，属于负类的概率为
      <img alt="$1 - p_i$" class="mathcode" src="https://latex.csdn.net/eq?%241%20-%20p_i%24"/>
      。给定一组独立同分布的样本
      <img alt="$(x_1, y_1), (x_2, y_2), ..., (x_m, y_m)$" class="mathcode" src="https://latex.csdn.net/eq?%24%28x_1%2C%20y_1%29%2C%20%28x_2%2C%20y_2%29%2C%20...%2C%20%28x_m%2C%20y_m%29%24"/>
      ，其中
      <img alt="$y_i \in \{0, 1\}$" class="mathcode" src="https://latex.csdn.net/eq?%24y_i%20%5Cin%20%5C%7B0%2C%201%5C%7D%24"/>
      是样本
      <img alt="$x_i$" class="mathcode" src="https://latex.csdn.net/eq?%24x_i%24"/>
      的真实标签，其似然函数为
      <img alt="$L(w) = \prod_{i = 1}^{m} p_i^{y_i} (1 - p_i)^{1 - y_i}$" class="mathcode" src="https://latex.csdn.net/eq?%24L%28w%29%20%3D%20%5Cprod_%7Bi%20%3D%201%7D%5E%7Bm%7D%20p_i%5E%7By_i%7D%20%281%20-%20p_i%29%5E%7B1%20-%20y_i%7D%24"/>
     </li>
     <li>
      <strong>
       对数似然函数：
      ...
      <img alt="$l(w) = \sum_{i = 1}^{m} [y_i \ln(p_i) + (1 - y_i) \ln(1 - p_i)]$" class="mathcode" src="https://latex.csdn.net/eq?%24l%28w%29%20%3D%20%5Csum_%7Bi%20%3D%201%7D%5E%7Bm%7D%20%5By_i%20%5Cln%28p_i%29%20&amp;plus;%20%281%20-%20y_i%29%20%5Cln%281%20-%20p_i%29%5D%24"/>
     </li>
     <li>
      <strong>
       参数估计：
      ...
      <img alt="$w$" class="mathcode" src="https://latex.csdn.net/eq?%24w%24"/>
      ，使得对数似然函数
      <img alt="$l(w)$" class="mathcode" src="https://latex.csdn.net/eq?%24l%28w%29%24"/>
    ...
    <p>
     我们使用代码来演示，基于博主在
     <a href="https://blog.csdn.net/chengyidechengxu/article/details/145958820" title="自然语言处理：文本表示-CSDN博客">
      自然语言处理：文本表示-CSDN博客
     </a>
     介绍的
     <strong>
...
class TFIDFTransformer:
    ...
    <pre><code class="language-python"># 从torch模块导入nn子模块，用于构建神经网络
...
class LogisticRegressionModel(nn.Module):
    ...
    <pre><code class="language-python"># 从torch.utils.data模块导入Dataset类，用于自定义数据集
...
class DataCollation:
    ...
    def collate_batch_data(cls, batch):
        ...
        return {'input_features': features, 'labels': labels}</code></pre>
    <p>
     <strong>
      nlp_text_classification.py
     </strong>
...
进程已结束，退出代码为 0</code></pre>
    <p class="img-center">
     <img alt="" height="369" src="https://i-blog.csdnimg.cn/direct/f4b47f323e4244e6b72fc9f4e1bbf6a1.png" width="500"/>
    </p>
    <p>
     <strong>
      注意：
    ...
    <p style="text-align:center">
     <img alt="$Accuracy = \frac{TP + TN}{TP + TN + FP + FN}$" class="mathcode" src="https://latex.csdn.net/eq?%24Accuracy%20%3D%20%5Cfrac%7BTP%20&amp;plus;%20TN%7D%7BTP%20&amp;plus;%20TN%20&amp;plus;%20FP%20&amp;plus;%20FN%7D%24"/>
    </p>
    <p>
     其中
     <img alt="$TP$" class="mathcode" src="https://latex.csdn.net/eq?%24TP%24"/>
     (真正例)是指实际为正例且被正确预测为正例的样本数，
     <img alt="$TN$" class="mathcode" src="https://latex.csdn.net/eq?%24TN%24"/>
     (真反例)是实际为反例且被正确预测为反例的样本数，
     <img alt="$FP$" class="mathcode" src="https://latex.csdn.net/eq?%24FP%24"/>
     (假正例)是实际为反例但被错误预测为正例的样本数，
     <img alt="$FN$" class="mathcode" src="https://latex.csdn.net/eq?%24FN%24"/>
    ...
    <p style="text-align:center">
     <img alt="$Precision = \frac{TP}{TP + FP}$" class="mathcode" src="https://latex.csdn.net/eq?%24Precision%20%3D%20%5Cfrac%7BTP%7D%7BTP%20&amp;plus;%20FP%7D%24"/>
    </p>
    <p>
     例如，模型预测了
     <strong>
     ...
     条是积极情感，那么精确率为
     <img alt="$\frac{25}{30} \approx 83.3\%$" class="mathcode" src="https://latex.csdn.net/eq?%24%5Cfrac%7B25%7D%7B30%7D%20%5Capprox%2083.3%5C%25%24"/>
     。
    </p>
    <h5>
     召回率
    ...
    <p style="text-align:center">
     <img alt="$Recall = \frac{TP}{TP + FN}$" class="mathcode" src="https://latex.csdn.net/eq?%24Recall%20%3D%20%5Cfrac%7BTP%7D%7BTP%20&amp;plus;%20FN%7D%24"/>
    </p>
    <p>
     例如，实际有
     <strong>
     ...
     条，召回率就是
     <img alt="$\frac{30}{40} = 75\%$" class="mathcode" src="https://latex.csdn.net/eq?%24%5Cfrac%7B30%7D%7B40%7D%20%3D%2075%5C%25%24"/>
     。
    </p>
    <h5>
     F1值
    ...
    <p style="text-align:center">
     <img alt="$F1 = \frac{2 \times Precision \times Recall}{Precision + Recall}$" class="mathcode" src="https://latex.csdn.net/eq?%24F1%20%3D%20%5Cfrac%7B2%20%5Ctimes%20Precision%20%5Ctimes%20Recall%7D%7BPrecision%20&amp;plus;%20Recall%7D%24"/>
    </p>
    <p>
     <strong>
      F1
...
if __name__ == "__main__":
    ...
    <pre><code class="language-python">NB: micro-f1 = 1.0, macro-f1 = 1.0
...
进程已结束，退出代码为 0</code></pre>
 ...
 <p alt="68747470733a2f2f626c6f672e63:73646e2e6e65742f6368656e67796964656368656e6778752f:61727469636c652f64657461696c732f313436303235373335" class_="artid" style="display:none">
 </p>
</div>
