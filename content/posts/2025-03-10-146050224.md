...
2
K\_i=\frac{mean(|x\_i(t)|^4)}{mean(|x\_i(t)|^2)^2}
...
2
K\_i=\frac{mean(|x\_i(t)|^2)}{mean(|x\_i(t)|)^2}
...
在获取到FIR滤波器参数后，下面就可以进行谱峭度的计算了，主要的函数就是K\_wpQ。
...
可以看到该函数的核心功能就是K\_wpQ\_local函数实现的。
...
下面我们再通过图示的方式，更为清晰地展示K\_wpQ函数运算的逻辑。
