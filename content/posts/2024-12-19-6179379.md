...
Android SDK 是一个压缩的文件夹。下载这个文件夹并把它的内容解压到您计算机上的适当位置。对于本教程，SDK 安装在 c:/software/google/android\_m3-rc37a。显然，如果要在 Mac OS X 和 Linux 上安装 SDK，应该安装在通常安装开发工具的位置。
...
****fill\_parent****:   使视图元素占满可用的空间。也可以认为这就是 “拉伸”。

****wrap\_content****:   这个值让 Android 把元素一个接一个地排列，不进行拉伸。
