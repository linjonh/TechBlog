...
![image-20250313110018679](https://i-blog.csdnimg.cn/img\_convert/073af5eeabbac436e8878ef9f598072e.png)
...
* |11000000| - 3 bytes
* Integer encoded as int16\_t (2 bytes).
* |11010000| - 5 bytes
* Integer encoded as int32\_t (4 bytes).
* |11100000| - 9 bytes
* Integer encoded as int64\_t (8 bytes).
* |11110000| - 4 bytes
...
```
![image-20250313131315802](https://i-blog.csdnimg.cn/img\_convert/a87e3937feefd00f3e638677f66e200a.png)
...
// 分配头空间和尾空间 11 字节
unsigned int bytes = ZIPLIST\_HEADER\_SIZE + ZIPLIST\_END\_SIZE;
...
p = (where == ZIPLIST\_HEAD) ? ZIPLIST\_ENTRY\_HEAD(zl) : ZIPLIST\_ENTRY\_END(zl);
...
size\_t curlen = intrev32ifbe(ZIPLIST\_BYTES(zl)), reqlen;
...
int forcelarge = 0;
nextdiff = (p[0] != ZIP\_END) ? zipPrevLenByteDiff(p, reqlen) : 0;
...
if (p[0] != ZIP\_END) {
/* Subtract one because of the ZIP\_END bytes */
...
if (p[reqlen + tail.headersize + tail.len] != ZIP\_END) {
ZIPLIST\_TAIL\_OFFSET(zl) =
...
* ```c
hash\_max\_ziplist\_entries // 512
```
最大元素长度，插入的任意一个 value 的长度超过 64
* ```c
hash\_max\_ziplist\_value // 64
```
