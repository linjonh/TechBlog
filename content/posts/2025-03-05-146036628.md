...
在Java生态中，Spring框架及其衍生技术（如Spring MVC、Spring Boot、Spring
Cloud）是开发企业级应用的核心工具。它们在功能、定位和使用场景上各有侧重，但又紧密联系。本文将详细解析它们的区别与联系，帮助你在面试中更好地回答相关问题。
...
**定义：**  
Spring MVC是Spring框架的一个模块，是一个基于MVC（Model-View-
Controller）设计模式的Web框架。它通过`DispatcherServlet`、`ModelAndView`和`View
Resolver`等组件，简化了Web应用的开发。
...
**核心特性：**
  ...
  * **简化配置** ：通过`application.properties`或`application.yml`文件集中管理配置。
