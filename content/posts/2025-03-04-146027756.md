...
[Bardbo/get\_bus\_lines\_and\_stations\_data\_from\_gaode: 这个项目是基于高德开放平台和公交网获取公交线路及站点数据，并生成shp文件，代码相对粗糙，但简单可用

![](https://csdnimg.cn/release/blog_editor_html/release2.3.8/ckeditor/plugins/CsdnLink/icons/icon-default.png?t=P1C7)
https://github.com/Bardbo/get\_bus\_lines\_and\_stations\_data\_from\_gaode](https://github.com/Bardbo/get_bus_lines_and_stations_data_from_gaode "Bardbo/get_bus_lines_and_stations_data_from_gaode: 这个项目是基于高德开放平台和公交网获取公交线路及站点数据，并生成shp文件，代码相对粗糙，但简单可用")
...
（如
`changsha、wuhan`
），函数会返回该城市所有公交线路的名称列表（line\_name）。

## **`3. 爬取公交路线（get_line_station_data` 函数）**

获取公交线路的url是高德的
[https://restapi.amap.com/v3/bus/linename?extensions=all&key={ak}&output=json&city={city}&offset=1&keywords={line\_name}](https://restapi.amap.com/v3/bus/linename?extensions=all&key=%7bak%7d&output=json&city=%7bcity%7d&offset=1&keywords=%7bline_name%7d "https://restapi.amap.com/v3/bus/linename?extensions=all&key={ak}&output=json&city={city}&offset=1&keywords={line_name}")
...
自己需要设置的是最开始的city、city\_phonetic、ak.
...
get\_station\_data：将站点坐标和名称从字符串格式转换为列表格式，将数据从横向展开为纵向，并去除重复项；

get\_line\_data：将线路的折线数据从字符串格式转换为列表格式;

create\_station\_shp：创建站点Shapefile，包含站点名称和坐标；

create\_lines\_shp：创建线路Shapefile，包含线路名称和折线坐标；

其实作者其实在这里还引入了从高德的火星坐标系转换为WGS\_84的函数，但是大部分时候这种转换并不可靠，所以建议高德爬取的数据就搭配高德地图进行可视化使用。
