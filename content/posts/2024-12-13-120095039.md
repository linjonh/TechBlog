...
>   
> 当虚拟环境被激活（即虚拟环境的 Python 解释器正在运行），属性 sys.prefix 和 sys.exec\_prefix 指向的是虚拟环境的基础目录，而 sys.base\_prefix 和 sys.base\_exec\_prefix 指向非虚拟环境的 Python 安装，即曾用于创建虚拟环境的那个 Python 安装。如果虚拟环境没有被激活，则 sys.prefix 与 sys.base\_prefix 相同，且 sys.exec\_prefix 与 sys.base\_exec\_prefix 相同（它们均指向非虚拟环境的 Python 安装）。
>   
