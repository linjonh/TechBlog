...
bash
keytool -import -alias website\_cert -file /path/to/certificate.crt -keystore $JAVA\_HOME/jre/lib/security/cacerts
\* 输入默认密码`changeit`（若提示）。
2. ​\*\*自动信任所有证书（仅调试用）​\*\* ：
