...
* column\_stack: 将多个1维数组作为列堆叠成一个2维数组
...
本质上column\_stack相当于hstack的一种特殊形式。在实现过程中，column\_stack会先将输入数组扩展成二维，然后再用hstack进行堆叠。因此，其具体过程等价于
...
虽然从定义上column\_stack只对一维数组进行操作，但是实际使用中如果输入数组的维度大于1，则其结果等同于hstack
