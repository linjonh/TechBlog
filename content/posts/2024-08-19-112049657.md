...
# 数据库变为可疑\_SQL Server 2008R2 数据库出现“可疑”导致无法访问解决办法
...
ALTER DATABASE jd13dafa SET SINGLE\_USER

3、修复数据库日志重新生成，此命令检查的分配，结构，逻辑完整性和所有数据库中的对象错误。当您指定“REPAIR\_ALLOW\_DATA\_LOSS”作为DBCC CHECKDB命令参数，该程序将检查和修复报告的错误。但是，这些修复可能会导致一些数据丢失。

DBCC CheckDB (jd13dafa , REPAIR\_ALLOW\_DATA\_LOSS)

4、使数据库变回为多用户模式

ALTER DATABASE jd13dafa SET MULTI\_USER
