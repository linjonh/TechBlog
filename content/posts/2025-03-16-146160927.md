...
<div class="blog-content-box">
 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <p class="img-center">
     <img alt="" height="489" src="https://i-blog.csdnimg.cn/img_convert/1f2aaad3c8a317a71dca47bca9afecad.jpeg" width="479"/>
    </p>
    ...
    <p>
     ...
     <img alt="L = [k^0, k^1, \dots, k^{n-1}]" class="mathcode" src="https://latex.csdn.net/eq?L%20%3D%20%5Bk%5E0%2C%20k%5E1%2C%20%5Cdots%2C%20k%5E%7Bn-1%7D%5D">
      ，其中 n 设置为 10，并且每个 3D 点表示为
      <img alt="k_i = (x, y, z) \in \mathbb{R}^3" class="mathcode" src="https://latex.csdn.net/eq?k_i%20%3D%20%28x%2C%20y%2C%20z%29%20%5Cin%20%5Cmathbb%7BR%7D%5E3">
       。拓扑描述为一个邻接矩阵
       <img alt="A \in \mathbb{R}^{m \times m}" class="mathcode" src="https://latex.csdn.net/eq?A%20%5Cin%20%5Cmathbb%7BR%7D%5E%7Bm%20%5Ctimes%20m%7D">
        ，其中 m 为检测到的中心线数量。在该矩阵中，
        <img alt="A_{ij} = 1" class="mathcode" src="https://latex.csdn.net/eq?A_%7Bij%7D%20%3D%201">
    ...
    <p class="img-center">
     <img alt="" height="545" src="https://i-blog.csdnimg.cn/img_convert/cf4dc1dd6e4057204db3829d053b280b.jpeg" width="1012"/>
    </p>
    <p style="text-align:center">
    ...
    <p>
     ...
     <img alt="F \in \mathbb{R}^{H \times W \times C}" class="mathcode" src="https://latex.csdn.net/eq?F%20%5Cin%20%5Cmathbb%7BR%7D%5E%7BH%20%5Ctimes%20W%20%5Ctimes%20C%7D">
      。车道线段解码器通过自注意力和与 BEV 特征
      <img alt="F" class="mathcode" src="https://latex.csdn.net/eq?F"/>
      的交叉注意力来更新可学习的查询
      <img alt="Q^l" class="mathcode" src="https://latex.csdn.net/eq?Q%5El"/>
      。更新后的查询被传入预测头进行车道分割。本文从预测车道图的起点和终点中提取关键点作为关键点提示
      <img alt="Y^k" class="mathcode" src="https://latex.csdn.net/eq?Y%5Ek"/>
      。真值 (GT) 车道图被转换为边序列
      <img alt="Y^l" class="mathcode" src="https://latex.csdn.net/eq?Y%5El"/>
      。边序列与关键点提示
      <img alt="Y^k" class="mathcode" src="https://latex.csdn.net/eq?Y%5Ek"/>
    ...
    <p>
     本文将一组实例级查询表示为
     <img alt="\{Q^l\}_{j=1}^{N_L}" class="mathcode" src="https://latex.csdn.net/eq?%5C%7BQ%5El%5C%7D_%7Bj%3D1%7D%5E%7BN_L%7D"/>
     ，其中
     <img alt="N_L" class="mathcode" src="https://latex.csdn.net/eq?N_L"/>
     为预设的查询数量，通常大于车道图中的中心线数量。查询被传入车道线段解码器中，以获得更新的查询：
    </p>
    <p style="text-align:center">
     <img alt="\hat{Q}^l = \text{LaneDec}(F, Q^l)" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7BQ%7D%5El%20%3D%20%5Ctext%7BLaneDec%7D%28F%2C%20Q%5El%29"/>
    </p>
    <p>
    ...
    <p style="text-align:center">
     <img alt="\hat{Q}^l_{emb_1}, \hat{Q}^l_{emb_2} = \text{MLP}(\hat{Q}^l), \text{MLP}(\hat{Q}^l)" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7BQ%7D%5El_%7Bemb_1%7D%2C%20%5Chat%7BQ%7D%5El_%7Bemb_2%7D%20%3D%20%5Ctext%7BMLP%7D%28%5Chat%7BQ%7D%5El%29%2C%20%5Ctext%7BMLP%7D%28%5Chat%7BQ%7D%5El%29"/>
    </p>
    <p style="text-align:center">
     <img alt="A = \text{Sigmoid}(\text{MLP}(\text{Concat}(\hat{Q}^l_{emb_1}, \hat{Q}^l_{emb_2})))" class="mathcode" src="https://latex.csdn.net/eq?A%20%3D%20%5Ctext%7BSigmoid%7D%28%5Ctext%7BMLP%7D%28%5Ctext%7BConcat%7D%28%5Chat%7BQ%7D%5El_%7Bemb_1%7D%2C%20%5Chat%7BQ%7D%5El_%7Bemb_2%7D%29%29%29"/>
    </p>
    <p>
    ...
    <p style="text-align:center">
     <img alt="\hat{y}^E = \text{TopoSeqDec}(F, \text{Concat}(y^K, y^E))" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7By%7D%5EE%20%3D%20%5Ctext%7BTopoSeqDec%7D%28F%2C%20%5Ctext%7BConcat%7D%28y%5EK%2C%20y%5EE%29%29"/>
    </p>
    <p>
     其中，
     <img alt="\text{TopoSeqDec}" class="mathcode" src="https://latex.csdn.net/eq?%5Ctext%7BTopoSeqDec%7D"/>
     表示拓扑序列解码器，
     <img alt="\hat{y}^E" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7By%7D%5EE"/>
     表示预测的边序列。
    </p>
    <h4>
     序列结构
    ...
    <p style="text-align:center">
     <img alt="\hat{y}^E = [\text{int}(x_i), \text{int}(y_i), \text{cls}, \text{con}, \text{int}(bx_i), \text{int}(by_i)]" class="mathcode" src="https://latex.csdn.net/eq?%5Chat%7By%7D%5EE%20%3D%20%5B%5Ctext%7Bint%7D%28x_i%29%2C%20%5Ctext%7Bint%7D%28y_i%29%2C%20%5Ctext%7Bcls%7D%2C%20%5Ctext%7Bcon%7D%2C%20%5Ctext%7Bint%7D%28bx_i%29%2C%20%5Ctext%7Bint%7D%28by_i%29%5D"/>
    </p>
    <p>
     其中，前两个整数
     <img alt="\text{int}(x_i)" class="mathcode" src="https://latex.csdn.net/eq?%5Ctext%7Bint%7D%28x_i%29"/>
     、
     <img alt="\text{int}(y_i)" class="mathcode" src="https://latex.csdn.net/eq?%5Ctext%7Bint%7D%28y_i%29"/>
     表示关键点的离散坐标。
     <img alt="\text{cls}" class="mathcode" src="https://latex.csdn.net/eq?%5Ctext%7Bcls%7D"/>
    ...
    <p class="img-center">
     <img alt="" height="260" src="https://i-blog.csdnimg.cn/img_convert/2cdfa332d72418ac6cb31f5502e4e9f3.jpeg" width="477"/>
    </p>
    <p style="text-align:center">
    ...
    <p>
     ...
     <img alt="y_k" class="mathcode" src="https://latex.csdn.net/eq?y_k"/>
    ...
    <p style="text-align:center">
     <img alt="y_k^E \leftarrow \begin{cases} k_{j}^{n-1} &amp; A_{ij} = 1 \\ k_{j}^{0} k_{j}^{n-1} &amp; A_{ij} = 0 \end{cases}" class="mathcode" src="https://latex.csdn.net/eq?y_k%5EE%20%5Cleftarrow%20%5Cbegin%7Bcases%7D%20k_%7Bj%7D%5E%7Bn-1%7D%20%26%20A_%7Bij%7D%20%3D%201%20%5C%5C%20k_%7Bj%7D%5E%7B0%7D%20k_%7Bj%7D%5E%7Bn-1%7D%20%26%20A_%7Bij%7D%20%3D%200%20%5Cend%7Bcases%7D"/>
    </p>
    <p>
     其中，关键点的坐标是离散的。随机顺序的提示到序列学习的目标表述如下：
    </p>
    <p style="text-align:center">
     <img alt="\max \sum_{i=1}^{L} w_i \log P(y_i^E | \text{Concat}(y_i^E, y_i^E), \mathcal{F})" class="mathcode" src="https://latex.csdn.net/eq?%5Cmax%20%5Csum_%7Bi%3D1%7D%5E%7BL%7D%20w_i%20%5Clog%20P%28y_i%5EE%20%7C%20%5Ctext%7BConcat%7D%28y_i%5EE%2C%20y_i%5EE%29%2C%20%5Cmathcal%7BF%7D%29"/>
    </p>
    <p>
     其中，
     <img alt="w_i" class="mathcode" src="https://latex.csdn.net/eq?w_i"/>
     表示类别权重，
     <img alt="y_i^E" class="mathcode" src="https://latex.csdn.net/eq?y_i%5EE"/>
     表示
     <img alt="y_i^E" class="mathcode" src="https://latex.csdn.net/eq?y_i%5EE"/>
     之前的 tokens，
     <img alt="y^E" class="mathcode" src="https://latex.csdn.net/eq?y%5EE"/>
    ...
    <p style="text-align:center">
     <img alt="L = \alpha_1 L_1 + \alpha_2 L_{\text{cls}} + \alpha_3 L_{\text{seg}} + \alpha_4 L_{\text{lt}} + \alpha_5 L_{\text{top}} + \alpha_6 L_{\text{seq}}" class="mathcode" src="https://latex.csdn.net/eq?L%20%3D%20%5Calpha_1%20L_1%20&amp;plus;%20%5Calpha_2%20L_%7B%5Ctext%7Bcls%7D%7D%20&amp;plus;%20%5Calpha_3%20L_%7B%5Ctext%7Bseg%7D%7D%20&amp;plus;%20%5Calpha_4%20L_%7B%5Ctext%7Blt%7D%7D%20&amp;plus;%20%5Calpha_5%20L_%7B%5Ctext%7Btop%7D%7D%20&amp;plus;%20%5Calpha_6%20L_%7B%5Ctext%7Bseq%7D%7D"/>
    </p>
    <p>
     其中，L1 表示 L1 损失。
     <img alt="L_{\text{cls}}" class="mathcode" src="https://latex.csdn.net/eq?L_%7B%5Ctext%7Bcls%7D%7D"/>
     表示焦点损失，用于车道分类。
     <img alt="L_{\text{seg}}" class="mathcode" src="https://latex.csdn.net/eq?L_%7B%5Ctext%7Bseg%7D%7D"/>
     包括交叉熵损失和 diss 损失。
     <img alt="L_{\text{lt}}" class="mathcode" src="https://latex.csdn.net/eq?L_%7B%5Ctext%7Blt%7D%7D"/>
     表示交叉熵损失，用于分类左右车道类型。
     <img alt="L_{\text{top}}" class="mathcode" src="https://latex.csdn.net/eq?L_%7B%5Ctext%7Btop%7D%7D"/>
    ...
    <p class="img-center">
     <img alt="" height="209" src="https://i-blog.csdnimg.cn/img_convert/8ebfb5e9773c866aa486b8a8353ddb7b.jpeg" width="1010"/>
    </p>
    <p style="text-align:center">
    ...
    <p>
     <img alt="" height="318" src="https://i-blog.csdnimg.cn/direct/3d666f93011c4c77b39dd540528128bc.png" width="1011"/>
    </p>
    <p style="text-align:center">
    ...
    <p>
     <img alt="" height="150" src="https://i-blog.csdnimg.cn/direct/edeff179163945e99e528b42da8217b4.png" width="1012"/>
    </p>
    <p style="text-align:center">
    ...
    <p>
     <img alt="" height="583" src="https://i-blog.csdnimg.cn/direct/24716341b52847ceadf710e2f3a15dee.png" width="763"/>
    </p>
    <p style="text-align:center">
     ▲图4｜ 定性结果©️【深蓝AI】编译
    </p>
 ...
 <p alt="68747470733a2f2f626c6f:672e6373646e2e6e65742f736f6172696e675f63617369612f:61727469636c652f64657461696c732f313436313630393237" class_="artid" style="display:none">
 </p>
</div>
