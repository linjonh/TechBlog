...
在游戏开发中，我们常常利用游戏引擎每一帧传递的 `delta` 值（代表从上一帧到当前帧所经过的时间，单位通常为秒）来实现计时逻辑。下面分别以 Godot
引擎和 Unity 引擎为例，介绍如何通过累加 `delta` 来判断每几秒的逻辑。

#### Godot 引擎

在 Godot 里，`_process` 函数会在每一帧被调用，并且会传入 `delta` 参数。以下是一个示例代码，展示如何通过累加 `delta`
来实现每 3 秒执行一次特定逻辑：
