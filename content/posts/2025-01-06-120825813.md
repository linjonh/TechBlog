...
下面以创建一个商品销售管理数据库（shop)为例,要求：有两个数据文件，其中主数据文件初始大小为10MB,最大容量为60MB,按2MB增长,文件名为SHOP. mdf;1个辅助数据文件为10MB,最大大小不限,按10%增长,文件名为 SHOP. ndf。该数据库有2个日志文件,SHOP\_log.df和SHOP\_log1.ldf,大小、最大长度和文件增长幅度均采用系统默认值。所有文件存放在d:\SHOP文件夹中。
