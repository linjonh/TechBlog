...
keywords: "基于 Qwen2.5-14B + Elasticsearch RAG 的大数据知识库智能问答系统"
categories: ['未分类']
...
本文介绍了一款基于 Qwen2.5-14B 大语言模型（换成 DeepSeek 原理一致）与 Elasticsearch
搜索引擎构建的大数据知识库智能问答系统。

该系统结合了[向量检索技术]()、检索增强技术（[RAG]()）和[生成式 AI]()，能够高效处理多格式文档并为用户提供准确、自然的回答。
...
![image.png](https://i-blog.csdnimg.cn/img_convert/4610f2c055c8e53ce21cf2adec386540.png)![image.png](https://i-blog.csdnimg.cn/img_convert/11ed52da63081158756f28ad58d70245.png)![image.png](https://i-blog.csdnimg.cn/img_convert/5d7008b5501f83198cbd7357a2e5ad02.png)
...
![8661c9cfe30e8f39a961e5b86c21ac5a.png](https://i-blog.csdnimg.cn/img_convert/8661c9cfe30e8f39a961e5b86c21ac5a.png)
  ...
  * **Elasticsearch 8.15.3** : 
    ...
    * 核心搜索引擎，负责存储和检索[向量化]()的文档数据。

  * **Kibana 8.15.3** : 

    * [可视化]()工具，用于监控和管理 Elasticsearch 的状态与索引。
...
用户查询首先通过向量化模型（`shibing624/text2vec-base-chinese`）转化为向量，在 Elasticsearch
中检索相关文档，随后将查询与检索结果传递给 Qwen2.5-14b 生成最终回答。
...
利用 `shibing624/text2vec-base-chinese` 对文档和查询进行[向量化]()：
...
基于 Qwen2.5-14b 与 Elasticsearch 的大数据知识库智能问答系统，通过向量检索与生成式 AI 的检索增强 RAG
结合，为用户提供了高效、精准的信息获取途径。
