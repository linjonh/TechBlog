...
第 11 行通过 socket() 函数创建了一个套接字，参数 AF\_INET 表示使用 IPv4 地址，SOCK\_STREAM 表示使用面向连接的套接字，IPPROTO\_TCP 表示使用 TCP 协议。在 Linux 中，socket 也是一种文件，有文件描述符，可以使用 write() / read() 函数进行 I/O 操作，这一点已在《socket是什么》中进行了讲解。

第 19 行通过 bind() 函数将套接字 serv\_sock 与特定的 IP 地址和端口绑定，IP 地址和端口都保存在 sockaddr\_in 结构体中。
...
第 19 行代码通过 connect() 向服务器发起请求，服务器的IP地址和端口号保存在 sockaddr\_in 结构体中。直到服务器传回数据后，connect() 才运行结束。
