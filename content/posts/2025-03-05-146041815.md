---
arturl_encode: "68747470733a2f2f62:6c6f672e6373646e2e6e65742f71715f33363037303130342f:61727469636c652f64657461696c732f313436303431383135"
layout: post
title: "doris-MySQL"
date: 2025-03-05 14:06:01 +08:00
...
### 使用须知 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E4%BD%BF%E7%94%A8%E9%A1%BB%E7%9F%A5 "​")

要连接到 MySQL 数据库，您需要

* MySQL 5.7, 8.0 或更高版本
* MySQL 数据库的 JDBC 驱动程序，您可以从
  [Maven 仓库](https://mvnrepository.com/artifact/com.mysql/mysql-connector-j "Maven 仓库")
  下载最新或指定版本的 MySQL JDBC 驱动程序。
  **推荐使用 MySQL Connector/J 8.0.31 及以上版本。**
* Doris 每个 FE 和 BE 节点和 MySQL 服务器之间的网络连接，默认端口为 3306。

### 连接 MySQL [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%BF%9E%E6%8E%A5-mysql "​")
...
[MySQL 开发指南](https://dev.mysql.com/doc/connector-j/en/connector-j-reference-configuration-properties.html "MySQL 开发指南")
中找到。

#### 连接安全 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%BF%9E%E6%8E%A5%E5%AE%89%E5%85%A8 "​")
...
[MySQL JDBC 安全文档](https://dev.mysql.com/doc/connector-j/en/connector-j-connp-props-security.html#cj-conn-prop_sslMode "MySQL JDBC 安全文档")
。

### 层级映射 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E5%B1%82%E7%BA%A7%E6%98%A0%E5%B0%84 "​")
...
### 类型映射 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E7%B1%BB%E5%9E%8B%E6%98%A0%E5%B0%84 "​")

#### MySQL 到 Doris 类型映射 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#mysql-%E5%88%B0-doris-%E7%B1%BB%E5%9E%8B%E6%98%A0%E5%B0%84 "​")
...
#### 时间戳类型处理 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E6%97%B6%E9%97%B4%E6%88%B3%E7%B1%BB%E5%9E%8B%E5%A4%84%E7%90%86 "​")

在 JDBC 类型 Catalog 读取数据时，BE 的 Java 部分使用 JVM 时区。JVM 时区默认为 BE 部署机器的时区，这会影响 JDBC 读取数据时的时区转换。

为了确保时区一致性，建议在 be.conf 的 JAVA\_OPTS 中设置 JVM 时区与 Doris session 的
`time_zone`
一致。
...
### 查询优化 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E6%9F%A5%E8%AF%A2%E4%BC%98%E5%8C%96 "​")

#### 谓词下推 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%B0%93%E8%AF%8D%E4%B8%8B%E6%8E%A8 "​")
...
#### 行数限制 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%A1%8C%E6%95%B0%E9%99%90%E5%88%B6 "​")

如果在查询中带有 limit 关键字，Doris 会将 limit 下推到 MySQL，以减少数据传输量。

#### 转义字符 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%BD%AC%E4%B9%89%E5%AD%97%E7%AC%A6 "​")

Doris 会在下发到 MySQL 的查询语句中，自动在字段名与表名上加上转义符：(``)，以避免字段名与表名与 MySQL 内部关键字冲突。

### 连接异常排查 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E8%BF%9E%E6%8E%A5%E5%BC%82%E5%B8%B8%E6%8E%92%E6%9F%A5 "​")
...
* Communications link failure The last packet successfully received from the server was 7 milliseconds ago.
  ...
  + 原因：
    - 网络问题：
      ...
      * 客户端和服务器之间的网络延迟过高。
    - MySQL 服务器设置
      * MySQL 服务器可能配置了连接超时参数，例如 wait\_timeout 或 interactive\_timeout，导致连接超时被关闭。
    - 防火墙设置
      * 防火墙规则可能阻止了客户端与服务器之间的通信。
    - 连接池设置
      * 连接池中的配置 connection\_pool\_max\_life\_time 可能导致连接被关闭或回收，或者未及时探活
    - 服务器资源问题
...
### 常见问题 [​](https://doris.apache.org/zh-CN/docs/lakehouse/database/mysql#%E5%B8%B8%E8%A7%81%E9%97%AE%E9%A2%98 "​")
...
3. 读取 MySQL Catalog 或其他 JDBC Catalog 时，出现加载类失败
   ...
   这是因为在创建 Catalog 时，填写的 driver\_class 不正确，需要正确填写，如上方例子为大小写问题，应填写为
   `"driver_class" = "com.mysql.cj.jdbc.Driver"`
4. 读取 MySQL 出现通信链路异常
...
6. 查询 MySQL 的过程中，如果发现和在 MySQL 库的查询结果不一致的情况
   ...
   ```
7. 查询 MySQL 的时候，出现长时间卡住没有返回结果，或着卡住很长时间并且 fe.warn.log 中出现出现大量 write lock 日志，可以尝试在 URL 添加 socketTimeout，例如：
...
8. 在使用 MySQL Catalog 的过程中发现 FE 的 JVM 内存或 Threads 数持续增长不减少，并可能同时出现 Forward to master connection timed out 报错
   ...
   2. 在 FE 和 BE conf 文件的 JAVA\_OPTS 中增加
