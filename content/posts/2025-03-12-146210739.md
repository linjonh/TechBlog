  ...
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    <p style="text-align:center">
     <img alt="" src="https://i-blog.csdnimg.cn/direct/5ca93d2f4e664eb58b0dd46fc3038d9c.png"/>
    </p>
    <p style="text-align:center">
     <img alt="" src="https://i-blog.csdnimg.cn/direct/bd6794cf8109475f9707b08b0beea7f4.png"/>
    </p>
    <ol>
     <li>
    ...
    <ol>
     <li>
      train_batch_size = 64：定义训练数据的批次大小为 64，即每次训练使用 64 个样本。
     </li>
     <li>
      test_batch_size = 128：定义测试数据的批次大小为 128。
     </li>
     <li>
      learning_rate = 0.01：设置学习率为 0.01，控制模型参数更新的步长。
     </li>
     <li>
      num_epochs = 20：定义训练的轮数为 20，即整个训练数据集将被遍历 20 次。
     </li>
    </ol>
    <p style="margin-left:0; margin-right:0">
     <strong>
    ...
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="315" src="https://i-blog.csdnimg.cn/direct/341f10706d9145c4b75f85d1642d606a.png" width="1127"/>
    </p>
    <ol>
     <li>
      examples = enumerate(test_loader)：使用enumerate函数对测试数据加载器test_loader进行包装，使其在迭代时可以同时返回索引和数据。
     </li>
     <li>
      batch_idx, (example_data, example_targets) = next(examples)：通过next函数从examples中获取第一个批次的数据，batch_idx是批次索引，example_data是该批次的图像数据，example_targets是对应的真实标签。
     </li>
     <li>
      example_data.shape 和 Out[10]: torch.Size([128, 1, 28, 28])：查看第一个批次图像数据的形状，结果表明该批次有 128 个样本，每个样本是 1 通道、大小为 28x28 的图像。
     </li>
    </ol>
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="743" src="https://i-blog.csdnimg.cn/direct/21ffa44632f54c02acdec70cf8507543.png" width="1123"/>
    </p>
    <ol>
     <li>
      <strong>
    ...
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="644" src="https://i-blog.csdnimg.cn/direct/3af09300e0d94890b3d17bbfb274c755.png" width="1119"/>
    </p>
    <ol>
     <li>
      <strong>
    ...
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="637" src="https://i-blog.csdnimg.cn/direct/326b45583dae48bfb09d4620a699982f.png" width="1118"/>
    </p>
    <ol>
     <li>
      <strong>
    ...
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="653" src="https://i-blog.csdnimg.cn/direct/1fd3a0cdafa54822be02673fd8fa2443.png" width="1081"/>
    </p>
    <ol>
     <li>
      <strong>
    ...
    <p style="margin-left:0; margin-right:0">
     <img alt="" height="557" src="https://i-blog.csdnimg.cn/direct/2ca3125eb8e74e0782b0d085e0fb0399.png" width="1123"/>
    </p>
    <ol>
     <li>
      plt.title('train loss')：设置绘制图形的标题为 “train loss”，用于表明该图展示的是训练损失相关内容。
     </li>
     <li>
      plt.plot(np.arange(len(losses)), losses)：使用plot函数绘制折线图。np.arange(len(losses))生成一个从 0 到losses列表长度减 1 的数组，作为横坐标，表示训练的轮次；losses列表存储了每一轮训练的平均损失值，作为纵坐标。这样就将训练轮次和对应的损失值进行了可视化展示。
     </li>
     <li>
      plt.legend(['Train Loss'], loc='upper right')：为图形添加图例，图例内容为 “Train Loss”，并将其放置在图形的右上角（loc='upper right'），方便读者识别曲线代表的含义。
     </li>
    </ol>
   </div>
  </div>
 </article>
 <p alt="68747470733a2f2f626c6f:672e6373646e2e6e65742f323430315f38343837333731352f:61727469636c652f64657461696c732f313436323130373339" class_="artid" style="display:none">
 </p>
</div>
