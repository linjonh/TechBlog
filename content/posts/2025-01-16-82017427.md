...
安装验签工具：
[Gen\_Signature\_Android2.apk](%E9%99%84%E4%BB%B6/Gen_Signature_Android2.apk)
。
...
2. 通过code参数加上AppID和AppSecret等，通过API换取access\_token；

3. 通过access\_token进行接口调用，获取用户基本数据资源或帮助用户实现基本操作。
...
在工程中新建一个libs目录，将开发工具包中libs目录下的libammsdk.jar复制到该目录中（如下图所示，建立了一个名为SDK\_Sample 的工程，并把jar包复制到libs目录下）。
