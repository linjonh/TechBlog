...
3. 测试 验证码 输入框对应的 verify\_code 的值，是否正确
...
pm.test(“Body matches string”, function () { pm.expect(pm.response.text()).to.include(“string\_you\_want\_to\_search”);
...
pm.test(“Body is correct”, function () { pm.response.to.have.body(“response\_body\_string”);

//获取响应体等于response\_body\_string
