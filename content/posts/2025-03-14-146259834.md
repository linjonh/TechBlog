---
layout: post
title: "雷电模拟器连接Android-Studio步骤"
date: 2025-03-14 16:08:33 +0800
description: "打开雷电模拟器，点击桌面系统应用—>打开设置—>关于平板电脑→连续点击5次版本号，会出现开发者选项—->进入开发者选项—->勾选打开usb调试。回到Android Studio，运行项目，将程序run起来，在logcat就会收到模拟器的链接信息，选择雷电模拟器。就能在雷电模拟器看到效果。命令行提示符，进入雷电模拟器安装目录。"
keywords: "雷电模拟器连接Android Studio步骤"
categories: ['Andriod']
tags: ['Studio', 'Android', 'Android']
artid: "146259834"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=146259834
    alt: "雷电模拟器连接Android-Studio步骤"
render_with_liquid: false
featuredImage: https://bing.ee123.net/img/rand?artid=146259834
featuredImagePreview: https://bing.ee123.net/img/rand?artid=146259834
cover: https://bing.ee123.net/img/rand?artid=146259834
image: https://bing.ee123.net/img/rand?artid=146259834
img: https://bing.ee123.net/img/rand?artid=146259834
---

<div class="blog-content-box">
 <div class="article-header-box">
  <div class="article-header">
   <div class="article-title-box">
    <h1 class="title-article" id="articleContentId">
     雷电模拟器连接Android Studio步骤
    </h1>
   </div>
  </div>
 </div>
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   <link href="../../assets/css/kdoc_html_views-1a98987dfd.css" rel="stylesheet"/>
   <link href="../../assets/css/ck_htmledit_views-704d5b9767.css" rel="stylesheet"/>
   <div class="htmledit_views" id="content_views">
    <ol>
     <li>
      <p>
       打开雷电模拟器，点击桌面系统应用—&gt;打开设置—&gt;关于平板电脑→连续点击5次版本号，会出现开发者选项—-&gt;进入开发者选项—-&gt;勾选打开usb调试。
       <img alt="" src="https://i-blog.csdnimg.cn/direct/3d40a3a2115e46e886c2b07523154aa9.png"/>
      </p>
      <ol>
       <li style="text-align: center;">
        <img alt="" src="https://i-blog.csdnimg.cn/direct/51d4f9208b8a48e5a70cf21528d5ab65.png"/>
       </li>
       <li>
        <p style="text-align:center">
         <img alt="" src="https://i-blog.csdnimg.cn/direct/c015e50914764419875f1c464b2045a4.png"/>
        </p>
       </li>
       <li>
        <p>
         命令行提示符，进入雷电模拟器安装目录。然后执行
        </p>
        <p>
         <code>
          Plain Text adb.exe connect 127.0.0.1:5555
         </code>
        </p>
       </li>
       <li>
        <p>
         回到Android Studio，运行项目，将程序run起来，在logcat就会收到模拟器的链接信息，选择雷电模拟器。就能在雷电模拟器看到效果。
        </p>
       </li>
      </ol>
     </li>
    </ol>
   </div>
  </div>
 </article>
 <p alt="68747470733a2f2f626c6f672e:6373646e2e6e65742f77656978696e5f33393931383631362f:61727469636c652f64657461696c732f313436323539383334" class_="artid" style="display:none">
 </p>
</div>


