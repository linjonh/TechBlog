...
• 输入约束语法可以简介为set\_input\_delay -max 40T…，set\_input\_delay -min 0…；
  
• 没有跨时域交互的电路，其他不同时钟的电路可以约束不考虑他们的路径，set\_false\_path…，方便后面PR布线；
  
• create\_generated\_clock可以使顶层时钟与模块内部时钟同源无相位差。
...
• 设置好搜索路径.load\_parasitics，库链接路径.link\_path；
  
• 读入设计的约束参数，定义时钟约束，不确定性uncertainty，涉后时间timing\_derate等约束；
