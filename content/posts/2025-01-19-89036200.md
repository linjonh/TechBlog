...
在这个代码的基础上制作更高阶的版本也非常容易实现。在puzzle()函数中引入3个全局变量dim\_x,dim\_y和depth，编写setdim()分别来设置维数。
...
标记矩阵的数值变为1到depth，维度为dim\_x,dim\_y
...
回调函数，基本是将3×3的3换成dim\_x,dim\_y，当数字超过depth变为1，胜利条件变为与1到depth的dim\_x×dim\_y的矩阵比较。
