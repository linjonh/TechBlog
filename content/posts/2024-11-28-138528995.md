...
在Linux中，使用vi或者vim编写脚本内容并命名为：mysql\_dump\_script.sh

#!/bin/bash#保存备份个数，备份31天数据number=31#备份保存路径backup\_dir=/root/mysqlbackup#日期dd=`date +%Y-%m-%d-%H-%M-%S`#备份工具tool=mysqldump#用户名username=root#密码password=TankB214#将要备份的数据库database\_name=edoctor#如果文件夹不存在则创建if [ ! -d
KaTeX parse error: Can't use function '\]' in math mode at position 13: backup\\_dir \̲]̲;then         m…
backup\_dir;fi#简单写法 mysqldump -u root -p123456 users > /root/mysqlbackup/users-
...
backup\_dir/log.txt#找出需要删除的备份delfile=`ls -l -crt $backup\_dir/\*.sql | awk '{print
KaTeX parse error: Expected 'EOF', got '}' at position 3: 9 }̲' | head -1\`#判…
numbercount=`ls -l -crt $backup\_dir/\*.sql | awk '{print
...
0 2 \* \* \* /root/mysql\_backup\_script.sh
