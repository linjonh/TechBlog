...
pycryptodomex 是 python 的一个加密库，可以通过用 pip 安装 whl 文件， 也可以通过源码进行编译安装， SDK tools/env/local\_pkgs/ 中内置了 pycryptodomex 源码
...
直接双击 zx-rtt/win\_env.bat 打开专有的 Windows 的 env 命令行工具，后面所有命令都在该命令行工具中进行操作
...
直接双击 zx-rtt/win\_cmd.bat 打开 Windows 的 CMD 命令行工具，后面所有命令的使用和 ENV 相同
...
编译后的镜像名称为 m3\_ZX7D0N\_page\_2k\_block\_128k\_v1.0.0.img
...
WINDOW 环境下解压 PanelLanDownloader.zip 后, 进入 PanelLan\_Device\_drive 目录, 双击 InstallDriver.exe 根据向导指引即可完成安装
