...
执行下面的SOL语句,创建表空间ustb\_ts和数据库ustbdb:
...
然后在working窗口下,执行下面的语句,修改数据库studentdb的默认表空间为app\_ts:
...
输出显示,更改数据库studentdb的默认表空间为app\_ts的语句没有执行成功。可见,必须关
...
可以看到,目前数据库studentdb的默认表空间是app\_ts了。
...
执行下面的SQL语句，创建数据库用户temp\_user，并授予用户temp\_user SYSADMIN权限:
...
执行下面的SQL语句,将数据库ustbdb的拥有者变更为数据库用户temp\_user:
...
从输出可以看到,数据库ustbdb的属主用户已经变更为用户temp\_user了。执行下面的SQL语句,
...
继续后面的任务之前,执行下面的SOL语句,删除用户temp\_user、数据库ustbdb、表空间ustb\_ts,
