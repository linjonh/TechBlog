...
2、熟悉 Linux TFTP 配置，为后续 Linux 底层开发做准备（后面会用 tftp 从宿主机传输镜像到 FS\_4412开发板）；
...
/source/rootfs \*(rw,sync,no\_root\_squash,no\_subtree\_check)
...
其中：/source/rootfs 是要共享的目录，\*代表允许所有的网络段访问，rw 是可读写权限，sync 是资料同步写入内存和硬盘，no\_root\_squash 是 NFS 客户端分享目录使用者的权限，如果客户端使用的是 root 用户，那么对于该共享目录而言，该客户端就具有 root 权限。
