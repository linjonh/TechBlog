...
* Directory:
  ...
  Dictionary在pycharm中就是一个文件夹，放置资源文件等，该文件夹其中并不包含\_ \_ init.py\_ \_文件
* Python Package:
    
  对于Python package 文件夹而言，与Dictionary不同之处在于其会自动创建\_ \_ init.py\_
  *文件。
    
  简单的说，python package就是一个目录，其中包括一组模块和一个*
  \_ init.py\_ \_文件。目录下具有init.py文件，这样可以通过from…import的方式进行.py文件的导入。
