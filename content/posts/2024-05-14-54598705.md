...
这里需要注意的是，一定要等编码器设置编码格式完成后，再将它加入到混合器中，编码器编码格式设置完成的标志是dequeueOutputBuffer得到返回值为MediaCodec.INFO\_OUTPUT\_FORMAT\_CHANGED。
...
2. 颜色空间。按照Android本身的意思，COLOR\_FormatYUV420Planar应该是所有硬件平台都支持的。但是实际上并不是这样。所以在设置颜色空间时，应该获取硬件平台所支持的颜色空间，确保它是支持你打算使用的颜色空间，不支持的话应该启用备用方案（使用其他当前硬件支持的颜色空间）。
...
6. 录制结束时，应该发送结束标志MediaCodec.BUFFER\_FLAG\_END\_OF\_STREAM，在编码后区获得这个标志时再终止循环，而不是直接终止循环。
