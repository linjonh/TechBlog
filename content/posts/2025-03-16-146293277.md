...
print(show_test_data)</code></pre>
    ...
    <p>
     <a class="has-card" href="https://data.baai.ac.cn/data" rel="nofollow" title="Data Hub">
     ...
     <span style="color:#4da8ee">
      注意：需要确保一下电脑有那么多内存，一个数据集几百个G；
     </span>
    </p>
    <pre><code>// 展示一下所下载的数据集
import json
import os
...
for filename in json_files:
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        # 打印正在处理的文件名
        print(json.dumps(data, indent=4, ensure_ascii=False))</code></pre>
    <h4>
     二、部署预训练模型
    </h4>
    <p>
     ...
     <a class="has-card" href="https://ollama.com/" rel="nofollow" title="Ollama">
      <span class="link-card-box" contenteditable="false">
       <span class="link-title">
        Ollama
       </span>
    ...
    <pre><code>// 命令行窗口, 系统的哈（win + R, cmd），不是pycharm等里面的终端
...
print(final_output)
    ...
    <p>
     数据预处理，主要是将结构化的数据tokenize一下，并且对数据进行填充或者截断，这步主要是确保数据的大小与模型的要求相匹配；tokenize并不仅仅是将词分解成一个token那么大，而且它还将token大小的词转化成了
     <span style="color:#4da8ee">
      <strong>
       数字
      </strong>
    ...
    <pre><code>// 这种是Hugging Face的，估计有可能用不了
...
// 经查阅，deepseek-r1:1.5b对应的分词器为LlamaTokenizerFast
    ...
    <p>
     <img alt="" height="491" src="https://i-blog.csdnimg.cn/direct/83d1a667cdc74c0ebc0263958a701173.png" width="2363"/>
    </p>
    <p>
     <img alt="" height="187" src="https://i-blog.csdnimg.cn/direct/78575908f9f14723a7c12d795d71ae8b.png" width="1669"/>
    </p>
    <p>
     目前，据我了解和transform不匹配（调用会报找不到模型文件的错误，
...
tokenizer = AutoTokenizer.from_pretrained(model_dir)</code></pre>
    ...
    <blockquote>
     ...
     <p style="text-align:center">
      <img alt="W=W_0 +\left ( A * B \right )" class="mathcode" src="https://latex.csdn.net/eq?W%3DW_0%20&amp;plus;%5Cleft%20%28%20A%20*%20B%20%5Cright%20%29"/>
     </p>
     <p>
      其中，
      <img alt="W" class="mathcode" src="https://latex.csdn.net/eq?W">
       是现在的权重，
       <img alt="W_0" class="mathcode" src="https://latex.csdn.net/eq?W_0">
        是原始的权重，
 ...
 <p alt="68747470733a2f2f626c:6f672e6373646e2e6e65742f7368616767793132333435362f:61727469636c652f64657461696c732f313436323933323737" class_="artid" style="display:none">
 </p>
</div>
