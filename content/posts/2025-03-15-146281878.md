...
编译器如何在 Point3d 和 Vertex 的 copy assignment operator 压制 Point 的 copy assignment operator呢？constructor 中的解决办法是添加一个额外的参数 \_\_most\_derived。解决办法是：为copy assignment operator 提供分发函数(split functions)以支持这个 class 称为 most-derived class或者成为中间的 base class。
