...
Py\_NoSiteFlag = 1;    //避免出现“'import site' failed”的错误
...
pModule = PyImport\_ImportModule("google");    //调用的文件名
  
pFunc= PyObject\_GetAttrString(pModule, "main");    //调用的函数名
  
  
PyObject \*arg;
  
arg = Py\_BuildValue("(s)", "look");    //构造参数，关键字为“look”
  
PyEval\_CallObject(pFunc, arg);    //调用函数
