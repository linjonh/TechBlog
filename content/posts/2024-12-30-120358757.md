...
layout: post
title: "大数据架构简介"
date: 2024-12-30 21:37:28 +0800
description: "大数据技术其实是分布式技术在数据处理领域的创新性应用，其本质和此前"
keywords: "大数据的方法和架构"
categories: ['大数据']
tags: ['大数据', 'Spark', 'Mapreduce']
artid: "120358757"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=120358757
    alt: "大数据架构简介"
render_with_liquid: false
featuredImage: https://bing.ee123.net/img/rand?artid=120358757
featuredImagePreview: https://bing.ee123.net/img/rand?artid=120358757
---
  ...
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <p>
     ...
     这些数据，既包括数据库的数据，也包括日志数据，还包括专门采集的用户行为数据；既包括企业内部自己产生的数据，也包括从第三方采购的数据，还包括使用网络爬虫获取的各种互联网公开数据。
    </p>
    <p>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/227faed535cf92e6b1e5107242f6436e.png"/>
    </p>
    <p>
     ▲图31-1 HDFS架构
    </p>
    ...
    <p>
     <strong>
      大数据处理的经典计算框架是MapReduce。
     </strong>
     MapReduce的核心思想是对数据进行分片计算。既然数据是以块为单位分布存储在很多台服务器组成的集群上的，那么能不能就在这些服务器上针对每个数据块进行分布式计算呢？
    </p>
    <p>
    </p>
    <p>
    ...
    <ul>
     <li>
      <p>
       ...
       ，每个服务器上会启动多个map进程，map优先读取本地数据进行计算，计算后输出一个&lt;key, value&gt;集合；
      </p>
     </li>
     <li>
      <p>
       ...
       ，MapReduce在每个服务器上都会启动多个reduce进程，然后对所有map输出的&lt;key, value&gt;集合进行shuffle操作。所谓的shuffle就是将相同的key发送到同一个reduce进程中，在reduce中完成数据关联计算。
      </p>
     </li>
    </ul>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/a72773fbea5ee19ea325644d64756438.png"/>
    </p>
    <p>
...
    }
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/fc8e768992fdc09bad95e7b6bbc08237.png"/>
    </p>
    <p>
    ...
    <p>
     ...
     这两种进程角色，JobTracker在MapReduce集群中只有一个，而TaskTracker则和DataNode一起启动在集群的所有服务器上。
    </p>
    <p>
    </p>
    <p>
    ...
    <p>
     传统上，主要使用SQL进行数据分析，
     <strong>
      如果能根据SQL自动生成MapReduce，就可以极大降低大数据技术在数据分析领域的应用门槛。
     </strong>
    </p>
    <p>
    </p>
    ...
    <pre><code>SELECT pageid, age, count(1) FROM pv_users GROUP BY pageid, age;</code></pre>
    <p>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/2e00576626fe57193a3d4fc5eb6ae1fd.png"/>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/9ca04ef545140e4822a93cd3a9e5ff58.png"/>
    </p>
    <p>
    ...
    <p>
     <strong>
      Hive要做的就是将SQL翻译成MapReduce程序代码。
     </strong>
     实际上，Hive内置了很多Operator，每个Operator完成一个特定的计算过程，Hive将这些Operator构造成一个有向无环图DAG，然后根据这些Operator之间是否存在shuffle将其封装到map或者reduce函数中，之后就可以提交给MapReduce执行了。
    </p>
    <p>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/ed7ba529ba50ba806a2fe9e29a9a3a11.png"/>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/68b904c2a17f5742cb25de2b04d58b2b.png"/>
    </p>
    <p>
     ▲图31-7 Hive整体架构
    </p>
    ...
    <p>
     Spark在MapReduce的基础上进行了改进，它主要使用内存进行中间计算数据存储，加快了计算执行时间，
     <strong>
      在某些情况下性能可以提升上百倍。
     </strong>
    </p>
    ...
    <p>
     ...
     ，即弹性数据集。在RDD上定义了许多常见的大数据计算函数，利用这些函数可以用极少的代码完成较为复杂的大数据计算。前面举例的WorkCount如果用Spark编程，只需要三行代码：
    </p>
    <p>
    </p>
...
counts.saveAsTextFile("hdfs://...")</code></pre>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/0aaf3abe60b0765599a368c4a6725621.png"/>
    </p>
    <p>
    ...
    <p>
     Spark虽然比MapReduce快很多，但是在大多数场景下计算耗时依然是分钟级别的，这种计算一般被称为大数据批处理计算。而在实际应用中，有些时候需要在
     <strong>
      毫秒级
     </strong>
     完成不断输入的海量数据的计算处理，比如实时对摄像头采集的数据进行监控分析，这就是所谓的大数据流计算。
    </p>
    <p>
    </p>
    <p>
    ...
    <p style="text-align:center;">
     <img alt="图片" src="https://i-blog.csdnimg.cn/blog_migrate/d7d798fd0c1161516c0e3fb27d3c9f00.png"/>
    </p>
    <p>
 ...
 <p alt="68747470733a2f:2f626c6f672e6373646e2e6e65742f6a635f62656e62656e2f:61727469636c652f64657461696c732f313230333538373537" class_="artid" style="display:none">
 </p>
</div>
