...
安装结束后，提示用root用户，执行脚本: /opt/db/dm/dmdbms/script/root/root\_installer.sh
...
注册实例服务的脚本: 安装目录/script/root/dm\_service\_installer.sh，必须切换到root用户执行。
...
disql工具在安装目录的bin目录下，可以把bin目录添加到环境变量PATH中，dmdba用户下查看配置文件： ~/.bash\_profile
