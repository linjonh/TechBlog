...
二者 比vector 多出来的 常用方法: push\_front, pop\_front
...
### 5.详解容器适配器–stack, queue, priority\_queue
...
1. 代码:
   ...
   ```
2. 为什么有的依赖 deque(queue, strack), 有的依赖 vector(priority\_queue)
...
1. iterator 是 普通的 正向迭代器 ---- 不仅能读,还能修改
   ...
   ```
2. const\_iterator常量的正向迭代器 ----- 只能读, 不能写
   ...
   ```
3. 为什么 iterator转化为const\_iterator是对的?
   ...
   ```
4. reverse\_iterator 反向迭代器,
   **搭配rbegin()**
     
   同样有 const\_reverse\_iterator
...
5. **函数对象好处**
   ...
   ```
6. priority\_queue默认是大根堆, 即从大到小排列, 改为小根堆
