---
layout: post
title: "FFmpeg音视频简单合并"
date: 2024-12-13 16:11:00 +0800
description: "一、将原有视频的音频消除ffmpeg.exe -i video原版.mp4 -vcodec copy"
keywords: "ffmpeg audio video 合成"
categories: ['Ffmpeg']
tags: ['无标签']
artid: "104008263"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=104008263
    alt: "FFmpeg音视频简单合并"
render_with_liquid: false
featuredImage: https://bing.ee123.net/img/rand?artid=104008263
featuredImagePreview: https://bing.ee123.net/img/rand?artid=104008263
---

<div class="blog-content-box">
 <div class="article-header-box">
  <div class="article-header">
   <div class="article-title-box">
    <h1 class="title-article" id="articleContentId">
     FFmpeg音视频简单合并
    </h1>
   </div>
  </div>
 </div>
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   <link href="../../assets/css/kdoc_html_views-1a98987dfd.css" rel="stylesheet"/>
   <link href="../../assets/css/ck_htmledit_views-704d5b9767.css" rel="stylesheet"/>
   <div class="htmledit_views" id="content_views">
    <h4>
     一、将原有视频的音频消除
    </h4>
    <p>
     <br/>
     <strong>
      ffmpeg.exe  -i  video原版.mp4  -vcodec  copy  -an  video去音频版.mp4
     </strong>
    </p>
    <h4>
     <br/>
     二、加入新的音频
    </h4>
    <p>
     <br/>
     <strong>
      ffmpeg.exe  -i  video去音频版.mp4  -i  audio原版.mp3  -vcodec  copy  -acodec  copy  音视频合并版.mp4
     </strong>
    </p>
    <p>
    </p>
    <p>
     <strong>
      注：
     </strong>
    </p>
    <table border="1" cellpadding="1" cellspacing="1" style="width:500px;">
     <tbody>
      <tr>
       <td style="width:84px;">
        -i
       </td>
       <td style="width:414px;">
        输入您要处理的视频文件路径
       </td>
      </tr>
      <tr>
       <td style="width:84px;">
        -an
       </td>
       <td style="width:414px;">
        不使用音频纪录
       </td>
      </tr>
      <tr>
       <td style="width:84px;">
        -vcodec
       </td>
       <td style="width:414px;">
        视频强制使用codec编解码方式。如果用copy表示原始编解码数据必须被拷贝。
       </td>
      </tr>
      <tr>
       <td style="width:84px;">
        -acodec
       </td>
       <td style="width:414px;">
        音频强制使用codec编解码方式。如果用copy表示原始编解码数据必须被拷贝。
       </td>
      </tr>
     </tbody>
    </table>
    <p>
    </p>
   </div>
  </div>
 </article>
 <p alt="68747470733a2f2f:626c6f672e6373646e2e6e65742f753031323732353632332f:61727469636c652f64657461696c732f313034303038323633" class_="artid" style="display:none">
 </p>
</div>


