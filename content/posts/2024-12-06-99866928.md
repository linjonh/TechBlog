...
# java.lang.OutOfMemoryError: pthread\_create (1040KB stack) failed: Try again

![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/612b2e69d806b02ccb278f60db3c5ff1.jpeg)

#### 文章目录

* [java.lang.OutOfMemoryError: pthread\_create (1040KB stack) failed: Try again异常分析及解决](#javalangOutOfMemoryError_pthread_create_1040KB_stack_failed_Try_again_2)
* + [问题描述：](#_3)
  + [问题分析：](#_105)
  + [代码定位思路：](#_109)
  + [行动：](#_112)

## java.lang.OutOfMemoryError: pthread\_create (1040KB stack) failed: Try again异常分析及解决
...
从log中可以看到java.lang.OutOfMemoryError: pthread\_create (1040KB stack) failed: Try again 内存溢出的运行时异常，但是可以肯定的是app没有内存过高，很不解，再看后面的pthread\_create failed可知是在创建线程时报的内存溢出，创建线程怎么会造成内存溢出呢？第一次遇到 只能先找度娘求救了，找到一篇
...
测试发现CPU\_COUNT 为1 ，最大容纳线程数为3，但是 threadMap.entrySet().size()为70.。。。。。一脸蒙蔽。
