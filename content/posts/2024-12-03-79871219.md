...
3. 如果slave\_IO\_running 为connecting 检查IP是否正确，用户名密码是否正确
4. 如果slave\_IO\_running 为no，检查数据库版本号，看错误日志
5. 如果slave\_SQL\_Running 为no，检查时候进行了写操作，看错误日志
6. SET GLOBAL log\_bin\_trust\_function\_creators = 1;