...
![](https://i-blog.csdnimg.cn/img\_convert/d0dfa7ef715e2539c21c3b9fdb35cb28.png)
###
### 1\. 颜色是什么
一束光中由不同波长的电磁波组成(光谱)，而自然界存在各种各样的光谱(定义为集合Q)。
![](https://i-blog.csdnimg.cn/img\_convert/43536acac0652a5d4b94caeededff487.png)
自然界中的光谱
人的眼睛中有三种视锥细胞（L，S，M），这三种视锥细胞对不同波长光的响应各不相同。
![](https://i-blog.csdnimg.cn/img\_convert/75ee3cdd37f456493d977784b9551111.png)
添加图片注释，不超过 140 字（可选）
人类通过视锥细胞响应比例的差异来识别不同光谱的"颜色"（集合S）。
![](https://i-blog.csdnimg.cn/img\_convert/f91c959692e7424aad535293c3c21fd7.png)
添加图片注释，不超过 140 字（可选）
但是不同光谱到LSM响应的映射是个多对一的过程(Q->S是满射非单射)，换言之存在不同的光谱产生一样颜色的情况(异谱同色)。
![](https://i-blog.csdnimg.cn/img\_convert/45ac90c599221fa015925a004a69660a.png)
添加图片注释，不超过 140 字（可选）
因此颜色可以说是人眼中LMS的一个响应组合。（想象一下三维空间中每个点是一组LMS的响应组合代表一种颜色）
![](https://i-blog.csdnimg.cn/img\_convert/ba98155bf3cab01c59604c22ac301d56.png)
###
### 2\. 颜色的三刺激值表示
...
![](https://i-blog.csdnimg.cn/img\_convert/074bffb1925fcc43706d0ccd41fc297c.png)
添加图片注释，不超过 140 字（可选）
忽略亮度因素，得到CIE XYZ色度图，每个颜色都可以用一个xyz刺激值来表示，色度图包含了人类所有可见颜色以及其对应的刺激值。
![](https://i-blog.csdnimg.cn/img\_convert/731f5a54bd665bab565a5a3ee4612fa1.png)
因此颜也可以使用XYZ刺激值来表示。（想象一下三维空间中的每个点是XYZ的组合代表一个颜色，它跟上面的LMS空间存在线性对应关系）
###
### 3\. 相机眼中的颜色
相机在不同的感光元件上放置不同的滤光片来实现sensor对光谱差异响应，因此相机眼中的颜色是一个RGB的响应组合。
![](https://i-blog.csdnimg.cn/img\_convert/85ccd8c353f55bb1b6667b8c825f571b.png)
相机响应曲线
相机RGB响应跟人眼的LMS响应往往存在较大的区别，为了使其跟人眼中的颜色对应起来，需要建立一个相机RGB响应值到CIE
XYZ刺激值的映射；这个过程涉及到相机颜色矫正和标定过程，且存在非线性转换关系（参见卢瑟条件）。
![](https://i-blog.csdnimg.cn/img\_convert/89d3e5e348ef3c7d418b1f451fca6bca.png)
相机RGB与色彩校正后RGB
###
### 4\. 显色呈现和还原
正如秦统一文字，CIE
...
![](https://i-blog.csdnimg.cn/img\_convert/179b177111a7c52de2a9589d99af189b.png)
至此一束光的"颜色"从记录再到呈现给人类的过程就结束了。
###
\*\*杂谈：\*\*
其实人类对颜色的识别是一个大脑高度参与的过程，大脑可能会帮你自动做白平衡操作，例如黑金/蓝白裙子心理实验。
![](https://i-blog.csdnimg.cn/img\_convert/896c38ceeb9791835e76871f6aa847c3.png)
参考资料：
...
Wikipedia](https://en.wikipedia.org/wiki/CIE\_1931\_color\_space "CIE 1931 color
space - Wikipedia")
