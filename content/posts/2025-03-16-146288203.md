...
> <div>, <p>, <h1>~<h6>, <ul>, <ol>, <li>, <table>, <form>, <header>,
> <footer>, <nav>, <article>, <section>等。
...
**效果** ：两个元素分别独占一行，且`div`的宽度为200px，`p`默认宽度为父容器100%。

![](https://i-blog.csdnimg.cn/direct/c7432fa2ff8047c9935d789ebc063737.png)
...
![](https://i-blog.csdnimg.cn/direct/7f1c9786c3104ebe872ec7e5988aa788.png)
...
![](https://i-blog.csdnimg.cn/direct/3ea658674fbd4ff6855742b127e5715d.png)
...
![](https://i-blog.csdnimg.cn/direct/80111777fef046149d130812b73e11dd.png)
...
### **1.定义与特点**
  ...
  2. **无法设置宽高** ：直接设置`width`和`height`无效，元素宽度由内容决定。
...
![](https://i-blog.csdnimg.cn/direct/81f8de699b894cfca45f87f075e103b9.png)
...
行内块元素兼具块元素和行内元素的特点：
  ...
  2. **可设置宽高** ：可以自由设置`width`、`height`和`margin`、`padding`。
...
![](https://i-blog.csdnimg.cn/direct/a375c47da0fe474a8cbc9026a7986a66.png)
...
**常见标签**| `<div>`, `<p>`, `<h1>`| `<span>`, `<a>`, `<em>`| `<img>`, `<input>`,
`<button>`  
...
## **五、关键注意事项**
  ...
  2. **行内元素的空白间隙** ：
     ...
     * 行内元素或行内块元素（如`<img>`）之间默认会因HTML中的换行或空格产生微小间隙，可通过以下方式解决： 
       * 使用`font-size: 0`在父元素上消除空白。
       * 将HTML标签写在一行上，避免换行。
       * 使用`vertical-align: top`调整对齐方式。
  3. **display属性转换** ：
...
## **七、常见问题解答**
  ...
  1. **为什么行内元素的`margin-top`和`margin-bottom`无效？**

     * 行内元素的高度由内容决定，垂直方向的外边距会被忽略，但水平方向的`margin-left`和`margin-right`有效。
  2. **如何让行内元素垂直居中？**

     * 使用`vertical-align: middle`（需与其他行内元素配合）或转换为块元素后用`margin: auto`。
  3. **如何让块元素水平居中？**

     * 设置`margin: 0 auto`（需指定宽度）。
