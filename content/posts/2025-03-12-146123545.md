...
### void Led\_Disp(unsigned char ucLed);
...
### void Seg\_Tran(unsigned char \*pucSeg\_Buf,unsigned char \*pucSeg\_Code);
...
### void Seg\_Disp(unsigned char \*pucSeg\_Code,unsigned char ucSeg\_Pos)；
...
### unsigned char Key\_Read\_KBD(void)；
...
3. 依次将每一列设置为低电平，读取P3的
   ...
   不
   **要忘记每个都左移4位,然后记得|**
4. 用Switch语句将按键按下后的值进行判断（Key\_New取反 便于理解）
5. 返回对应按键的值
6. 不要忘记default 返回都没有按下的值 设为0

### 5.编写ADC代码

### unsigned char PCF8591\_ADC(void)；
...
### void EEPROM\_Read(unsigned char \*pucBuf,unsigned char addr,unsigned char num)；
...
### void EEPROM\_Write(unsigned char \*pucBuf,unsigned char addr,unsigned char num)
