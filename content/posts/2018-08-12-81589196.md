...
另外，traceback.print\_exc()跟traceback.format\_exc()有什么区别呢？

区别就是，format\_exc()返回字符串，print\_exc()则直接给打印出来。即traceback.print\_exc()与print(traceback.format\_exc())效果是一样的。print\_exc()还可以接受file参数直接写入到一个文件。比如可以像下面这样把相关信息写入到tb.txt文件去。
