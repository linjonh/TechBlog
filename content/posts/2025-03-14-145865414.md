...
* 思路：我们通过模拟栈遍历字符串 s，逐个字符处理。当遇到当前字符和 ret（结果字符串）末尾的字符相同时，说明这两个字符相同且相邻，因此可以将末尾的字符移除（使用 pop\_back()），实现删除重复字符的效果。否则，将当前字符添加到 ret 的末尾。最终返回 ret 作为结果。这个方法有效地将相邻的重复字符一一删除，直到整个字符串处理完成。
