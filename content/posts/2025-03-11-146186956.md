...
• 文件路径由`PASSWORD\_FILE`参数控制，默认路径为`$YASDB\_DATA/instance/yasdb.pwd`。
...
• 参数文件名为`yasdb.ini`，为文本文件，存放在`$YASDB\_DATA/config`路径下。

• 虽然可以直接编辑，但不建议直接修改，推荐使用SQL命令修改参数。

### 4. 修改参数配置

• 示例：修改`data\_buffer\_size`参数
...
• 可以通过`V$DATABASE`视图中的`log\_mode`字段查看当前归档模式配置。
...
• 注意：从归档模式调整到非归档模式时，需要设置`replication\_addr`为空。
