...
该类断言用于直接标记是否成功或者失败。可以使用SUCCEED()宏标记成功，使用FAIL()宏标记致命错误（同ASSERT\_
*)，ADD\_FAILURE()宏标记非致命错误（同EXPECT\_*
）。举个例子
...
3. 第3步：在main（）中调用RUN\_ALL\_TESTS（）
...
从上面的框框中的案例名称大概能够看出案例的命名规则: prefix/test\_case\_name.test.name/index
