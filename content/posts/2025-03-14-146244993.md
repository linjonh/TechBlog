...
# 多线程程序的测试和调试\_第11章\_《C++并发编程实战》笔记
...
3. 使用 std::adopt\_lock： 手动锁定互斥量，然后使用 std::lock\_guard 结合 std::adopt\_lock 接管已锁定的互斥量，确保在作用域结束时自动解锁。同时保持锁的获取顺序一致，避免死锁。
...
* Clang线程安全注解：标记变量为GUARDED\_BY(mutex)。
...
6. B、D
   ...
   D正确（原子操作若内存序选择错误，如memory\_order\_relaxed可能导致其他线程看到不一致的状态）。
