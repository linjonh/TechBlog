...
其中:udp\_socket\_wrapper.h主要负责Socket相关操作，如Socket创建、启动、端口绑定、停止。

udp\_socket2\_windows.h主要负责windows平台上的Socket相关操作，与之对应的就是linux平台上的udp\_socket\_posix.h

udp\_transport.h主要负责包的发送和接收，如果你想实现自己的数据包收发逻辑，可重写该类，如他里面的LoopBack方式就是通过重写该模块来实现的。
