...
client端的字符集;三是dmp文件的字符集。在做数据导入的时候，需要这三个字符集都一致才能正确导入。开始—运行—sqlplus，用户名输入：system
as sysdba
...
这个比较简单。在windows平台下，就是注册表里面HKEY_LOCAL_MACHINE\SOFTWARE\ORACLE\HOME0\NLS_LANG
。还可以在dos窗口里面自己设置，比如:
...
b)数据文件是什么编码，就将客户端设置为什么编码（例如NLS_LANG=AMERICAN _ AMERICA.ZHS16GBK
在注册表中修改），然后用工具导入数据。
