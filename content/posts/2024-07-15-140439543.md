...
先下载MQTT.fx客户端
[mqttfx-1.7.1-windows-x64.exe\_免费高速下载|百度网盘-分享无限制 (baidu.com)](https://pan.baidu.com/s/13v0ROf1dOzu2tXqmw3iBMg "mqttfx-1.7.1-windows-x64.exe_免费高速下载|百度网盘-分享无限制 (baidu.com)")
...
其实也不需要工具也可以产生password，阿里云在点击设备的时候会有一个MQTT连接参数，里面有我们需要的password和client\_id，但是复制的client\_id需要考虑securemode。
...
如果正常控制，说明协议通道已经搭建好了。但是当我们使用mqttfx客户端模拟去控制esp8266时，我们向主题发送控制信息，esp8266却没有任何反应。因为两个人发布和订阅的主题虽然都是setled，但是前面的device\_name确是不一致的，这样订阅的主题不对当然获取不到有用数据。我们需要使用新的功能
**消息转发的云产品流转。**
