...
print 'Search Params:\n', json.dumps(params, ensure\_ascii=False)
  
print 'Search Response:\n', json.dumps(r.json(), ensure\_ascii=False, indent=4)

在实际编写自动化测试脚本时，我们需要进行一些封装。如下代码中我们对豆瓣电影搜索接口进行了封装，test\_q方法只需使用nosetests提供的yield方法即可很方便的循环执行列表qs中每一个测试集：
...
msg\_html = MIMEText(content, 'html', 'utf-8')
  
msg.attach(msg\_html)

# 添加附件
  
msg\_attachment = MIMEText(content, 'html', 'utf-8')
  
msg\_attachment["Content-Disposition"] = 'attachment; filename="{0}"'.format(report\_file)
  
msg.attach(msg\_attachment)

msg['Subject'] = mail\_subjet
  
msg['From'] = mail\_user
  
msg['To'] = ';'.join(mail\_to)
  
try:
  
# 连接邮件服务器
  
s = smtplib.SMTP(mail\_host, 25)
  
# 登陆
  
s.login(mail\_user, mail\_pwd)
  
# 发送邮件
  
s.sendmail(mail\_user, mail\_to, msg.as\_string())
