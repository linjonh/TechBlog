...
其中，$C$ 是配置管理仓库中的配置和软件包数量，$n$ 是仓库中的配置和软件包版本数量，$V\_i$ 是每个配置和软件包的版本号。
...
其中，$P$ 是系统性能指标的总和，$m$ 是系统性能指标的数量，$W\_i$ 是每个指标的权重。
...
其中，$D$ 是自动化部署过程中的任务数量，$n$ 是任务数量，$T\_i$ 是每个任务的时间。
...
其中，$R$ 是自动化回滚过程中的失效数量，$n$ 是失效数量，$F\_i$ 是每个失效的时间。
...
其中，$G$ 是自动化报告过程中的报告数量，$m$ 是报告数量，$L\_i$ 是每个报告的长度。
...
$ curl -u admin:changeme -d '{"index": "main", "time\_range": {"start": "2021-01-01T00:00:00Z", "end": "2021-01-31T23:59:59Z"}, "search": "index=main | stats count by sourcetype"}' http://localhost:8000/services/search/run
