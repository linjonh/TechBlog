...
本次学习，借助的平台是PyCharm 2024.1.3，python版本3.11
numpy版本是1.26.4，pytorch版本2.0.0+cu118，d2l的版本是1.0.3
...
如前几回一样，本次仍然采用CIFAR10数据集，因此不做重点解释（有兴趣的可以查看[第六回](https://blog.csdn.net/zhuwenlong130130/article/details/145896185?spm=1001.2014.3001.5501)内容），本回只展示代码：
...
注:  
在本回设计的模型中，数据最小为96 *
96。有兴趣的可以参考[第八回小记](https://blog.csdn.net/zhuwenlong130130/article/details/146085020?spm=1001.2014.3001.5501)自己进行计算。
...
如代所示，码残差块中定义了两个3 _3的卷积，每个卷积层后面接了一个规范化层和一个Relu激活函数（体现在传播函数中）。self.conv3中定义了一个1_
...
注：  
由于本回采用d2l.train_ch6()进行数据训练，里面集成了损失函数和优化器，因此不需要单独定义（在[第八回小记](https://blog.csdn.net/zhuwenlong130130/article/details/146085020?spm=1001.2014.3001.5501)中介绍了如何安装d2l库）。

## 模型训练

### 1、实例化ResNet模型

这里输入为3个通道，因为彩色图片有三个数据通道。输出为10，因为数据集有10个类别（数据集的介绍，在[第六回](https://blog.csdn.net/zhuwenlong130130/article/details/145896185?spm=1001.2014.3001.5501)中）。
...
输出含有训练集精度、测试集精度和消耗的时间等（这里我进对源码进行了修改，可以查看[第八回小记](https://blog.csdn.net/zhuwenlong130130/article/details/146085020?spm=1001.2014.3001.5501)）。
