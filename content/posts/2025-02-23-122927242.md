...
今天在写公司项目出了个问题，由于应该前后端没有分离的项目，所以前端也自己写了，今天将前端传一个字符串参数的时候 一段字符串中间包含这样的参数 DATE\_FORMAT( CURDATE(), '%Y-%m-%d')，然后发现后端接收参数为null了，原因是因为%Y，服务端解析url编码会出现问题，需要将%Y改成%25Y就可以了，有大佬知道这个问题吗