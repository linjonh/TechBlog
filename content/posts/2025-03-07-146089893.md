...
* 构建时Next.js会读取该文件（变量需以 NEXT\_PUBLIC\_ 前缀暴露到客户端）。
...
- PROJECT\_NAME : 项目名称（用于PM2和Nginx配置
  
- PROJECT\_PORT : Next.js服务端口（默认3000）
  
- PROJECT\_GIT\_REPO : 你的Git仓库地址（确保服务器有访问权限 ）
  
- PROJECT\_DIR : 项目部署目录（默认/var/www/my-next-app ）
