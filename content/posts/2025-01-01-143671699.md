...
featuredImage: https://bing.ee123.net/img/rand?artid=143671699
featuredImagePreview: https://bing.ee123.net/img/rand?artid=143671699
---
...
：在满足 1NF 的基础上，确保表中的非主属性完全依赖于主键，而不是依赖于主键的一部分（候选键）。
</li>
<li>
<strong>
第三范式（3NF）
</strong>
：在满足 2NF 的基础上，确保非主属性不依赖于其他非主属性（消除传递依赖）。
</li>
</ul>
<p>
...
<pre><code class="language-java">// 创建一个 SQLiteOpenHelper 实例
...
：SQLite 能够在多种操作系统上运行，包括 Windows、Linux、Mac OS X、Android 和 iOS 等，这为跨平台嵌入式应用提供了便利。
</li>
<li>
<strong>
易于集成
</strong>
：SQLite 的代码库小巧，易于集成到应用程序中，开发者不需要配置复杂的数据库服务器，可以直接在应用中嵌入 SQLite。
</li>
<li>
<strong>
无需管理
</strong>
：由于 SQLite 将所有数据存储在单一的磁盘文件中，所以没有复杂的数据管理任务，例如备份、恢复或数据库维护，减少了系统管理员的工作负担。
</li>
<li>
<strong>
良好的性能
</strong>
：对于小型到中型的数据集，SQLite 提供了出色的性能，足以满足许多嵌入式系统的性能需求。
</li>
</ul>
<p>
...
<pre><code class="language-sql">-- 示例 SQL 查询语句
...
<pre><code class="language-sql">-- 示例创建索引的 SQL 语句
