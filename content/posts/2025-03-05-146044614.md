...
**补充**
：有两个CHDWIFI，只有连接上才能设置自动加入，CHD-WIFI(一般宿舍是这个)和CHD\_WIFI(一般是教室，食堂)，两个WIFI都这样操作一下。（可以先操作一个，另一个随时补上，这一步主要是用来自动接入WIFI，甚至可以不操作，手动接入WIFI时，也能触发脚本进行登陆认证）
...
2.选取CHD-WIFI和CHD\_WIFI（不管上一步设置了哪一个的自动加入，这一步都选两个，因为这个是触发条件，和上面关系不大）
