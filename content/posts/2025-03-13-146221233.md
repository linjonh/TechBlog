 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="markdown_views prism-atom-one-light" id="content_views">
    ...
    <pre><code>sudo apt-get install git curl cmake xz-utils m4 flex bison python3 libtbb-dev g++-multilib
</code></pre>
    <h3>
     <a id="LLVM_11">
     </a>
    ...
    <pre><code class="prism language-bash">cmake <span class="token parameter variable">-S</span> llvm <span class="token parameter variable">-B</span> build <span class="token parameter variable">-G</span> <span class="token string">'Unix Makefiles'</span> <span class="token parameter variable">-DLLVM_ENABLE_PROJECTS</span><span class="token operator">=</span><span class="token string">"clang;lld"</span> <span class="token parameter variable">-DCMAKE_BUILD_TYPE</span><span class="token operator">=</span>Release
</code></pre>
    <p>
     这里使用全部的线程构建：
    </p>
    ...
    <pre><code class="prism language-bash">cmake <span class="token parameter variable">--build</span> <span class="token builtin class-name">.</span> <span class="token parameter variable">--target</span> <span class="token function">install</span>
</code></pre>
    <p>
     不过没有复制
     <code>
    ...
    <pre><code class="prism language-bash"><span class="token function">sudo</span> <span class="token function">cp</span> FileCheck /usr/local/bin/FileCheck
</code></pre>
    <p>
     安装完之后看看
     <code>
...
21.0.0git
    ...
    <pre><code>git clone https://github.com/ispc/ispc.git
</code></pre>
    <blockquote>
     <p>
    ...
    <pre><code>cmake -B build-ispc ispc
</code></pre>
    <p>
     然后开始构建（不要用官方文档里的
     <code>
...
make -j$(nproc)
    ...
    <pre><code>export PATH="$PATH:&lt;自己改目录&gt;/build-ispc/bin"
</code></pre>
    <blockquote>
     <p>
      关于这部分如果你不熟悉，那么可以看看
      <a href="https://blog.csdn.net/qq_33919450/article/details/122076994">
       macOS在终端上如何直接使用脚本或者下载的程序 - ZhongUncle CSDN
      </a>
      。
     </p>
    </blockquote>
...
Intel(r) Implicit SPMD Program Compiler (Intel(r) ISPC), 1.27.0dev (build commit f988909621671856 @ 20250313, LLVM 21.0.0)
    ...
    <pre><code>ispc SGEMM_kernels.ispc -O3 -o SGEMM_kernels_ispc.o -h SGEMM_kernels_ispc.h --target=avx2-i32x16 --pic
</code></pre>
    <blockquote>
     <p>
      <code>
    ...
    <pre><code>clang++ -O3 SGEMM_main.cpp SGEMM_kernels_ispc.o ../common/tasksys.cpp
</code></pre>
    <p>
     否则可能会出现如下问题：
    </p>
    <pre><code>/usr/bin/ld: SGEMM_kernels_ispc.o: relocation R_X86_64_32 against `.text' can not be used when making a PIE object; recompile with -fPIE
...
ispc iterations = 500[default],	 Matrix A Rows = 2048,	 Matrix A Columns/ matrix B Rows = 16, Matrix B Columns = 4096
 ...
 SGEMM_tileBlockNoSIMDIntrin_2_withTasks     1.0505 millisecs   255.5048 GFLOPs Validation: valid.
    ...
    <p>
     ...
     这行）：
     <br/>
     <img alt="请添加图片描述" src="https://i-blog.csdnimg.cn/direct/608f3364e72746f69c3d80c96600d2ef.png"/>
    </p>
    <h3>
     <a id="_127">
     </a>
 ...
 <p alt="68747470733a2f2f62:6c6f672e6373646e2e6e65742f71715f33333931393435302f:61727469636c652f64657461696c732f313436323231323333" class_="artid" style="display:none">
 </p>
</div>
