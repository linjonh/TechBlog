...
举个例子models中添加一个商品 类中有id(自动生成)、name、code、price、crate\_time、update\_time大家自己创建下 按照上面的命令
...
* 流程：
  ...
  2. 运行
     `python manage.py makemigrations`
     ，生成迁移文件。(注意如果说有报错需要查看自己的settings.py中是否包含INSTALLED\_APPS是否包含了myapp)
  3. 运行
     `python manage.py migrate`
     ，将迁移应用到数据库（可以查看数据库中建立的表，生成的表是app名称\_对象名）。