...
通过派生TestFixture类来设计某个类或某组相关功能的单元测试,Fixture定义公共函数setUp()初始化每个成员变量,tearDown()来释放setUp中使用的资源。在每个测试中,CPPUNIT\_ASSERT(bool)来判断某个函数和表达式的正确性,在派生类的声明中,通过CPPUNIT\_TEST来增加对应的测试函数,通过CPPUNIT\_TEST\_SUITE和CPPUNIT\_TEST\_SUITE\_END来分装所有的测试函数,规定这些测试函数执行的顺序
...
把CppUnit相关的lib文件和dll文件（cppunitd.lib,cppunitd\_dll.lib,testrunnerd.lib）加入到工程中：
