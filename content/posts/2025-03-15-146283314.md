...
玩命更新中~\_哔哩哔哩\_bilibili](https://www.bilibili.com/video/BV1XGwXeYEYY/ "2025版
Flask3 Python web开发 视频教程\(无废话版\) 玩命更新中~\_哔哩哔哩\_bilibili")
...
name = StringField("年级名称", render\_kw={'placeholder': '请输入年级名称'}, validators=[DataRequired()])
remark = TextAreaField("备注", render\_kw={'placeholder': '请输入备注'})
submit = SubmitField("登录")
grade.py里加一个add()视图函数
@grade\_bp.route('/add', methods=['GET', 'POST'])
def add():
gradeForm = GradeForm()
if request.method == 'GET':
return render\_template('grade/add.html', form=gradeForm)
else:
