...
如果修复不成功，一般情况是安装软件打开时没有使用管理员权限打开，可以手动修改注册表来支持长路径：打开注册表HKEY\_LOCAL\_MACHINE\SYSTEM\CurrentControlSet\Control\FileSystem设置为
...
在桌面新建test\_1文件夹，用来保存ESP-IDF新建的工程，然后打开ESP-IDFCMD终端输入“idf.py
