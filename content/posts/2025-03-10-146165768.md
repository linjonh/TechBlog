...
* **配置解释器**
  ...
  启动 PyCharm 打开源码，并配置刚刚设置的 open\_manus conda 环境，我这里已经配置过了，没有配置过第一步显示的是 Add interpreter
...
>   
> base\_url：百炼云平台的接口地址，只要是百炼的模型，接口地址几乎是相同的，不用修改
>   
> api\_key：这个就是之前注册的 api-key，直接复制接口
>   
> 其他的都不用修改
...
> 提示词：搜索整理昨天的所有AI相关的最新咨询，整理成markdown文件，文件名为：news\_ai
...
* browser\_use\_tool.py 修改

将 \_ensure\_browser\_initialized 函数替换为如下代码
