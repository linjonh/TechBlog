...
\* 创建容器并挂载数据卷 `docker run -it --name MyCentOS -v /root/data:/root/data\_container centos:7 /bin/bash`
\* /root/data 宿主机的目录，没有则会创建
\* /root/data\_container 代表容器的目录，没有会自动创建
\* 创建只读容器加参数 ro (ReadOnly简写) `docker run -it --name MyCentOS -v /root/data:/root/data\_container:ro centos:7 /bin/bash`
### 数据卷容器
...
WORKDIR $MYPATH
#配置java与tomcat环境变量
ENV JAVA\_HOME /usr/local/jdk1.8.0\_73
ENV CLASSPATH $JAVA\_HOME/lib/dt.jar:$JAVA\_HOME/lib/tools.jar
ENV CATALINA\_HOME /usr/local/apache-tomcat-8.5.71
ENV CATALINA\_BASE /usr/local/apache-tomcat-8.5.71
