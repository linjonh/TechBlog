...
#### 阿里云服务器安装oracle11g
\* + [一. 安装docker](#\_docker\_12)
+ [二 . 开始拉取镜像](#\_\_\_16)
+ - \* [2.1.创建docker容器](#21docker\_32)
\* [2.2 启动容器](#22\_\_40)
\* [2.3 进入镜像进行配置](#23\_\_49)
+ [三. 环境配置](#\_\_56)
+ [四. 数据库连接](#\_\_152)
+ - \* [4.1 使用pl/sql 进行连接](#41\_plsql\_\_159)
\* [4.2 navicat连接](#42\_navicat\_218)
+ [五. docker常用命令](#\_docker\_232)
前言：
> 1. oracle上课要用，老师指导oracle安装在本地的虚拟机上，每次都得打开，而且还时不时报错，为了能够快速使用oracle数据库，就有了我这篇文章
> 2. 我是看了这篇
> [Docker 拉取 oracle11g镜像配置](https://blog.csdn.net/qq\_38380025/article/details/80647620)
...
可以查看我的这篇文章
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/423bb4bdc6e0e60851f9ab601cd954c1.png)
### 二 . 开始拉取镜像
时间可能有点长
```sql
docker pull registry.cn-hangzhou.aliyuncs.com/helowin/oracle\_11g
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/652963f97c965aa2361c8bf97e29838d.png)
下载好了之后可以查看.
```sql
docker images
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/80d58a2a19153f85223d4a3ef39c2299.png)
##### 2.1.创建docker容器
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/77fea9fc922eff2060c2b049873ff247.png)
##### 2.3 进入镜像进行配置
```python
...
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/9505a287294ecd4d044fd3ba018b84c6.png)
```sql
wq:保存退出
```
\*\*使用命令进入sql\*\*
...
sqlplus /nolog
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/9822ca88f23edb6d1e5776d54523972f.png)
> 如果这里报错
> \*\*sqlplus 命令没找到\*\*
> 那我们还是看这篇文章
> [Docker 拉取
>
> oracle11g镜像配置](https://blog.csdn.net/qq\_38380025/article/details/80647620)
> 乖乖先创建软连接先吧
\*\*设置软路由（如果能正常使用sqlplus 命令就跳过）\*\*
我们还是先切换到root账号（如果已经在root下就不需要）。
编辑profile文件配置ORACLE环境变量
![vi](https://i-blog.csdnimg.cn/blog\_migrate/7f1d59ae91890412d948ac6a6189a362.png)
在最后面加上以下配置，wq保存退出
```bash
export ORACLE\_HOME=/home/oracle/app/oracle/product/11.2.0/dbhome\_2
export ORACLE\_SID=helowin
...
exit
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/21bb30469387b3b8f3e912d4a3ed2ee3.png)
这样在服务器上就可以登录了
```sql
#每次重新连接前，要先进入镜像
docker exec -it oracle11g bash
...
sqlplus 账号/密码
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/4f07c546de59ebc044f77166e2868926.png)
### 四. 数据库连接
看一下oracle 的 lsnrctl 服务，这里我的是helowin，和helowinXDB
```python
lsnrctl status
```
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/1b4f46ef1c411535de1c732b99cd21cc.png)
##### 4.1 使用pl/sql 进行连接
...
instantclient文件下载地址：
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/fd05afd8413eddc0b705fc15a2e7d85a.png)
也可以到我的网盘下载，链接：
提取码：x9k1
然后先不用登录，就去首页面，选择首选项，选择你下载好的oci库，
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/74ffa12caf22fd1bba878253bfa6307f.png)
具体pl/sql配置可以看这篇文章
\*\*连接方式一：直接连接\*\*
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/9819dd4e6c9a9df5b9328106a2733e5a.png)
...
(CONNECT\_DATA =
(SERVER = DEDICATED)
(SERVICE\_NAME = helowin)
)
)
```
然后在配置环境就可以了
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/46dcdabd61593584639f50a06679a29e.png)
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/39e1ba82f26818c30a045bf196236663.png)
登录成功：
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/3befe133fd3c421b55a57cd04e9c300e.png)
\*\*连接方式三: 通过命令直接启动连接\*\*
```bash
...
##### 4.2 navicat连接
账号密码之前设置过了
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/fc4bea3c7dd8ddbac0c1817d99d25ec2.png)
参考文章：
### 五. docker常用命令
启动并加入开机启动
```
...
请给我点个赞鼓励我吧
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog\_migrate/94fd5d406f5126e47ac2532c1d9d7eae.png#pic\_center)
