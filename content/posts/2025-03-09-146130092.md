...
给定一个下标从 0 开始的二进制字符串 `s` 和两个整数 `minJump` 和 `maxJump`。初始时，你位于下标 0 处（保证该位置为
'0'）。你需要判断是否能到达字符串的最后一个位置（下标为 `s.length - 1`）。移动规则如下：
...
### **复杂度分析**
  ...
  * **时间复杂度** ：![O\(n\)](https://latex.csdn.net/eq?O%28n%29)，每个位置仅遍历一次。
  * **空间复杂度** ：![O\(n\)](https://latex.csdn.net/eq?O%28n%29)，需存储 `dp` 数组和前缀和数组（方法一）或仅 `dp` 数组（方法二）。

### **总结与优化**

  1. **动态规划是核心** ：通过 `dp` 数组记录状态，避免重复计算。
  2. **前缀和优化** ：将区间查询复杂度从 ![O\(n\)](https://latex.csdn.net/eq?O%28n%29) 降至 ![O\(1\)](https://latex.csdn.net/eq?O%281%29)，大幅提升效率。
