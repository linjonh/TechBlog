...
最傻瓜最蠢的办法就是找到 start\_windows.Bat，创建一个快捷方式，然后拖到开机启动菜单里去。如果不想用这种方式，可以使用 sc 命令把 open\_webui 注册为系统服务，还有一种方法是用 nssm 把 open\_webui 注册为系统服务，我尝试过 nssm 虽然注册成功但是启动服务报错，大家如果成功了可以告诉我。
...
保存后，把文件名和后缀名修改为 start\_windows. Bat，并为 start\_windows. Bat 创建快捷方式。
  
然后开始---运行---输入 shell: startup 命令，把 start\_windows. Bat 的快捷方式拖到启动菜单里。
