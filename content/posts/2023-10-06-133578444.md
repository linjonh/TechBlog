...
命令: bash build\_chain.sh -l 127.0.0.1:4 -p 30300,20200,8545
...
命令:bash nodes/127.0.0.1/start\_all.sh
...
(1)使用gen\_node\_cert.sh 生成新节点node4

命令: bash gen\_node\_cert.sh -c nodes/cert/agency- o node4
...
对于[rpc]模块，修改 channel\_listen\_port和jsonrpc\_listen\_port;

对于[p2p]模块，修改listen\_port并在node中增加自身节点信息
...
编辑certficate\_blacklist,设置node3节点ID为黑名单
...
命令:setSystemConfigByKey tx\_count\_limit 2000
...
命令:getSystemConfigByKey tx\_count\_limit
