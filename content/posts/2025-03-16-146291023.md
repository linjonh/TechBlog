 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre id="zKeY9"><code>from keras import models
...
model.add(layers.Dense(10, activation='softmax'))</code></pre>
    ...
    <pre id="jjWjS"><code>pip install keras
pip install tensorflow</code></pre>
    ...
    <pre id="jNX0C"><code>from keras.datasets import imdb
...
(train_data, train_labels), (test_data, test_labels) = imdb.load_data(num_words=10000)</code></pre>
    ...
    <pre id="wh35b"><code>import numpy as np
...
x_test = vectorize_sequences(test_data)</code></pre>
    ...
    <pre id="vClya"><code>y_train = np.asarray(train_labels).astype('float32')
y_test = np.asarray(test_labels).astype('float32')</code></pre>
    ...
    <pre id="GCfHu"><code>model = models.Sequential()
...
model.add(layers.Dense(1, activation='sigmoid'))</code></pre>
    ...
    <pre id="DrbMK"><code>model.compile(optimizer='rmsprop',
    ...
    <pre id="urvyg"><code>x_val = x_train[:10000]
...
partial_y_train = y_train[10000:]</code></pre>
    ...
    <pre id="FWfvT"><code>history = model.fit(partial_x_train,
                    partial_y_train,
                    epochs=20,
                    batch_size=512,
    ...
    <pre id="gVTXr"><code>test_loss, test_acc = model.evaluate(x_test, y_test)
print('Test accuracy:', test_acc)</code></pre>
    ...
    <pre id="WuxeM"><code>from keras.datasets import reuters

(train_data, train_labels), (test_data, test_labels) = reuters.load_data(num_words=10000)</code></pre>
    <p id="u235aa721">
     <strong>
      在训练模型之前，我们需要对数据进行预处理。
     </strong>
    ...
    <pre id="GaCuV"><code>def vectorize_sequences(sequences, dimension=10000):
    results = np.zeros((len(sequences), dimension))
    for i, sequence in enumerate(sequences):
        results[i, sequence] = 1.
    return results
...
x_test = vectorize_sequences(test_data)</code></pre>
    ...
    <pre id="VpHXh"><code>from keras.utils.np_utils import to_categorical

y_train = to_categorical(train_labels)
y_test = to_categorical(test_labels)</code></pre>
    <p id="uf60cc162">
     <strong>
      接下来，我们构建一个简单的神经网络。
     </strong>
    ...
    <pre id="jpZWc"><code>model = models.Sequential()
...
model.add(layers.Dense(46, activation='softmax'))</code></pre>
    ...
    <pre id="XKZKv"><code>model.compile(optimizer='rmsprop',
    ...
    <pre id="aKXZH"><code>x_val = x_train[:1000]
...
partial_y_train = y_train[1000:]</code></pre>
    ...
    <pre id="NOZn5"><code>history = model.fit(partial_x_train,
                    partial_y_train,
                    epochs=20,
                    batch_size=512,
                    validation_data=(x_val, y_val))</code></pre>
    ...
    <pre id="h2vqF"><code>test_loss, test_acc = model.evaluate(x_test, y_test)
print('Test accuracy:', test_acc)</code></pre>
    <p id="u42528fd4">
    ...
    <pre id="WUvVV"><code>from keras.datasets import boston_housing

(train_data, train_targets), (test_data, test_targets) = boston_housing.load_data()</code></pre>
    <p id="ue387db33">
     <strong>
    ...
    <pre id="Mu0FG"><code>mean = train_data.mean(axis=0)
train_data -= mean
...
test_data /= std</code></pre>
    ...
    <pre id="lyNJ5"><code>model = models.Sequential()
...
model.add(layers.Dense(1))</code></pre>
    ...
    <pre id="eq4ax"><code>model.compile(optimizer='rmsprop', loss='mse', metrics=['mae'])</code></pre>
    <p id="uac1bc3bc">
     <strong>
      在训练模型之前，我们需要划分出一个验证集。
     </strong>
    ...
    <pre id="eM2vF"><code>x_val = train_data[:100]
partial_x_train = train_data[100:]

y_val = train_targets[:100]
partial_y_train = train_targets[100:]</code></pre>
    <p id="ufb2e2182">
     <strong>
      最后，我们开始训练模型。
     </strong>
    ...
    <pre id="ZsJ8j"><code>history = model.fit(partial_x_train,
                    partial_y_train,
                    epochs=100,
                    batch_size=16,
                    validation_data=(x_val, y_val))</code></pre>
    ...
    <pre id="EOyYH"><code>test_loss, test_mae = model.evaluate(test_data, test_targets)
print('Test MAE:', test_mae)</code></pre>
 ...
 <p alt="68747470733a2f2f626c6f67:2e6373646e2e6e65742f65617274687a68616e67323032312f:61727469636c652f64657461696c732f313436323931303233" class_="artid" style="display:none">
 </p>
</div>
