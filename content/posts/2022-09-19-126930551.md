...
4、此时会在~.ssh文件中生成id\_isa\_pub文件和authorized\_keys文件。
...
panwenjun@server:~$ sudo vim /etc/vsftpd.chroot\_list
...
DAEMON\_OPTIONS(`Family=inet, Name=MTA-v4, Port=smtp, Addr=127.0.0.1’)dnl
