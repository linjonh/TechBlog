...
很多时候修改代码后重新编译运行发现界面还是原来的，没有变过来，这是因为VS 2008 Feature Pack采用了和BCG一样的做法，把界面配置信息保存在注册表。这时只需将注册表的相关项删除再重新编译运行，就能看到修改后的界面。注册表项的位置一般在HKEY\_CURRENT\_USER/Software/应用程序向导生成的本地应用程序/"你的工程名"。我习惯写一个批处理文件，里面的命令如下：
  
reg delete HKEY\_CURRENT\_USER/Software/应用程序向导生成的本地应用程序/"你的工程名" /f
