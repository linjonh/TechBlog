...
Spring MVC 是一种基于 Java 的实现了 MVC（Model-View-Controller，模型 - 视图 - 控制器）设计模式的 Web
应用框架，它是 Spring 框架的一个重要组成部分，用于构建 Web 应用程序。
...
可以看到 required 的默认值为true,表⽰含义就是:该注解修饰的参数默认为必传。既然如此,我们可以通过设置 **@RequestParam**
...
后端也可以看到数组长度为4![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/9a7bbcae18704f76b7ce102c49554fed.png)

  2. ![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/df0def8855b244ef8a54aeac409b4768.png)

同一个参数传多次，也会被认为同一个数组，后端也可以看到数组长度为3![在这里插入图片描述](https://i-blog.csdnimg.cn/direct/742f2eef5b2d4ab5affc30b0a9c0ce78.png)
...
Json字符串和Java对象的转换本质上是jackson-
databind提供的功能,SpringMVC框架中已经把该⼯具包引⼊了进来,咱们直接使⽤即可,如果脱离SpringMVC使⽤,需要引⼊相关依赖
