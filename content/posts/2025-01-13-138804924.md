...
> 方法1：ALTER TABLE old\_tbname RENAME new\_tbname;
>   
> 方法2： RENAME TABLE old\_tbname TO new\_tbname;

eg：

ALTER TABLE db\_hero RENAME dbhero;使db\_hero修改为dbhero
  
RENAME TABLE dbhero to db\_hero;将该表恢复为之前的命名
...
ALTER TABLE db\_hero add hobby varchar(30) not null after age;
...
> ALTER TABLE db\_hero MODIFY email varchar(50) not null;
...
ALTER TABLE db\_hero change email e\_mail varchar(40) not null;

###### 5.删除字段

> ALTER TABLE 表名 DROP 列名

ALTER TABLE db\_hero DROP wq
...
alter table db\_hero drop primary key;
...
CREATE TABLE dbhero like db\_hero;
...
> INSERT INTO table\_name (column1, column2) VALUES (value1, value2), (value3, value4),

3.使用NULL

在允许NULL值的列中，你可以显式地插入NULL。例如：

INSERT INTO table\_name (column1, column2) VALUES (NULL, value2);

4.从其他表中插入

> INSERT INTO table\_name1 (column1, column2) SELECT column1, column2 FROM table\_name2;

![](https://i-blog.csdnimg.cn/blog_migrate/5e681a6d3ca0fbe2aa1e0202906c1dcc.png)

5.插入数据并返回ID

对于自增主键的表，插入数据后你可能需要得到新插入行的ID。这可以通过LAST\_INSERT\_ID()函数实现：
...
>
> INSERT INTO table\_name DEFAULT VALUES;
