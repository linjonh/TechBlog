...
| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| api\_key | str | 必填 | DeepSeek API 密钥 |
| base\_url | str | “https://api.deepseek.com” | API 基础 URL |

#### 7.2 send\_message 方法参数

| 参数 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| message | str | 必填 | 用户消息内容 |
| system\_prompt | str | “You are a helpful assistant” | 系统提示词 |
| model | str | “deepseek-chat” | 使用的模型名称 |
| stream | bool | False | 是否使用流式响应 |
| save\_history | bool | True | 是否保存对话历史 |
| temperature | float | 可选 | 控制回复的随机性 |
| max\_tokens | int | 可选 | 限制回复长度 |
