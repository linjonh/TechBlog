...
y(P\_0)=\sum\_{P\_n\in R}w(P\_n)\cdot x(P\_0+P\_n)
...
，对其与它在R中的所有偏移位置（上下左右及对角线）的特征点与卷积核对应的位置进行加权求和，得到新特征图上对应的P\_0点。例如
...
y(P\_0)=\sum\_{P\_n\in R}w(P\_n)\cdot x(P\_0+P\_n+\Delta P\_n)
...
i
j
y(i,j)=\sum\_{p \in bin(i,j)} x(P\_0+P)/n\_{ij}
...
i
j
y(i,j)=\sum\_{p \in bin(i,j)} x(P\_0+P+\Delta P\_{ij})/n\_{ij}
...
\Delta P\_{ij}=\gamma \cdot \Delta \hat{P\_{ij}} \cdot (w,h)
