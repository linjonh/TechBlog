---
arturl_encode: "68747470733a2f2f:626c6f672e6373646e2e6e65742f68616e313230323031322f:61727469636c652f64657461696c732f313034393030393634"
layout: post
title: "Android-FFMPEG-开发FFMPEG-直播功能完整流程-源码-源码交叉编译-AS工程配置-音视频打开读取解码格式转换-原生绘制播放-音视频同步-"
date: 2022-10-13 14:04:32 +08:00
...
* + - * [I . FFMPEG 播放视频流程总结](#I__FFMPEG__25)
      * [II . FFMPEG 下载及交叉编译](#II__FFMPEG__104)
      * [III . Android Studio 配置 FFMPEG 函数库](#III__Android_Studio__FFMPEG__193)
      * [IV . FFMPEG 初始化](#IV__FFMPEG__423)
      * [V . FFMPEG 获取 AVStream 音视频流](#V__FFMPEG__AVStream__522)
      * [VI . FFMPEG 获取编解码器](#VI__FFMPEG__599)
      * [VII . FFMPEG 读取音视频流中的数据到 AVPacket ( 压缩编码后的数据包 )](#VII__FFMPEG__AVPacket____730)
      * [VIII . FFMPEG AVFrame 图像格式转换 YUV -> RGBA](#VIII__FFMPEG_AVFrame__YUV__RGBA_794)
      * [IX . ANativeWindow 原生绘制](#IX__ANativeWindow__919)
      * [X . FFMPEG 音频重采样](#X__FFMPEG__1095)
      * [XI . OpenSLES 播放音频](#XI__OpenSLES__1295)
      * [XII . FFMPEG 音视频同步](#XII__FFMPEG__1761)
      * [XIII . GitHub 代码地址](#XIII__GitHub__1869)
...
**1 . GitHub 代码地址 :**
[FFMPEG 直播示例](https://github.com/han1202012/011_FFMPEG)

**2 . 效果展示 :**

使用的是湖南卫视的直播源 rtmp://58.200.131.2:1935/livetv/hunantv
  
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/701b31bd4b8c46100f1adc14211de6f6.gif)
...
[【Android FFMPEG 开发】FFMPEG 解码 AVPacket 数据到 AVFrame ( AVPacket->解码器 | 初始化 AVFrame | 解码为 AVFrame 数据 )](https://blog.csdn.net/shulianghan/article/details/104768104)
...
**11 . FFMPEG 音频重采样 :**
[【Android FFMPEG 开发】FFMPEG 音频重采样 ( 初始化音频重采样上下文 SwrContext | 计算音频延迟 | 计算输出样本个数 | 音频重采样 swr\_convert )](https://blog.csdn.net/shulianghan/article/details/104871223)
...
**14 . FFMPEG 直播示例 :**
[【Android FFMPEG 开发】FFMPEG 直播功能完整流程 + 源码 ( 源码交叉编译 -> AS工程配置 -> 音视频打开/读取/解码/格式转换 -> 原生绘制播放 -> 音视频同步 )](https://hanshuliang.blog.csdn.net/article/details/104900964)
...
在 FFMPEG 源码根目录下 , 创建 build\_ffmpeg.sh 文件 , 内容如下 ;
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2f3fac6b4b33ff9c3299153e220d7983.png)
...
![在这里插入图片描述](https://i-blog.csdnimg.cn/blog_migrate/2f4abc01f05850b7c45a705b0a39f4d0.png)
...
**av\_read\_frame ( AVFormatContext *s , AVPacket *pkt )**
...
**ANativeWindow\_setBuffersGeometry ( )**
...
**④ 获取 ANativeWindow\_Buffer 绘制缓冲区 :**
...
**⑤ 填充图像数据到 ANativeWindow\_Buffer 绘制缓冲区中 :**

将图像字节数据使用内存拷贝到 ANativeWindow\_Buffer 结构体的 bits 字段中 ;
...
**① 初始化音频重采样上下文 :
struct SwrContext *swr\_alloc\_set\_opts( … )
,
int swr\_init(struct SwrContext *s)**
...
**int64\_t swr\_get\_delay(struct SwrContext *s, int64\_t base)**
...
**int64\_t av\_rescale\_rnd(int64\_t a, int64\_t b, int64\_t c, enum AVRounding rnd) av\_const**
...
**int swr\_convert(struct SwrContext *s, uint8\_t **out, int out\_count, const uint8\_t **in , int in\_count)**
...
音频重采样 swr\_convert ( ) 返回值 samples\_per\_channel\_count 是 每个通道的样本数 ;
...
装载输出混音器对象 到 SLDataLocator\_OutputMix , 在将 SLDataLocator\_OutputMix 结构体装载到 SLDataSink 中 ;
...
设置播放状态为 SL\_PLAYSTATE\_PLAYING ;
