...
其中base64主要用来进行base64编码解码，comm\_codec主要用来通用的解析XML，格式转换等函数集合，config主要用来加载配置文件config.xml，LogClinet主要用来打印日志文件，strtok\_r主要用来字符串切割，其中NetFrame是包含main方法的主线程文件。创建完了CPP和头文件之后，将创建Makefile文件。
...
找到curl-devel.x86\_64，再yum install curl-devel.x86\_64 安装即可
