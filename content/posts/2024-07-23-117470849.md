...
（1）达梦数据库屏蔽关键词的方法有两种，上述的方法，针对是该客户端或者应用服务器有效，如果更换了客户端，还需要重新配置一次；而如果想要全局有效，屏蔽终端的区别，可以修改dm.ini文件的EXCLUSIVE\_\_RESERVED\_WORDS参数值为需要屏蔽的关键词，但是这种操作不建议。
  
（2）如果是修改了应用服务器的dm\_svc.conf文件，修改完成后需要重新应用服务。