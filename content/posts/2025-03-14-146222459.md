...
在深度学习中，我们通常通过损失函数下降的曲线来观察模型训练的情况。而YOLOv8训练时主要包含三个方面的损失：矩形框损失(box\_loss)、置信度损失（obj\_loss）和分类损失(cls\_loss)，在训练结束后，我们也可以在logs目录下找到生成对若干训练过程统计图。下图为博主训练模型训练曲线图。
