...
其中Init\_First是为这次迁移起的一个名称，如下图所示：
...
20200224163036\_Init\_First.cs：是针对本次迁移生成的文件。每次迁移都会生成一个对应的迁移文件。里面有一个Up方法和一个Down方法，Up方法表示要执行的操作，Down方法相当于回滚，如果需要还原到上一个状态，就会执行Down方法。
