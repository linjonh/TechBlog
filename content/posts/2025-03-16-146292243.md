...
2. **开关token设计**
   ...
   \left[c\_{1}, Z\_{1}, Z\_{1}', E\_{1}\right]=L\_{1}\left(c\_{0}, P, S, E\_{0}\right)
...
3. **跨模式特征蒸馏损失（Cross-mode feature distillation loss）**
   ...
   ∥
   2
   \mathcal{L}\_{cs}(S)=\mathbb{E}\_{(x, y) \sim \mathcal{D}}\left\| F\_{f}(P, x)-F\_{f}(P, S, x)\right\| \_{2}
