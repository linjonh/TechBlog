...
UniApp 是一个基于 Vue.js 的跨平台开发框架，允许开发者使用一套代码构建多端应用（如小程序、H5、App 等）。UniApp 的生命周期结合了
Vue.js 的生命周期和各个平台（如微信小程序、App 等）的生命周期。以下是 UniApp 的主要生命周期钩子：
...
## **注意：**
  ...
  * 在小程序平台，部分生命周期（如 `onPullDownRefresh`、`onReachBottom` 等）是小程序特有的。

[vue生命周期+微信小程序生命周期详解_小程序生命周期和vue生命周期-
CSDN博客](https://blog.csdn.net/layxing27/article/details/109398512?fromshare=blogdetail&sharetype=blogdetail&sharerId=109398512&sharerefer=PC&sharesource=m0_61811926&sharefrom=from_link
