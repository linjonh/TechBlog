...
1
N
\begin{equation} \mathrm{softmax}(\{x\_1,\ldots,x\_N\})=\left\{\frac{e^{x\_i}}{\sum\_{j=1}^Ne^{x\_j}}\right\}\_{i=1}^N \end{equation}
...
−
m
\begin{equation} \frac{e^{x\_{i}}}{\sum\_{j=1}^{N}e^{x\_{j}}}=\frac{e^{x\_{i}-m}}{\sum\_{j=1}^{N}e^{x\_{j}-m}} \end{equation}
...
\begin{equation} m\_i \leftarrow \max (m\_{i-1},x\_i) \end{equation}
...
m
N
\begin{equation} d\_i \leftarrow d\_{i-1} + e^{x\_i-m\_N} \end{equation}
...
d
N
\begin{equation} a\_i \leftarrow \frac{e^{x\_i-m\_N}}{d\_N} \end{equation}
...
m
i
d\_{i}^{ \prime} := \sum\_{j=1}^{i}e^{x\_{j}-m\_{i}}
...
m
N
d\_{i}^{ \prime} := \sum\_{j=1}^{i}e^{x\_{j}-m\_{N}}
...
m
i
\begin{equation} \begin{aligned} d\_{i}^{\prime} & =\sum\_{j=1}^ie^{x\_j-m\_i} \\ & =\left(\sum\_{j=1}^{i-1}e^{x\_j-m\_i}\right)+e^{x\_i-m\_i} \\ & =\left(\sum\_{j=1}^{i-1}e^{x\_j-m\_{i-1}}\right)e^{m\_{i-1}-m\_i}+e^{x\_i-m\_i} \\ & =d\_{i-1}^{\prime}e^{m\_{i-1}-m\_i}+e^{x\_i-m\_i} \end{aligned} \end{equation}
...
m
i
\begin{aligned} m\_i & \leftarrow \max (m\_{i-1}, x\_i) \\ d^{\prime}\_i & \leftarrow d^{\prime}\_{i-1}e^{m\_{i-1}-m\_i}+e^{x\_i-m\_i} \end{aligned}
...
N
′
a\_i \leftarrow \frac{e^{x\_i - m\_N}}{d^{\prime}\_N}
...
m
i
\begin{aligned} x\_i & \leftarrow Q[k,:]K^T[:,i] \\ m\_i & \leftarrow \max (m\_{i-1},x\_i) \\ d^{\prime}\_i & \leftarrow d^{\prime}\_{i-1} e^{m\_{i-1}-m\_i} + e^{x\_i-m\_i} \end{aligned}
...
\begin{align} a\_i & \leftarrow \frac{e^{x\_i-m\_N}}{d^{\prime}\_N} \\ \bm{o}\_i & \leftarrow \bm{o}\_{i-1} + a\_iV[i,:] \end{align}
...
\begin{align} \bm{o}\_i := \sum\_{j=1}^i\left(\frac{e^{x\_j-m\_N}}{d\_N^{\prime}}V[j,:]\right) \end{align}
...
\bm{o}\_i^{\prime}:=\left(\sum\_{j=1}^i\frac{e^{x\_j-m\_i}}{d\_i^{\prime}}V[j,:]\right)
...
\begin{equation} \begin{aligned} \bm{o}^{\prime}\_{i}& = \sum\_{j=1}^{i} \frac{e^{x\_{j}-m\_{i}}}{d^{\prime}\_{i}}V[j, : ]\\ & = \left(\sum\_{j=1}^{i-1}\frac{e^{x\_{j}-m\_{i}}}{d^{\prime}\_{i} }V[j, : ]\right) + \frac{e^{x\_{i}-m\_{i}}}{d^{\prime}\_{i}}V[i, : ]\\ & = \left(\sum\_{j=1}^{i-1}\frac{e^{x\_{j}-m\_{i-1}}}{d^{\prime}\_{ i-1}}\frac{e^{x\_{j}-m\_{i}}}{e^{x\_{j}-m\_{i-1}}}\frac{d^{\prime}\_{i-1}}{d^{ \prime}\_{i}}V[j, : ]\right) + \frac{e^{x\_{i}-m\_{i}}}{d^{\prime}\_{i}}V[i, : ]\\ & = \left(\sum\_{j=1}^{i-1}\frac{e^{x\_{j}-m\_{i-1}}}{d^{\prime}\_{ i-1}}V[j, : ]\right) \frac{d^{\prime}\_{i-1}}{d^{\prime}\_{i}}e^{m\_{i-1}-m\_{i}} + \frac{e^{x\_{i}-m\_{i}}}{d^{\prime}\_{i}}V[i, : ]\\ & = \bm{o}^{\prime}\_{i-1}\frac{d^{\prime}\_{i-1} e^{m\_{i-1}-m\_{ i}}}{d^{\prime}\_{i}} + \frac{e^{x\_{i}-m\_{i}}}{d^{\prime}\_{i}}V[i, : ] \end{aligned} \end{equation}
...
x
i
d^{\prime}\_i,\ d^{\prime}\_{i-1}, \ m\_i, \ m\_{i-1}, \ x\_i
...
\begin{aligned} {x\_{i}} & \leftarrow {Q[k,:]} {K^{T}[:,i]}\\ {m\_{i}} & \leftarrow \max\left( {m\_{i-1},x\_{i}} \right)\\ {d^{\prime}\_{i}} & \leftarrow {d^{\prime}\_{i-1}e^{m\_{i-1}-m\_{i}}}+ {e^{x\_{i}-m\_{i}}}\\ {o^{\prime}\_{i}} & \leftarrow {o^{\prime}\_{i-1}}\frac{ {d^{\prime}\_{i-1}e^{m\_{i-1}-m\_{i}}}}{ { d^{\prime}\_{i}}}+\frac{ {e^{x\_{i}-m\_{i}}}}{ {d^{\prime}\_{i}}} {V[i,:]}\end{aligned}
...
i
′
x\_i, \ m\_i, \ d^{\prime}\_i, \bm{o}^{\prime}\_i
...
\begin{split}\bm{x}\_{i}&\ \leftarrow\ Q[k, : ] K^{T}[:,(i-1) b : i b]\\ m\_{i}^{\text{(local)}}&\ =\ \max\_{j=1}^{b} (\bm{x}\_{i} [j]) \\ m\_{i}&\ \leftarrow\ \max\big(m\_{i-1},m\_{i}^{\text{(local)}} \big)\\ d\_{i}^{\prime}&\ \leftarrow\ d\_{i-1}^{\prime} e^{m\_{i-1}-m\_{i}} + \sum\_{j=1}^{b} e^{\bm{x}\_{i}[j]-m\_{i}}\\ \bm{o}\_{i}^{\prime}&\ \leftarrow\ \bm{o}\_{i-1}^{\prime} \frac{d\_{i-1}^{\prime} e^{m\_{i-1}-m\_{i}}}{d\_{i}^{\prime}} + \sum\_{j=1}^{b} \frac{e^{\bm{x}\_{i}[j]-m\_{i}}}{d\_{i}^{\prime}}V[j+(i-1) b, : ]\end{split}
