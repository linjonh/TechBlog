...
cap = cv2.VideoCapture(source, cv2.CAP\_FFMPEG)
  
cap.set(cv2.CAP\_PROP\_HW\_ACCELERATION, cv2.VIDEO\_ACCELERATION\_ANY)
...
* 开启FFmpeg日志分析：export OPENCV\_FFMPEG\_DEBUG=1
* 强制指定解码器：cv2.CAP\_PROP\_CODEC\_PREFERENCE= cv2.VIDEO\_ACCELERATION\_ANY
