...
经常接触Python的同学可能会注意到，当需要安装第三方python包时，可能会用到easy\_install命令。easy\_install是由PEAK(Python Enterprise Application Kit)开发的setuptools包里带的一个命令，它用来安装egg包。egg包是目前最流行的python应用打包部署方式。如何制作和安装egg包？下面我就简单的分析了一下。
...
* 通过引导程序
  [ez\_setup.py](http://peak.telecommunity.com/dist/ez_setup.py)
...
现在就可以使用easy\_install命令来安装其他的egg包了。
...
* version 版本号，默认0.0.0
* packages 这里要用到setuptools的另一个函数find\_packages，顾名思义，find\_packages用来将指定目录下的文件打包。
* zip\_safe 默认是False，这样在每次生成egg包时都会检查项目文件的内容，确保无误。

还有一些描述性的属性，如description，long\_description，author，author\_email，license，keywords，platform，url等。 填充setup.py文件如下：:
...
在egg-demo目录下建立和上述name名称相同的目录demo，demo目录下写\_\_init\_\_.py文件：
...
可以看到，多了一个文件夹demo，里面有我们写的\_\_init\_\_.py。 奉行敏捷原则，先安装了体验一下再说：
...
成功输出！这说明安装正确。我们的一个egg包诞生了。 一般情况下，我们的源程序都放在src目录下，所以接下来将demo文件夹移动到src里。但这样也要修改setup.py文件，修改find\_packages函数中参数为'src'，同时增加package\_dir参数：
