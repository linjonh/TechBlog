...
安装的wheel只有两个：Cartopy-0.20.2-cp38-cp38-win\_amd64.whl        和        Pillow-8.4.0-cp38-cp38-win\_amd64.whl
...
例如：pip install C:\tools\python\python\Lib\site-packages\Cartopy-0.20.2-cp38-cp38-win\_amd64.whl
...
其实安装完上面两个wheel后，会发现在安装Cartopy-0.20.2-cp38-cp38-win\_amd64.whl时就已经把shapely、pyproj、pyshp和cartopy这四个依赖包都安装上了，所以就不需要我们分别下载pyshp等其它wheel了。

综上，python3.8只需要安装Cartopy-0.20.2-cp38-cp38-win\_amd64.whl        和        Pillow-8.4.0-cp38-cp38-win\_amd64.whl就够了，如果安装完这两个还不行，就需要分别下载安装其它的依赖包了。