...
* 与此同时，为了避免冗余，AIDL 音频 HAL 中移除了 IDevicesFactory 接口。HAL 模块（即 IModule 的实例）现在直接以其名称在 Service Manager 中注册，例如 bluetooth 或 r\_submix。唯一的例外是主模块（primary module），它仍然以 default 作为实例名称进行注册。
...
| audio\_policy\_configuration.xml / audio\_policy\_engine\_configuration.xml | IConfig / IModule |
