...
fn function\_name(parameter\_list) -> ReturnType {
// 函数体
// 返回值（可选）
}
2.返回值
...
fn borrow\_test(s: &String){
println!("{}", (\*s).to\_uppercase());
}
fn main() {
...
fn increment\_by\_reference(x: &mut i32) {
\*x += 1; // 修改引用指向的值
}
fn main() {
let mut num = 5;
increment\_by\_reference(&mut num);
println!("{}", num); // 输出：6
}
4.递归函数
递归函数通常包含两个主要部分：
