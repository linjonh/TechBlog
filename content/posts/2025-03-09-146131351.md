...
在这个配置中，upstream 定义了一个服务器组 nodejs，包含两个 Node.js 实例（运行在 8000 和 8001 端口）。Nginx 会将请求分发到这个组中的服务器，实现负载均衡。server 块定义了 Nginx 监听的端口和处理请求的规则，location 块中的 proxy\_pass 将请求转发到 Node.js 服务器。
