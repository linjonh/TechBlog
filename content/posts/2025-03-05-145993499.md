...
# fpga系列 硬件(时序收敛)：set\_input\_delay +最大输入延时&最小输入延时+建立时间&保持时间
...
* 对于最大输入延时，需确保数据在时钟的有效边沿之前已经稳定，并且满足寄存器的建立时间要求（
  ...
  ≤
  时钟周期
  \text{最大输入延时} + T\_{clk-q} + T\_{setup} \leq \text{时钟周期}
...
* 对于最小输入延时，确保数据在时钟的有效边沿之后仍然保持稳定，并且满足寄存器的保持时间要求（
  ...
  l
  d
  \text{最小输入延时} \geq T\_{clk-q} + T\_{hold}
...
* set\_input\_delay用于指定输入数据引脚相对于其时钟沿的延时，主要用于数据路径中的pin2reg(输入端口到内部寄存器的延迟):
