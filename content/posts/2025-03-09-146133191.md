...
AlphaFold3 `data_transforms 模块的 `squeeze_features 函数的作用**去除**
**蛋白质特征张量中不必要的单维度（singleton dimensions）和重复维度** ，以使其适配 **AlphaFold3** 预期的输入格式。
...
##### Step 2: 移除单维度
  ...
  * **如果最后一维`final_dim` 为 `1`**，说明这个维度是**无意义的单维度** ，需要去除： 
...
`seq_length` 和 `num_alignments` 可能是 **列表或张量** ，但它们的数值其实是一个单独的整数，因此需要转换成
**标量值** 。

##### **结论**

1️⃣ **转换`aatype`**: 从 **one-hot 编码** 转换成 **索引表示** 。  
2️⃣ **移除无用的单维度** : 让 `msa`, `resolution`, `deletion_matrix` 等数据符合 AlphaFold3
预期格式。  
