...
* 解压Win2kServer\_SP0\_target的rar镜像到某一目录。
* 文件 --> 打开，选择你的解压目录，选择相应vmx文件。
* 配置靶机虚拟机的硬件：鼠标选择Win2kServer\_SP0\_targe,Metasploitable\_ubuntu的选项卡，菜单中选择虚拟机 -->设置，设备选项卡中选择点击内存，在右侧设置合适大小256MB。
...
此时 Metasploitable\_ubuntu内的ip设置则是需要在命令行下输入sudovim/etc/network/interfaces命令。
