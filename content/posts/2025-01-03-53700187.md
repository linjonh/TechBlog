...
<font color=’'red>不是开机阶段，而是在每一次打开settings或者当前切换用户(因为系统为每一个用户维护一个单独的search\_index.db)，或者是当前的语言发生变化，此时会回调
...
* 根据当前包名构建指定URI，例如，settings：content://com.android.settings/settings/indexables\_xml\_res
* 然后通过context对象查找对应的Provider的数据
    
  之所以构建出content://com.android.settings/settings/indexables\_xml\_res这样的URI是因为所有的需要被搜索到的设置项所在的APP，其Provider都需要继承自SearchIndexablesProvider
...
到现在为止，在setting的搜索框中，就可以搜索到network\_setting.xml中指定的设置项了。
