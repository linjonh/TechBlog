---
layout: post
title: "解决5解决狂神说_Linux配置java环境变量后出现命令不能使用的问题,形如-bash-ls-command-not-found"
date: 2024-03-29 17:33:04 +0800
description: "解决5：解决狂神说_Linux配置java环境变量后出现命令不能使用的问题，形如：-bash: ls"
keywords: "profile添加java后命令不可用"
categories: ['_问题以及解决方法']
tags: ['Linux']
artid: "115002874"
image:
    path: https://api.vvhan.com/api/bing?rand=sj&artid=115002874
    alt: "解决5解决狂神说_Linux配置java环境变量后出现命令不能使用的问题,形如-bash-ls-command-not-found"
render_with_liquid: false
---

<div class="blog-content-box">
 <div class="article-header-box">
  <div class="article-header">
   <div class="article-title-box">
    <h1 class="title-article" id="articleContentId">
     解决5：解决狂神说_Linux配置java环境变量后出现命令不能使用的问题，形如：-bash: ls: command not found
    </h1>
   </div>
  </div>
 </div>
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   <link href="../../assets/css/kdoc_html_views-1a98987dfd.css" rel="stylesheet"/>
   <link href="../../assets/css/ck_htmledit_views-704d5b9767.css" rel="stylesheet"/>
   <div class="markdown_views prism-atom-one-light" id="content_views">
    <svg style="display: none;" xmlns="http://www.w3.org/2000/svg">
     <path d="M5,0 0,2.5 5,5z" id="raphael-marker-block" stroke-linecap="round" style="-webkit-tap-highlight-color: rgba(0, 0, 0, 0);">
     </path>
    </svg>
    <h4>
     <a id="5_Linuxjavabash_ls_command_not_found_0">
     </a>
     解决5：解决狂神说_Linux配置java环境变量后出现命令不能使用的问题，形如：-bash: ls: command not found
    </h4>
    <hr/>
    <p>
     <strong>
      问题描述：
     </strong>
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/dcb9b5568dbc13fab0250931cc7d86f1.png"/>
    </p>
    <hr/>
    <p>
     <strong>
      解决方法一：
     </strong>
    </p>
    <pre><code>export PATH=/bin:/usr/bin:$PATH
</code></pre>
    <p>
     但是该命令在重启后就会失效，没有从根本解决问题。
    </p>
    <hr/>
    <p>
     <strong>
      解决方法二：
     </strong>
    </p>
    <p>
     感觉还是之前更改的配置有问题，打开
     <code>
      vim /etc/profile
     </code>
     ,以下是之前的配置：
    </p>
    <pre><code>JAVA_HOME=/usr/java/jdk1.8.0_221-amd64
CLASSPATH=%JAVA_HOME%/lib;%JAVA_HOME%/jre/lib
PATH=$JAVA_HOME/bin;$JAVA_HOME/jre/bin
export PATH CLASSPATH JAVA_HOME
</code></pre>
    <p>
     更改为
    </p>
    <pre><code>export JAVA_HOME=/usr/java/jdk1.8.0_221-amd64
export JRE_HOME=${JAVA_HOME}/jre
export CLASSPATH=.:${JAVA_HOME}/lib:${JRE_HOME}/lib
export PATH=${JAVA_HOME}/bin:$PATH
export PATH CLASSPATH JRE_HOME JAVA_HOME
</code></pre>
    <p>
     执行
     <code>
      source /etc/profile
     </code>
     ,之后发现就问题圆满解决。
    </p>
    <hr/>
    <p>
     本文仅供自己参考学习，如有问题欢迎各位大佬指出。
    </p>
    <p>
     本文参考文章链接：https://blog.csdn.net/shichen2010/article/details/77964188
    </p>
   </div>
   <link href="../../assets/css/markdown_views-a5d25dd831.css" rel="stylesheet"/>
   <link href="../../assets/css/style-e504d6a974.css" rel="stylesheet"/>
  </div>
 </article>
 <p alt="68747470733a2f2f626c6f672e:6373646e2e6e65742f77656978696e5f34333733393236362f:61727469636c652f64657461696c732f313135303032383734" class_="artid" style="display:none">
 </p>
</div>


