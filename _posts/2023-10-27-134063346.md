...
date: 2023-10-27 15:01:15 +0800
description: "文章浏览阅读3w次，点赞120次，收藏435次。南京邮电大学数学实验报告，使用2023b版本完成，参"
keywords: "南邮数学实验"
categories: ['开发语言']
tags: ['开发语言', 'matlab']
artid: "134063346"
...
pretty(taylor(f,x,'Order',5,'ExpansionPoint',0))</code></pre>
    <p>
...
for i=1:n
    ...
    <pre><code class="language-Matlab">fplot(@(x)g(x,@f),[0,1])</code></pre>
    <p>
...
for i=1:n
    ...
    <pre><code class="language-Matlab">fplot(@(x)g(x,@f),[-pi,pi])</code></pre>
    <p>
     保存f.m和g.m两个文件，并运行添加路径，然后在命令行窗口中输入main中的代码，回车执行
    </p>
    <h3>
...
for i=1:3
    ...
    <pre><code class="language-Matlab">clear
ezplot('log(x^2+y*628)-(x^3)*y-sin(x)')</code></pre>
    <p>
     这儿请把m写进式子里
    </p>
    <h3>
...
fzero('3*x^2 + 4*157^(1/2)*x + 619/3',-7)</code></pre>
    <p>
...
solve(y,x)</code></pre>
    ...
    <pre><code class="language-Matlab">function p=dd(f,x0,n)
p=x0;
for i=2:n
    p(i)=f(p(i-1));
end
    ...
    <pre><code class="language-Matlab">dd(@(x)(2*x+1)/(x-m),___,20)</code></pre>
    <p>
...
for j=1:4
    ...
    <pre><code class="language-Matlab">function Martin(a,b,c,N)
m=628;
...
axis equal</code></pre>
    ...
    <pre><code class="language-Matlab">m=628
subplot(2,2,1)
Martin(m,m,m,5000)
subplot(2,2,2)
...
subplot(2,2,4)
Martin(-m/10,17,4,20000)</code></pre>
    <p>
...
for j=1:25
 ...
 <p alt="6874747073:3a2f2f626c6f672e6373646e2e6e65742f67776a323030332f:61727469636c652f64657461696c732f313334303633333436" class_="artid" style="display:none">
 </p>
</div>
