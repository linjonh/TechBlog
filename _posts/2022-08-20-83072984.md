...
parser.add_argument('-uu', type=str,default=None)</code></pre>
    ...
    <blockquote>
     <pre><code class="language-html hljs">if __name__ == '__main__':
    ...
    <blockquote>
     ...
     <p>
      ...
      .println(pr.waitFor());//返回0表示执行成功，返回1表示执行失败（PS：网上看到有数如果执行多条命令的时候，这个waitFor必不可少，本次没有实际用途，暂未深入研究）
     </p>
     <p>
