...
export default {
    ...
    <p>
     <span style="color:#ff9900;">
      <strong>
       这里有个坑：手动执行handleSelectCompany方法去清空值，容易出现问题：
      </strong>
     </span>
     <strong>
      当切换公司类型后，能成功清空公司名称。
    ...
    <p>
     <strong>
      想要清空掉历史记录，只能手动调用
      <span style="color:#ff9900;">
       fetch-suggestions接收函数的方法
      </span>
     </strong>
     <span style="color:#0d0016;">
      ,让其在关闭dialog或打开dialog的时候清空历史列表。可是这个方法接收2个参数，第一个是查询的字符串，第二个是callback函数。这个
      <strong>
       callback函数不知道传递啥才能 自动调用autocomplete组件
      </strong>
      。
     </span>
    </p>
...
&lt;/el-select&gt;</code></pre>
    ...
    <p>
     ...
     <span style="color:#ff9900;">
      <strong>
       对于非文本框/非select标准下拉等组件，对对象的某属性直接赋值都无效。需要使用  this.$set的方式。
      </strong>
     </span>
    </p>
