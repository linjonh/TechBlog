...
description: "文章浏览阅读2.5w次，点赞28次，微信小程序开发需要基于微信提供的开发者工具与 如果开发者对小程序"
keywords: "小程序wss视频播放"
categories: ['技术']
tags: ['小程序直播开发']
artid: "84922748"
 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="markdown_views prism-atom-one-dark" id="content_views">
    ...
    <p>
     SDK 集成指引详见：
     <a href="https://www.zego.im/html/document/#Application_Scenes/Video_Live/SDK_Integration_Guide:MiniProgram" rel="nofollow">
      微信小程序 SDK集成指引
     </a>
    </p>
    <p>
     SDK 提供的 API 说明详见：
     <a href="https://www.zego.im/html/document/#Live_Room/API_Instructions:MiniProgram" rel="nofollow">
      微信小程序 SDK API 说明
     </a>
    </p>
    <p>
    ...
    <pre><code>ZegoLive/pages/liveroom/room/room.wxml
...
&lt;/live-player&gt;
    ...
    <blockquote>
     ...
     <ol>
      <li>
       <p>
        live 模式主要用于直播类场景，比如赛事直播、在线教育、远程培训等等。该模式下，小程序内部的模块会优先保证观看体验的流畅，通过调整min-cache 和 max-cache 属性，您可以调节观众(播放)端所感受到的时间延迟的大小，文档下面会详细介绍这两个参数。
       </p>
      </li>
      <li>
       <p>
...
&lt;/live-pusher&gt;
    ...
    <p>
     2.1 推流 API 调用流程图
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/6ec67db3155a6a608cd0b1b931acf507.jpeg">
      <br/>
      详细讲解请参考 2.6 小节。
     </img>
    </p>
    <p>
     2.2 拉流 API 调用流程图
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/52e1b99088eb27059e480a99d0fd3f04.jpeg">
      <br/>
      2.3 初始化 SDK
     </img>
    </p>
    <pre><code>ZegoLive/pages/liveroom/room/room.js
...
zg.login(self.data.roomID, self.data.loginType == "anchor" ? 1 : 2, token, function (streamList) {
    ...
    <p>
     登录错误码列表如下：
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/d8900b667a9ec83372364a6aa47951a0.png">
      <br/>
      2.6 单主播直播
     </img>
    </p>
...
stopMixStream: function () {
  ...
  zg.stopMixStream(mixConfig, undefined, undefined);
    ...
    <p>
     字段说明如下：
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/97d9fac44c5090f42d71f3c94d788509.png">
      <br/>
      对于 hash 计算中使用字段的更详细说明：
      <br/>
      <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/6ea3cf9e177d09bcfc48769c5b968d1f.png"/>
     </img>
    </p>
    <blockquote>
     <p>
     ...
     <ol>
      <li>
       <p>
        login_token 传输过程中，会经过 base64 加密。每次登录都要重新获取 login_token。
       </p>
      </li>
      <li>
       <p>
        业务方开发的小程序需要和业务后台建立一种安全通讯和鉴权机制，业务方使用自有的账户体系或第三方认证体系的登录完成后，业务方小程序和业务后台交互获取该
       </p>
      </li>
      <li>
       <p>
    ...
    <pre><code>func makeTokenSample(appid uint32, app_key string, idname string, expired_add int64) (ret string, err error){
    nonce := UniqueId()
    ...
    <pre><code>public function getToken(int $app_id, string $app_key, string $idname, int $expired_add)
{
    $nonce = uniqid();
...
public class ZegouUtils {
    ...
    <p>
     ...
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/82a66e0a2b2c1debcbc6d49ef8884c0e.png">
      <br/>
    ...
    <p>
     ...
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/39e20827e13a987c1339e6d281058d98.png">
      <br/>
      其他 4 位错误码均为小程序框架的报错，请参考：小程序官方文档
     </img>
    </p>
    ...
    <p>
     ...
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/e181544aba97e607d14f4ace5f7981c5.jpeg">
      <br/>
    ...
    <p>
     ...
     <a href="http://sig-wstoken.zego.im:8181/tokenindex" rel="nofollow">
    ...
    <p>
     微信小程序组件 live-pusher
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/e67fb3e1eb1f3bbead794a6b361119ea.png"/>
     <br/>
     6.5 小程序如何切换摄像头、截图?
    </p>
    <p>
    ...
    <p>
     切换摄像头的示例代码可参考：
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/c4bc474810f32c3e4d111bf9dd30dbc5.png"/>
     <br/>
     7、微信公众平台域名配置
    </p>
    <p>
    ...
    <p>
     ...
     <br/>
     <img alt="在这里插入图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/d039d539993d59b9d1662edbcd6f7ea0.png"/>
    </p>
   </div>
 ...
 <p alt="68747470733a2f2f626c6f:672e6373646e2e6e65742f73696e61745f3230313436343231:2f61727469636c652f64657461696c732f3834393232373438" class_="artid" style="display:none">
 </p>
</div>
