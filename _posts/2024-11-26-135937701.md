...
date: 2024-11-26 23:54:12 +0800
description: 文章浏览阅读9.5k次，点赞43次，收藏102次。本文详细解释了数据库领域的SQL语言分类，包括DD
 ...
 <article class="baidu_pl">
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="htmledit_views" id="content_views">
    ...
    <pre><code>CREATE DATABASE testDB;                 --创建一个名为testDB的数据库
CREATE TABLE Students (ID INT, Name TEXT);  --创建一个名为Students的表，包含ID和Name两个字段</code></pre>
    <p>
     <code>
      ALTER
     </code>
    ...
    <pre><code>ALTER TABLE Students ADD Grade INT;        --在Students表中添加一个名为Grade的字段
ALTER TABLE Students DROP COLUMN Grade;    --在Students表中删除名为Grade的字段</code></pre>
    <p>
     <code>
      DROP
     </code>
    ...
    <pre><code>DROP DATABASE testDB;                     --删除名为testDB的数据库
DROP TABLE Students;                      --删除Students表</code></pre>
    <p>
     <code>
      TRUNCATE
     </code>
    ...
    <pre><code>TRUNCATE TABLE Students;                 --删除Students表中的所有数据，但不删除表本身</code></pre>
    <p>
     <code>
      RENAME
     </code>
    ...
    <pre><code>ALTER TABLE Students RENAME TO Pupils;  --将Students表重命名为Pupils</code></pre>
    <p>
     <code>
      COMMENT
     </code>
    ...
    <pre><code>COMMENT ON COLUMN Students.Name IS 'Student Name';  --在Students表的Name列上添加注释'Student Name'</code></pre>
    <p>
    ...
    <pre><code>SELECT * FROM Students;                    --选择Students表中的所有数据
SELECT Name, Grade FROM Students;          --选择Students表中的Name和Grade数据</code></pre>
    <p>
     <code>
      INSERT
     </code>
    ...
    <pre><code>INSERT INTO Students (ID, Name, Age)
VALUES (1, 'Tom', 18);                    --在Students表中插入一条新的数据</code></pre>
    <p>
     <code>
      UPDATE
     </code>
...
SET Age = 19 
WHERE ID = 1;                             --将Students表中ID为1的记录的Age设置为19</code></pre>
    <p>
     <code>
      DELETE
     </code>
...
WHERE ID = 1;                             --删除Students表中ID为1的数据</code></pre>
    <p>
     <code>
      MERGE
     </code>
    ...
    <pre><code>MERGE INTO Students AS Target
USING (SELECT ID, Name FROM Enrolled_Students) AS Source
ON Target.ID = Source.ID
WHEN MATCHED THEN 
UPDATE SET Name = Source.Name 
WHEN NOT MATCHED THEN 
INSERT (ID, Name) VALUES (Source.ID, Source.Name); --更新已存在的记录或插入新的记录</code></pre>
    <p>
     <code>
      CALL
     </code>
    ...
    <pre><code>CALL Update_Students_Age(18); -- 调用名为“Update_Students_Age”的存储过程</code></pre>
    <p>
     <code>
      EXPLAIN PLAN
     </code>
    ...
    <pre><code>EXPLAIN PLAN FOR SELECT * FROM Students; -- 显示查询所有学生数据的执行计划</code></pre>
    <p>
     <code>
      LOCK TABLE
     </code>
    ...
    <pre><code>LOCK TABLE Students IN EXCLUSIVE MODE;  -- 对Students表施加独占锁</code></pre>
    <p>
    ...
    <pre><code>GRANT SELECT, INSERT, UPDATE ON Students TO user1;</code></pre>
    <p>
    ...
    <pre><code>REVOKE UPDATE ON Students FROM user1;</code></pre>
    <p>
    ...
    <pre><code>INSERT INTO Students (ID, Name, Age) VALUES (1, 'Tom', 18);
COMMIT;  -- 提交事务，将插入的数据保存到数据库中</code></pre>
    <p>
     <code>
      ROLLBACK
     </code>
    ...
    <pre><code>INSERT INTO Students (ID, Name, Age) VALUES (1, 'Tom', 18);
ROLLBACK;  -- 回滚事务，撤销上一条插入命令</code></pre>
    <p>
     <code>
      SAVEPOINT
     </code>
    ...
    <pre><code>SAVEPOINT SP1;  -- 创建保存点 SP1
INSERT INTO Students (ID, Name, Age) VALUES (1, 'Tom', 18);
ROLLBACK TO SP1;  -- 回滚至保存点 SP1，不会撤销保存点后的操作</code></pre>
    <p>
 ...
 <p alt="68747470733a2f2f626c6f:672e6373646e2e6e65742f70616e746f75797563686979752f:61727469636c652f64657461696c732f313335393337373031" class_="artid" style="display:none">
 </p>
</div>
