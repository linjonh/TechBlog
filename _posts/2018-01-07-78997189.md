  ...
  <div class="article_content clearfix" id="article_content">
   ...
   <div class="markdown_views prism-atom-one-dark" id="content_views">
    ...
    <a href="http://39.105.187.122/csdn" rel="nofollow">
     <img border="0" src="https://i-blog.csdnimg.cn/blog_migrate/11ab61846568f7310c1fec6b75306fcd.jpeg">
     </img>
    </a>
    <blockquote>
     <p>
    ...
    <blockquote>
     <p>
      <a href="http://blog.csdn.net/luanpeng825485697/article/details/78347433">
       python数据挖掘系列教程
      </a>
     </p>
    </blockquote>
    ...
    <p>
     <img alt="这里写图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/1d853cb41942dc87d9ea99b73ebc6992.png"/>
    </p>
    <p>
    ...
    <pre><code># 根据连接爬取中文网站，获取标题、子连接、子连接数目、连接描述、中文分词列表，
import urllib
...
class crawler:
    ...
    <pre><code># 爬取指定域名范围内的所有网页，beginurl为开始网址，host为根网址
def crawlerhost(beginurl,host,dbname):
    ...
    <pre><code># 读取数据库信息，检验是否成功建立了搜索数据库
def readdb(dbname):
...
class searcher:
    ...
    <pre><code>select w0.urlid,w0.location,w1.location from wordlocation w0,wordlocation w1 where w0.wordid=126 and w0.urlid=w1.urlid and w1.wordid=127
</code></pre>
    <p>
    ...
    <pre><code>where w0.wordid=126 and w0.urlid=w1.urlid and w1.wordid=127
</code></pre>
    <p>
     再来看from后面的语句。表示wordlocation表可以用w0这个名称代替，wordlocation这个表也可以用w1这个名称代替。
    </p>
    <pre><code>from wordlocation w0,wordlocation w1 
</code></pre>
    <p>
    ...
    <pre><code># 评价值归一化：因为不同的评价方法的返回值和含义不同。这里所有的评价值归一化到0-1,默认越大越好
    ...
    <pre><code>  # 利用指向该链接的链接数目进行评价（仅计算回指数目）。
    # rows是[(urlid1,wordlocation1_1,wordlocation1_2,wordlocation1_3...),(urlid2,wordlocation2_1,wordlocation2_2,wordlocation2_3...)]
    ...
    <p>
     <img alt="这里写图片描述" src="https://i-blog.csdnimg.cn/blog_migrate/70561b35be5073f0c4dfe31a435be221.jpeg"/>
    </p>
    <p>
    ...
    <pre><code># （每个链接的pagerank=指向此链接的网页的pagerank/网页中的链接总数*0.85+0.15，其中0.85表示阻尼因子，表示网页是否点击该网页中的链接）
    ...
    <pre><code># 根据pagerank值进行评价的函数。（利用外部回值链接进行评价）。
# rows是[(urlid1,wordlocation1_1,wordlocation1_2,wordlocation1_3...),(urlid2,wordlocation2_1,wordlocation2_2,wordlocation2_3...)]
    ...
    <pre><code># 对查询到的链接进行排名。参数：rows，wordids查询字符串id列表
...
#根据urlid查询url
    ...
    <pre><code>mynet=nn.searchnet('csdn.db')
if __name__ == '__main__':
    ...
    <pre><code>3.421882	56	http://blog.csdn.net/luanpeng825485697/article/details/78451057
...
if __name__ == '__main__':
    ...
    <pre><code># 根据神经网络（用户点击行为学习）进行评价的函数。神经网络在nn.py中实现。
    # rows是[(urlid1,wordlocation1_1,wordlocation1_2,wordlocation1_3...),(urlid2,wordlocation2_1,wordlocation2_2,wordlocation2_3...)]
 ...
 <p alt="68747470733a2f2f626c6f672e63:73646e2e6e65742f6c75616e70656e67383235343835363937:2f61727469636c652f64657461696c732f3738393937313839" class_="artid" style="display:none">
 </p>
</div>
