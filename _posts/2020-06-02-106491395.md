...
<span class="token keyword">throw</span> <span class="token string">"topic 不能为空"</span><span class="token punctuation">;</span>
...
<span class="token comment">// topic 需要和接收者的 topic 相同，才能接收到消息，此处 user 是我用来确定发布和接收者唯一性的参数</span>
...
<span class="token comment">// message 就是需要发送的消息内容，需要 string 类型</span>
...
<pre><code class="prism language-javascript"><span class="token comment">// 初始化 mqtt 连接等</span>
