...
const swconf = {
  {{ if site.pwa.cache.enabled }}
    ...
    resources: [
      '{{ "/assets/css/:THEME.css" | replace: ':THEME', site.theme | prepend: site.base_url | relative_url }}',
      '{{ "/" | prepend: site.base_url | relative_url }}',
      {{ for tab in site.tabs }}
        '{{- tab.url | prepend: site.base_url | relative_url -}}',
      {{ endfor }}

      {{ assign cache_list = site.static_files | where: 'swcache', true }}
      {{ for file in cache_list }}
        '{{ file.path | prepend: site.base_url | relative_url }}'{{- unless forloop.last -}},{{- endunless -}}
      {{ endfor }}
    ],
    ...
    interceptor: {
      ...
      paths: [
        {{ for path in site.pwa.cache.deny_paths }}
          {{ unless path == empty }}
            '{{ path | prepend: site.base_url | relative_url }}'{{- unless forloop.last -}},{{- endunless -}}
          {{ endunless  }}
        {{ endfor }}
      ],
